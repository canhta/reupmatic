...
class SynthesisContractTests(unittest.TestCase):
    def test_strict_spoken_input(self):
        ...
        for patch in ({'source_layer': 'displayed'}, {'language': 'zh'}, {'language': []},
                      {'model_id': '../model'}, {'source_token': 'x'}, {'voice_id': ''}, {'path': '/private'},
                      {'cues': []}, {'cues': [p['cues'][0], p['cues'][0]]},
    ...
    def test_frame_spans_must_exactly_cover_all_cues_and_gaps(self):
        p = params('a'*64)
        valid = {'frames': 21600, 'segments': [{'cue_id': 'cue-1', 'start_frame': 0, 'end_frame': 4800},
                 {'cue_id': 'cue-2', 'start_frame': 16800, 'end_frame': 21600}], 'runtime': 'controlled-sdk'}
        self.assertEqual(validate_audio(valid, p), valid)
        for patch in ({'frames': True}, {'frames': 0}, {'frames': 21599}, {'segments': []},
                      {'segments': valid['segments'][::-1]}, {'runtime': ''}, {'path': '/source'}):
            with self.subTest(patch=patch), self.assertRaisesRegex(WorkerError, 'MODEL_OUTPUT_INVALID'):
...
class SynthesisModelTests(unittest.TestCase):
    ...
    def test_numeric_voice_data_must_be_bounded_finite_and_nonzero(self):
        ...
        for patch in ({'speaker_emb': [0]*192}, {'speaker_emb': [float('nan')]*192},
                      {'speaker_emb': [1]*191}, {'ref_codes': [[-1]*8]}, {'ref_codes': [[]]},
                      {'ref_codes': [[1]*8, [1]*7]}, {'id': 'bad\x00id'}):
            value = copy.deepcopy(original)
            value['voices'][0].update(patch)
            path.write_text(json.dumps(value))
            with self.subTest(patch=patch), self.assertRaisesRegex(WorkerError, 'SYNTHESIS_VOICES_INVALID'):
