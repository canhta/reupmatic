"""Published translation schemas match the typed host and worker shapes."""
import copy
...
class TranslationContracts(unittest.TestCase):
    ...
    def test_public_and_worker_shapes_reject_paths_styles_auto_and_unknown_options(self):
        ...
        request = {'v': 1, 'id': 'request-123', 'revision': 2, 'method': 'speech.translate', 'params': options()}
        self.worker.validate(request)
        for change in [{'path': '/source.srt'}, {'target_language': 'auto'}, {'model_id': 'remote'},
                       {'cues': []}, {'rules': [{'find': 'x', 'replace': 'y', 'regex': True}]}]:
        ...
        self.worker.validate({**request, 'method': 'translation.configure', 'params': {'path': '/manifest.json'}})
    ...
    def test_plain_result_and_lightweight_status_are_explicit(self):
        ...
        status = {'available': True, 'code': None, 'model_id': 'a' * 64,
                  'source_language': 'en', 'target_language': 'vi', 'verified': False}
        self.validator(self.defs['status']).validate(status)
        for change in [{'verified': True}, {'model_id': None}, {'target_language': None}, {'code': 'MODEL_MISSING'}]:
    ...
    def test_current_project_and_text_layer_versions_keep_translation_provenance(self):
        origin = {'kind': 'translation', 'layer': 'transcript', 'token': 'source-123', 'request_id': 'request-123',
                  'source_language': 'en', 'target_language': 'vi', 'model_id': 'a' * 64,
                  'runtime': 'controlled@1', 'rules': [{'find': 'x', 'replace': 'y'}], 'policy': 'keep-existing'}
        self.validator(self.defs['origin']).validate(origin)
        meta = {'token': 'target-123', 'language': 'vi', 'origin': {'kind': 'manual'}, 'edited': False, 'stale': False}
        layers = {'version': 2, 'displayed': meta,
        ...
        self.assertFalse(schema.is_valid({**project, 'version': 4}))
        self.assertFalse(schema.is_valid({**project, 'text_layers': {**layers, 'version': 1}}))
    ...
    def test_bundle_accepts_exactly_one_hashed_vocabulary_layout(self):
        ...
        bundle = {'version': 1, 'engine': 'ctranslate2-sentencepiece', 'directory': 'local/model',
                  'source_language': 'en', 'target_language': 'vi', 'files': {**files, 'shared_vocabulary.json': 'a' * 64}}
        schema.validate(bundle)
        schema.validate({**bundle, 'files': {**files, 'source_vocabulary.json': 'a' * 64, 'target_vocabulary.json': 'b' * 64}})
        ...
        self.assertFalse(schema.is_valid({**bundle, 'files': {**bundle['files'], 'source_vocabulary.json': 'a' * 64}}))
