...
import { changeEditor, openEditorHistory, redoEditor, undoEditor } from '../dist-core/projects/editor-history.js';

const initial = { cues: [{ id: 'cue', start_ms: 0, end_ms: 1000, text: 'Tiếng Việt' }], sample: { start_ms: 0, end_ms: 1000 } };
test('editor undo restores video/audio, sample and subtitle changes as whole snapshots', () => {
  ...
  let state = changeEditor(opened, { processing: { version: 1, editing: { speed: 2, audio: { muted: true, gain_db: 0 } } } });
  state = changeEditor(state, { sample: { start_ms: 500, end_ms: 1000 }, cues: [{ ...initial.cues[0], text: 'Changed' }] });
...
test('no-op changes retain identity, history is bounded and branch edits invalidate redo', () => {
  ...
  let state = opened;
  for (let n = 1; n < 80; n++) state = changeEditor(state, { sample: { start_ms: n, end_ms: 1000 } });
  assert.equal(state.past.length, 60);
  const branch = changeEditor(undoEditor(state), { processing: { version: 1, editing: { speed: 2 } } });
