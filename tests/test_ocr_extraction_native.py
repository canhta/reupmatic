...
class OcrExtractionNativeTests(VisionFixture, unittest.TestCase):
    def params(self, aid, duration=1000):
        return {"asset_id": aid, "start_ms": 0, "end_ms": duration,
                "language": "en", "sample_ms": 2000, "min_confidence": .5}

    def test_whole_source_exceeds_sample_limit_merges_cues_and_retains_raw_evidence(self):
        self.source.unlink()
        subprocess.run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i", "color=black:s=160x90:r=6:d=121",
                        "-c:v", "libx264", "-threads", "2", "-an", "-n", str(self.source)], check=True)
        ...
        self.assertEqual(result["cues"], [{"id": "ocr-000001", "start_ms": 0, "end_ms": 121000, "text": "Fixture OCR"}])
        ...
        observations = sum(len(json.loads((directory / chunk["file"]).read_text())["observations"]) for chunk in chunks)
