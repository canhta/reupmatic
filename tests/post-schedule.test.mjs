...
import { formatPlannedTime, plannedCandidates, readPlanDraft } from '../dist-core/distribution/post-schedule.js';
...
test('planned wall time uses the chosen timezone, not UI locale or machine zone', () => {
  ...
  assert.equal(new Date(plannedCandidates('2026-09-16T10:30', 'Asia/Kathmandu')[0]).toISOString(), '2026-09-16T04:45:00.000Z');
});
...
test('nonexistent and repeated daylight-saving hours are never chosen silently', () => {
  ...
  assert.deepEqual(values.map(value => new Date(value).toISOString()), ['2026-11-01T05:30:00.000Z', '2026-11-01T06:30:00.000Z']);
  assert.throws(() => readPlanDraft({ enabled: true, local: '2026-11-01T01:30:00', timezone: 'America/New_York', instant: '' }), /INVALID_PLAN/);
});
...
test('invalid dates, zones and forged local/instant pairs fail closed', () => {
  ...
  assert.throws(() => readPlanDraft({ enabled: true, local: '2026-09-16T12:00', timezone: 'UTC', instant: '1' }), /INVALID_PLAN_TIME/);
});
...
test('unchanged drafts preserve exact stored milliseconds and optional plans remain optional', () => {
  ...
  const draft = { enabled: true, local: '2026-09-16T10:30:05', timezone: 'UTC', instant: String(instant) };
