...
import { collectFiles, FolderMonitor, within } from '../dist-core/folders/folder-monitor.js';
...
async function create(f, changes = {}) {
  const result = await f.monitor.create({ ...f.config, ...changes });
...
test('folder config saves paused; access gate is checked at create and start', async (t) => {
  ...
  assert.equal(f.monitor.snapshot().rules[0].state, 'paused');
...
test('stable file is admitted once; intake does not unpause the shared queue', async (t) => {
  ...
  const id = await create(f);
  await f.monitor.start(id);
  assert.equal(f.batchStore.list().length, 0);
  f.advance();
  await f.monitor.reconcile(id);
...
test('receipt failure after queue commit retries the same admission ID', async (t) => {
  const f = await fixture(t);
  await fs.writeFile(path.join(f.source, 'a.mp4'), 'content');
  const acknowledge = f.store.acknowledge.bind(f.store);
  let fail = true;
  f.store.acknowledge = (...args) => {
    if (fail) {
      fail = false;
      throw new RemoteError('STORAGE_FAILED');
    }
  ...
  f.advance();
  await f.monitor.reconcile(id);
  const jobId = f.batchStore.list()[0].id;
  assert.equal(f.monitor.snapshot().rules[0].admitted, 0);
  await f.monitor.reconcile(id);
  assert.equal(f.batchStore.list().length, 1);
  assert.equal(f.batchStore.list()[0].id, jobId);
  assert.equal(f.monitor.snapshot().rules[0].admitted, 1);
});
...
test('pause stops new intake without cancelling accepted jobs', async (t) => {
  ...
  f.advance();
  await f.monitor.reconcile(id);
  await f.monitor.pause(id);
  await fs.writeFile(path.join(f.source, 'b.mp4'), 'other');
  await f.monitor.reconcile(id);
  assert.equal(f.monitor.snapshot().rules[0].state, 'paused');
  assert.equal(f.batchStore.list()[0].state, 'queued');
  assert.equal(f.batchStore.list().length, 1);
...
test('restart preserves baseline/receipts, stays paused, then admits files missed while off', async (t) => {
  ...
  try {
    assert.equal(monitor.snapshot().rules[0].state, 'paused');
    await monitor.start(id);
    f.advance();
    await monitor.reconcile(id);
    assert.equal(f.batchStore.list().length, 1);
    assert.equal(f.batchStore.list()[0].input.video.name, 'offline.mp4');
  } finally {
    await monitor.close();
  }
});
...
test('reject equal roots, source inside output and linked automation loops', async (t) => {
  ...
  await assert.rejects(f.monitor.create({ ...f.config, source_dir: f.output, output_dir: other }), {
    code: 'WATCH_OUTPUT_LOOP',
  });
...
test('pause during async watcher attachment closes the late handle and admits nothing', async (t) => {
  ...
  const id = await create(f);
  const pending = f.monitor.start(id);
  await started;
  await f.monitor.pause(id);
  release();
  await pending;
