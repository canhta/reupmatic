...
test('real shared coordinator: no-subtitle render, imported SRT, cache and failed-item isolation', async () => {
  ...
  const video = path.join(dir, 'video Việt.mp4'); const subtitle = path.join(dir, 'track.srt');
  execFileSync(process.env.FFMPEG_PATH || 'ffmpeg', ['-v', 'error', '-f', 'lavfi', '-i',
    'testsrc2=size=160x90:rate=30:duration=2', '-c:v', 'libx264', '-threads', '2', '-n', video]);
  await writeFile(subtitle, '1\n00:00:00,100 --> 00:00:01,800\nReupmatic — Tiếng Việt\n');
  const hash = (data) => createHash('sha256').update(data).digest('hex');
  const before = hash(await readFile(video));
  const client = new WorkerClient(pythonExecutable(root), path.join(root, 'worker/main.py'), path.join(dir, 'workspace'));
  const coordinator = new RenderCoordinator(client); const events = [];
  coordinator.on('job', (event) => events.push(event));
  try {
    const source = await client.request('asset.register', { path: video, kind: 'video' }).result;
    const subs = await client.request('asset.register', { path: subtitle, kind: 'subtitle' }).result;
    const request = () => ({ request_id: randomUUID(), asset_id: source.asset_id, revision: 3,
      mode: 'sample', start_ms: 500, end_ms: 1500, encoding: 'lossless', cues: [] });
    const first = await coordinator.start(request()).result;
    assert.equal(first.duration_ms, 1000); assert.equal(first.cache_hit, false);
    ...
    await assert.rejects(coordinator.start({ ...request(), asset_id: 'missing' }).result, { code: 'UNKNOWN_ASSET' });
    assert.equal((await coordinator.start(request()).result).cache_hit, true);
    assert.equal(coordinator.activeCount, 0);
    assert.equal(events.filter((event) => event.event === 'result').length, 4);
    assert.ok(events.filter(event => event.event === 'result')
    ...
    await coordinator.close(); await client.stop();
...
test('real shared coordinator registers composition clips and preserves captured provenance', async () => {
  ...
  execFileSync(process.env.FFMPEG_PATH || 'ffmpeg', ['-v', 'error', '-f', 'lavfi', '-i',
    'testsrc2=size=160x90:rate=30:duration=2', '-c:v', 'libx264', '-threads', '2', '-n', filename]);
  const sha256 = createHash('sha256').update(await readFile(filename)).digest('hex');
  const client = new WorkerClient(pythonExecutable(root), path.join(root, 'worker/main.py'), path.join(dir, 'workspace'));
  const coordinator = new RenderCoordinator(client), events = [];
  coordinator.on('job', event => events.push(event));
  try {
    ...
    const composition = { version: 1, canvas: { width: 160, height: 90, fps: 30 }, clips: [
      { id: 'first', source, start_ms: 0, end_ms: 1000, speed: 1 },
      { id: 'second', source, start_ms: 1000, end_ms: 2000, speed: 2 },
    ] };
    const request = { request_id: randomUUID(), asset_id: asset.asset_id, revision: 12, mode: 'sample',
      start_ms: 500, end_ms: 1250, encoding: 'review', cues: [], composition };
    ...
    assert.deepEqual(event.data.source_composition.clips.map(clip => clip.id), ['first', 'second']);
    assert.equal(event.revision, 12);
    assert.equal(createHash('sha256').update(await readFile(filename)).digest('hex'), sha256);
    assert.equal(coordinator.activeCount, 0);
  } finally {
    await coordinator.close(); await client.stop(); await rm(dir, { recursive: true, force: true });
  }
});
