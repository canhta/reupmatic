...
import { parseProcessingRecipe, parseModelFingerprints, requiredModels } from '../dist-core/processing/recipe.js';
...
test('recipes are canonical, independent snapshots, not executable configuration', () => {
  ...
  for (const bad of [null, [], {}, { version: 2, ocr }, { version: 1 },
    { version: 1, ocr, command: 'run' }, { version: 1, ocr: { ...ocr, sample_ms: NaN } },
...
test('manual subtitles cannot be replaced by automatic OCR', () => {
  assert.throws(() => parseProcessingRecipe({ version: 1, ocr }, true), /PROCESSING_SUBTITLE_CONFLICT/);
  assert.doesNotThrow(() => parseProcessingRecipe({ version: 1,
    inpaint: { target: 'manual', padding_px: 4, region } }, true));
});
...
test('model snapshots match exactly the requirements of the saved recipe', () => {
  ...
  assert.deepEqual(Object.keys(parseModelFingerprints(models, recipe)), ['inpainting', 'ocr_en', 'ocr_vi']);
  for (const value of [{}, { ...models, ocr_zh: 'a'.repeat(64) }, { ...models, inpainting: 'bad' }]) {
