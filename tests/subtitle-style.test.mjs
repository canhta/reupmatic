...
import { defaultSubtitleStyle, parseSubtitleStyle, applyCueStyle } from '../dist-core/subtitles/style.js';
...
test('global and cue-specific subtitle styles survive project, processing and split boundaries', () => {
  const style = parseSubtitleStyle({ ...defaultSubtitleStyle, font_family: 'DejaVu Sans', position: 8 });
  ...
  assertCues(cues);
  const project = createProject({ path: '/video.mp4', sha256: 'a'.repeat(64) }, {
    cues, sample: { start_ms: 0, end_ms: 2000 }, processing,
  });
...
test('style validation rejects ASS injection, invalid opacity, unsupported keys and non-finite numbers', () => {
  for (const patch of [{ font_family: 'Arial\nStyle:bad' }, { font_family: 'Arial,10' },
    { text_color: 'red' }, { box_opacity: 1.1 }, { position: 0 }, { font_size_pct: NaN },
    { margin_x_pct: 50 }, { bold: 1 }, { command: 'anything' }]) {
    assert.throws(() => parseSubtitleStyle({ ...defaultSubtitleStyle, ...patch }), /INVALID_SUBTITLE_STYLE/);
  }
