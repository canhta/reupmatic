...
class EditingNativeTests(unittest.TestCase):
    def setUp(self):
        ...
        subprocess.run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i", "testsrc2=s=320x180:r=30:d=4",
                        "-f", "lavfi", "-i", "sine=frequency=440:duration=4", "-c:v", "libx264",
                        "-threads", "2", "-c:a", "aac", "-shortest", "-n", str(self.source)], check=True)
        self.original = hashlib.sha256(self.source.read_bytes()).hexdigest()
        self.session = Session(self.root / "workspace")
        self.asset = self.session.call("asset.register", {"path": str(self.source), "kind": "video"})["asset_id"]
    ...
    def render(self, editing, **params):
        ...
        return self.session.call("media.process", {"asset_id": self.asset, "mode": "full", "encoding": "review",
                                 "processing": processing, "model_fingerprints": {}, **params})

    def test_trim_speed_crop_portrait_and_mute_are_encoded_and_cached(self):
        editing = {"trim": {"start_ms": 500, "end_ms": 2500}, "speed": 2,
                   "crop": {"x": .25, "y": 0, "width": .5, "height": 1}, "flip": "horizontal",
                   "audio": {"muted": True, "gain_db": 0},
                   "color": {"brightness": .1, "contrast": 1.1, "saturation": .8},
        ...
        probe = json.loads(subprocess.check_output(["ffprobe", "-v", "error", "-show_streams", "-of", "json", result["path"]]))
    ...
    def test_audio_gain_and_sample_intersection_share_source_timing(self):
        editing = {"trim": {"start_ms": 500, "end_ms": 2500}, "speed": .5,
        ...
        quieter = self.render({**editing, "audio": {"muted": False, "gain_db": -6}},
                              mode="sample", start_ms=1000, end_ms=3000)
        self.assertEqual(result["duration_ms"], 3000)
        self.assertTrue(result["has_audio"])
        def rms(filename):
            data = array.array("f", subprocess.check_output(["ffmpeg", "-v", "error", "-i", filename,
                               "-map", "0:a:0", "-ac", "1", "-f", "f32le", "-"]))
            return math.sqrt(sum(value*value for value in data) / len(data))
        self.assertAlmostEqual(rms(quieter["path"])/rms(result["path"]), 10**(-6/20), delta=.04)
    ...
    def test_invalid_edit_does_not_publish_or_poison_following_jobs(self):
        with self.assertRaisesRegex(RuntimeError, "EDIT_EMPTY_RANGE"):
            self.render({"trim": {"start_ms": 2000, "end_ms": 3000}}, mode="sample", start_ms=0, end_ms=1000)
    ...
    def test_subtitles_follow_source_time_before_speed_change(self):
        ...
        sid = self.session.call("asset.register", {"path": str(subtitles), "kind": "subtitle"})["asset_id"]
        ...
        def frame(filename, time):
            return subprocess.check_output(["ffmpeg", "-v", "error", "-i", filename, "-ss", str(time),
                "-frames:v", "1", "-pix_fmt", "rgb24", "-f", "rawvideo", "-threads", "1", "-"])
        def difference(time):
            first, second = frame(plain["path"], time), frame(captioned["path"], time)
            return sum(abs(a-b) for a, b in zip(first, second))/len(first)
        self.assertGreater(difference(.35), difference(.1) + .15)
