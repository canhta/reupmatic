"""Real shared worker and subprocess tests using CONTROLLED SDK/weight doubles."""
import hashlib
import json
import os
import sys
...
class TranslationNativeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest.write_text(json.dumps({'version': 1, 'engine': 'ctranslate2-sentencepiece',
            'directory': str(self.model), 'source_language': 'en', 'target_language': 'vi', 'files': files}))
    ...
    def session(self, **environment):
        env = {**os.environ, 'PYTHONPATH': str(self.sdk),
               'CONTROLLED_TRANSLATION_PID': str(self.root / 'child.pid'), **environment}
        session = Session(self.workspace, env=env)
        self.sessions.append(session)
        status = session.call('translation.configure', {'path': str(self.manifest)})
        self.assertTrue(status['available'])
        self.assertFalse(status['verified'])
        params = {'source_layer': 'transcript', 'source_token': 'source-12345678',
            'source_language': 'en', 'target_language': 'vi', 'model_id': status['model_id'],
    ...
    def test_real_worker_child_keeps_ids_times_languages_rules_and_runtime(self):
        ...
        for key in ('source_token', 'model_id', 'rules', 'source_layer', 'source_language', 'target_language'):
