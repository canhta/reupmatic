...
test('voice UI keeps explicit generation, library controls and no document mutation', async () => {
  ...
  assert.match(panel,/job\.start\(\{ language, voice_id: voice/);
  ...
  assert.match(review,/<audio controls preload="metadata"/);
...
test('native synthesis boundary exposes allowlisted verbs, not arbitrary paths', async () => {
  ...
  assert.match(main,/synthesis\.activeCount/); assert.match(main,/synthesis\.close\(\)/);
});
...
test('EN/VI voice strings cover all task keys and truthful non-alignment language', async () => {
  ...
  assert.match(strings,/not alignment/); assert.match(strings,/chưa căn thời gian/);
  const components = await Promise.all(['SynthesisPanel','SynthesisReview'].map(name=>source(`app/ui/features/speech/synthesis/${name}.tsx`)));
  for (const text of components) for (const match of text.matchAll(/t\('(synthesis\w+)'/g)) assert.ok(keys(en).includes(match[1]),match[1]);
});
