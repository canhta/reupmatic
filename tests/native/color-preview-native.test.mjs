// The colour preview must mirror what the worker's FFmpeg `eq` filter actually
// emits. This renders real frames and compares them with the same matrix the
// UI hands to SVG feColorMatrix, so the two paths are checked against each
// other rather than against a re-statement of the formula.
...
// Both FFmpeg and the preview are pinned to full-range BT.709 so the only
// difference left is FFmpeg's own rounding, not a colorimetry mismatch.
const FULL_RANGE_BT709 =
  'scale=in_range=full:out_range=full:in_color_matrix=bt709:out_color_matrix=bt709';

/** A broad, mid-range pattern: no channel clips, in the source or after `eq`. */
...
/** Applies an feColorMatrix row-major matrix the way the browser does. */
...
// `eq` builds its brightness offset from the truncated float
// `(int)(100 * brightness + 100) * 511 / 200`, so e.g. -0.2 lands on -55/255
// rather than -51/255; it also rounds through 8-bit YUV, which chroma scaling
// amplifies. The preview models the exact formula, not those rounded
// intermediates, so the two agree to about 2% on average (~5/255) and 4% at
// worst (~10/255) — the ticket's stated tolerance for "exact enough".
