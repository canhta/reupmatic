...
async function native(t) {
  ...
  const { stdout } = await run(
    ...
    [
      '-c',
      'from pathlib import Path; import sys,json; from synthesis_fixture import bundle,sdk; root=Path(sys.argv[1]); manifest,_=bundle(root); print(json.dumps({"manifest":str(manifest),"sdk":str(sdk(root))}))',
      root,
    ],
    { env: { ...process.env, PYTHONPATH: path.join(repo, 'tests/python') } },
  );
  ...
  const input = {
    ...
    params: {
      ...
      voice_id: 'test-voice',
      cues: [{ id: 'voice-1', text: 'Xin chào', start_ms: 5000, end_ms: 6000 }],
    },
  };
  const artifacts = new SynthesisArtifacts(workspace),
    coordinator = new SynthesisCoordinator(worker, artifacts);
...
test('real worker failure preserves an earlier admitted speech artifact and stays usable', async (t) => {
  ...
  const retry = {
    ...f.input,
    request_id: randomUUID(),
    params: { ...f.input.params, model_id: 'f'.repeat(64) },
  };
