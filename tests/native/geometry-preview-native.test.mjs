// The rotate/crop/flip preview must mirror what the worker's FFmpeg framing
// chain actually emits. This drives the real TypeScript -> Python -> FFmpeg
// bridge with a quadrant-pattern source, then checks that every sampled output
// pixel shows the source region the preview's own mapping (`mapOutputToSource`)
// says it should. Preview and export are compared against each other, not
// against a re-statement of the formula.
...
// Solid quadrants make the test robust to the worker's even-pixel rounding:
// only which quadrant a sampled point lands in matters, not the exact pixel.
const QUADRANTS = [
...
/** The quadrant a source point lands in, or null when it is too close to a boundary to judge. */
...
test('the preview mapping agrees with a real worker render for rotate, crop and flip', {
  ...
  try {
    ...
    for (const { label, editing } of CASES) {
      ...
      let checked = 0;
      // A 7x7 grid; points the preview maps to a quadrant boundary (or to pad)
      // are skipped, because a single pixel there is ambiguous by design.
