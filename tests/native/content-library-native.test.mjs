...
test('real Library -> relink -> saved project -> SQLite batch -> FFmpeg with audio', {
  timeout: 60000,
}, async () => {
  const directory = await realpath(
    await mkdtemp(path.join(os.tmpdir(), 'reupmatic-library-native-')),
  );
  ...
  let library = new ContentLibrary(dbPath, path.join(directory, 'managed'), {
  ...
  try {
    ...
    const copied = await library.importOriginal(source, {
      mode: 'copy',
      duplicates: 'separate',
    });
    ...
    const { source: resolved } = await library.resolveOriginal(item.id);
    ...
    assert.equal(await hashFile(copied.item.path), before);
    library.close();
    library = new ContentLibrary(dbPath, path.join(directory, 'managed'), {
      inspectOriginal: inspect,
    });
    assert.equal(library.getContent(item.id).links.length, 2);
    assert.equal(library.getContent(item.id).path, moved);
    assert.equal(library.getContent(item.id).availability, 'unchecked');
