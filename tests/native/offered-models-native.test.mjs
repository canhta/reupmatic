...
// A stand-in for the real installer so this suite drives an install's lifecycle without moving a
// byte. It hands back the controls the host would otherwise only expose to a real transfer.
...
test('a running install belongs to the host, so a panel that goes away and returns re-attaches to it', async (t) => {
  ...
  // The panel unmounts here — a Settings category switch, or the screen closing. It sends no
  // cancel, and the host must still be transferring.
...
test('activating a downloaded model re-runs its own configure against the manifest on disk', async (t) => {
  ...
  // Nothing downloaded yet: activating is refused with the missing-model code, before any
  // configure runs, and an id not in the catalogue is refused as not offered.
  ...
  // The install writes one manifest per entry; once it is on disk, activate routes the entry's
  // own task to the matching configure transaction, pointing at that manifest.
...
test('removing a downloaded entry forgets its engine store, then deletes the bundle', async (t) => {
  ...
  // Nothing downloaded: there is nothing to remove, and no store is touched.
  ...
  // A synthesis entry routes to its own store's unconfigure.
