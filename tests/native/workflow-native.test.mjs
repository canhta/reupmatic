...
test('real saved workflow -> shared SQLite queue -> FFmpeg -> Library export -> one local destination post', {
  timeout: 45000,
}, async () => {
  const directory = await realpath(
    await mkdtemp(path.join(os.tmpdir(), 'reupmatic-workflow-native-')),
  );
  ...
  const library = new ContentLibrary(
    path.join(directory, 'library.sqlite'),
    path.join(directory, 'managed'),
    {
      inspectOriginal: async (filename) => {
        const asset = await worker.request('asset.register', { path: filename, kind: 'video' })
          .result;
  ...
  const catalog = new WorkspaceCatalog(catalogPath, async (contentId, exportId) => {
    const item = library.getContent(contentId);
    if (!item.links.some((link) => link.id === exportId && link.kind === 'export'))
      throw new Error('INVALID_EXPORT');
    return library.resolveAsset(contentId, exportId);
  });
  const queue = new BatchQueue(worker, renderer, {
    databasePath: path.join(directory, 'batch.sqlite'),
  });
  const service = new WorkflowService(catalog, {
    queue,
    resolveLibrary: async (id) => (await library.resolveOriginal(id)).source,
    async checkOutput() {},
    async resolveModels() {
      throw new Error('No models should run in this plain workflow');
    },
  });
  try {
    ...
    catalog.assignContentLabel({ id: item.id, expected_revision: null, label_ids: [label.id] });
    const workflow = catalog.saveWorkflow(
      {
        id: 'workflow_native_01',
        expected_revision: null,
        name: 'Xuất bản local',
        item_ids: [item.id],
        processing: null,
    ...
    const job = queue.get(queue.snapshot().items[0].id);
    assert.equal(job.state, 'complete', job.error_code);
    assert.equal(
      workflowRunState(catalog.snapshot(true).runs[0], queue.snapshot().items),
      'complete',
    );
    await library.registerAsset(item.id, 'export', job.output.path, job.output.sha256);
    const exported = library.getContent(item.id).links.find((link) => link.kind === 'export');
    assert.ok(library.exportChoices().some((choice) => choice.export_id === exported.id));
    const channel = catalog.saveChannel({
      id: 'channel_native_01',
      expected_revision: null,
      name: 'Kênh thử',
      platform: 'youtube',
    ...
    const link = catalog.saveLink({
      id: 'link_native_01',
      expected_revision: null,
      name: 'Link thủ công',
      url: 'https://shopee.vn/example?af_siteid=original',
      label_ids: [label.id],
      archived: false,
    });
    const post = await catalog.createPost({
      id: 'post_native_01',
      expected_revision: null,
      title: 'Nội dung thử',
      body: '',
    ...
    const probe = JSON.parse(
      execFileSync(
        ...
        ['-v', 'error', '-show_streams', '-of', 'json', exported.path],
        { encoding: 'utf8' },
      ),
    );
    assert.ok(probe.streams.some((stream) => stream.codec_type === 'audio'));
    catalog.close();
    const reopened = new WorkspaceCatalog(catalogPath, async () => {
      throw new Error('UNUSED_EXPORT');
    });
    try {
      assert.equal(reopened.getPost(post.id).export.sha256, job.output.sha256);
      assert.equal(reopened.snapshot(true).runs[0].job_ids[0], job.id);
      assert.equal(
        reopened.listPosts({ search: '', view: 'published', offset: 0, limit: 25 }).total,
        0,
      );
    } finally {
      reopened.close();
