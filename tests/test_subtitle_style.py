...
class SubtitleStyleTests(unittest.TestCase):
    def test_styles_are_strict_and_style_only_recipe_has_no_models(self):
        ...
        for patch in ({'font_family': 'Arial,10'}, {'font_family': 'Arial\nOverride'}, {'text_color': 'red'},
                      {'box_opacity': 1.1}, {'position': True}, {'position': 1.5}, {'bold': 1},
                      {'font_size_pct': float('nan')}, {'unknown': 1}):
            with self.subTest(patch=patch), self.assertRaisesRegex(WorkerError, 'INVALID_SUBTITLE_STYLE'):
    ...
    def test_percent_to_ass_unit_mapping_and_box_alpha(self):
        adapter = SimpleNamespace(Color=lambda *args: args, Alignment=int, SSAStyle=lambda **args: args)
        style = ass_style(adapter, {**DEFAULT_STYLE, 'box_opacity': .5}, 720, 1280)
...
class SubtitleSerializationTests(unittest.TestCase):
    def test_global_and_override_styles_are_serialized_at_frame_size(self):
        cues = [{'id': '1', 'start_ms': 100, 'end_ms': 1900, 'text': 'Tiếng Việt'},
                {'id': '2', 'start_ms': 2000, 'end_ms': 3000, 'text': 'English',
                 'style': {**DEFAULT_STYLE, 'bold': True, 'position': 8}}]
        document = cue_document(cues, {**DEFAULT_STYLE, 'font_size_pct': 5}, {'width': 720, 'height': 1280})
    ...
    def test_srt_retains_literal_text_and_never_emits_global_style(self):
        ...
        cues = [{'id': '1', 'start_ms': 123, 'end_ms': 1456, 'text': text,
