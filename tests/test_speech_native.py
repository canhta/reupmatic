...
class SpeechNativeTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(['ffmpeg', '-v', 'error', '-f', 'lavfi', '-i', 'color=s=160x90:r=24:d=3',
                        '-itsoffset', '1', '-f', 'lavfi', '-i', 'sine=frequency=440:duration=2',
                        '-c:v', 'libx264', '-threads', '2', '-c:a', 'pcm_s16le', '-n', str(cls.source)], check=True)
        subprocess.run(['ffmpeg', '-v', 'error', '-i', str(cls.source), '-map', '0:v:0',
                        '-c', 'copy', '-an', '-n', str(cls.silent)], check=True)
    ...
    def setUp(self):
        ...
        self.manifest.write_text(json.dumps({'version': 1, 'engine': 'faster-whisper',
                                            'directory': str(self.model), 'languages': ['vi'], 'files': files}))
    ...
    def session(self, **environment):
        ...
        params = {'asset_id': asset['asset_id'], 'source_sha256': asset['sha256'],
                  'model_id': status['model_id'], 'language': 'vi', 'start_ms': 1000, 'end_ms': 2000}
    ...
    def test_controlled_recognition_returns_correlated_source_clock_unicode_and_provenance(self):
        ...
        self.assertEqual(result['cues'][0], {'id': 'stt-1', 'start_ms': 1100, 'end_ms': 1300,
                                            'text': 'Xin chào Việt Nam'})
        self.assertEqual(result['source_sha256'], self.original)
        self.assertEqual(result['model_id'], params['model_id'])
        self.assertEqual((result['clock'], result['timing'], result['language']), ('source', 'segment', 'vi'))
        self.assertEqual(result['runtime'], 'faster-whisper@0.0.0;ctranslate2@0.0.0')
        terminals = [event for event in session.events if event.get('data', {}).get('kind') == 'stt']
    ...
    def test_cancel_reaps_real_child_cleans_audio_and_unblocks_shared_queue(self):
        ...
        while True:
            ...
            if message['id'] == request and message['event'] == 'progress' and message['data'].get('fraction'):
                break
    ...
    def test_language_fallback_is_refused_and_no_audio_is_explicit(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'NO_AUDIO'):
            session.call('speech.transcribe', {**params, 'asset_id': asset['asset_id'], 'source_sha256': asset['sha256']})
