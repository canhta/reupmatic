...
test('translation has matching typed preload/host channels and participates in close cancellation', () => {
  const bridge = read('app/ui/bridge.d.ts'), preload = read('app/electron/preload.cts');
  const host = read('app/electron/features/speech/translation/ipc.ts');
  for (const [method, channel] of [['translationStatus', 'translation-status'], ['translationStart', 'translation-start'],
    ['translationCancel', 'translation-cancel'], ['translationConfigure', 'translation-configure'],
  ...
  for (const method of ['onTranslationJob', 'onTranslationModelsChanged']) assert.match(bridge, new RegExp(`${method}\\(`));
...
test('draft review exposes manual keep, explicit replacement, all rows and final timings', () => {
  ...
  assert.match(source, /translationPrevious/); assert.match(source, /translationNext/);
...
test('translation hook retains draft on failed retry and does not apply from job events', () => {
  ...
  assert.match(read('app/ui/features/editor/EditorWorkspace.tsx'), /<TranslationPanel key={`translation-\${editor.documentId}`}/);
});

test('translation rule/setup forms use existing library primitives without starting work on selection', () => {
  const panel = ui('TranslationPanel.tsx'), rules = ui('TranslationRules.tsx');
  assert.match(panel, /pairAvailable/); assert.match(panel, /languageMismatch/);
  assert.match(panel, /translationLimits/); assert.match(panel, /translationPairMissing/);
  assert.match(panel, /onClick={\(\) => void job.start/);
  ...
  for (const name of ['TextInput', 'Button', 'Collapsible']) assert.match(rules, new RegExp(`core/${name}`));
});
...
test('dynamic translation phases/errors and capture consequences are bilingual and documented', () => {
  ...
  for (const key of ['translationRunning', 'translationSourceChanged', 'translationTargetMismatch',
    'translationTruncated', 'translationCapturedHelp', 'translationConfirm', 'translationStaleHelp']) {
  ...
  assert.match(read('docs/ui/astryx-component-map.md'), /0.14 translation review/);
});
