...
import { singleByteRange, registeredMediaResponse } from '../dist-core/media/media-response.js';
...
test('unsatisfiable, invalid and unsupported ranges are distinct', () => {
  for (const value of ['bytes=10-', 'bytes=-0', 'bytes=5-2']) assert.equal(singleByteRange(value, 10), 'unsatisfiable');
  assert.equal(singleByteRange('bytes=0-', 0), 'unsatisfiable');
  for (const value of [null, 'items=0-1', 'bytes=bad', 'bytes=0-1,4-5']) assert.equal(singleByteRange(value, 10), null);
});
test('registered file response serves actual ranges, HEAD, misses and cancel safely', async () => {
  ...
  const request = (headers = {}, method = 'GET') => new Request('https://local/video', { headers, method });
  try {
    ...
    const ifRange = await registeredMediaResponse(request({ Range: 'bytes=0-1', 'If-Range': 'old' }), filename);
    assert.equal(ifRange.status, 200); await ifRange.text();
    ...
    const cancelled = await registeredMediaResponse(request(), filename); await cancelled.body.cancel();
