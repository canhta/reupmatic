...
class ModelConfigurationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.host = SimpleNamespace(workspace=self.root / 'workspace',
        ...
        self.value = {'version': 1, 'inpainting': {'model': {
            'path': 'model.onnx', 'sha256': hashlib.sha256(self.weights.read_bytes()).hexdigest()}}}
        self.manifest.write_text(json.dumps(self.value), encoding='utf-8')

    def configure(self):
        with patch.dict(os.environ, {}, clear=True):
            return configure_models(self.host, {'id': 'configure', 'params': {'path': str(self.manifest)}})
    ...
    def test_empty_configuration_and_urls_are_not_accepted(self):
        ...
        with self.assertRaisesRegex(WorkerError, 'INVALID_REQUEST'):
            configure_models(self.host, {'id': 'config', 'params': {'path': 'https://example.test/model.json'}})
    ...
    def test_environment_override_is_explicit_and_does_not_get_overwritten(self):
        with patch.dict(os.environ, {'REUPMATIC_MODEL_MANIFEST': str(self.manifest)}):
            with self.assertRaisesRegex(WorkerError, 'MODEL_CONFIG_OVERRIDE'):
                configure_models(self.host, {'id': 'config', 'params': {'path': str(self.manifest)}})
