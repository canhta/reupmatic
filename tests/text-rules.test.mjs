...
test('literal rules handle Unicode, case and replacement dollar signs without regex expansion', () => {
  ...
  assert.equal(preview.matched_cues, 2); assert.equal(preview.replacements, 2);
...
test('regex groups and selected scope preview independently before apply', () => {
  const result = previewTextRule(cues, { mode: 'regex', find: '(Cà) (phê)', replacement: '$2 $1', case_sensitive: true }, ['a']);
  assert.equal(result.cues[0].text, 'phê Cà [1] $1'); assert.deepEqual(result.cues[1], cues[1]);
  assert.equal(result.changes[0].before, cues[0].text);
  assert.throws(() => previewTextRule(cues, { ...literal, mode: 'regex', find: '(' }), /TEXT_RULE_INVALID/);
...
test('zero-width regex terminates on Unicode input and result sizes are bounded', () => {
  assert.equal(previewTextRule([{ ...cues[0], text: '😀' }], { ...literal, mode: 'regex', find: '(?:)', replacement: '-' }).cues[0].text, '-😀-');
  assert.throws(() => previewTextRule(cues, { ...literal, find: ' ', replacement: 'x'.repeat(10000) }), /TEXT_RULE_LIMIT/);
});
