...
class TimedEvidenceTests(unittest.TestCase):
    def test_adjacent_text_merges_but_a_blank_frame_breaks_a_cue(self):
        ...
        def observation(start, text):
            return {'start_ms': start, 'end_ms': start + 500,
                    'detections': [{'text': text, 'confidence': .9, 'box': [1, 2, 30, 10]}] if text else []}
        cues = timed_cues([observation(1000, 'Tiếng Việt'), observation(1500, 'Tiếng Việt'),
                           observation(2000, ''), observation(2500, 'Tiếng Việt')])
        self.assertEqual([(c['start_ms'], c['end_ms'], c['text']) for c in cues],
...
class MaskGeometryTests(unittest.TestCase):
    def test_normalized_rectangle_uses_half_open_pixel_edges(self):
        ...
        mask = make_mask(100, 50, {'x': .1, 'y': .2, 'width': .3, 'height': .4}, [], 0)
...
class ModelRegistryTests(unittest.TestCase):
    def setUp(self):
        ...
        self.artifact = {'path':'local.onnx', 'sha256':hashlib.sha256(artifact.read_bytes()).hexdigest()}
    ...
    def test_unconfigured_vietnamese_is_not_replaced_with_chinese(self):
        ...
        from vision.models import ModelRegistry
        from runtime.errors import WorkerError
        entry = {'det':self.artifact,'rec':self.artifact,'keys':self.artifact,
                 'det_version':'PP-OCRv4','rec_version':'PP-OCRv4','rec_height':48}
    ...
    def test_url_and_unknown_manifest_fields_are_rejected(self):
        ...
        from vision.models import ModelRegistry
        from runtime.errors import WorkerError
        self.manifest.write_text(json.dumps({'version':1,'inpainting':{'model':{
            **self.artifact,'path':'https://example.invalid/model.onnx'}}}))
        ...
        self.assertEqual(ModelRegistry(self.manifest).status()['ocr']['code'],'MODEL_MANIFEST_INVALID')
...
class VisionValidationTests(unittest.TestCase):
    def test_invalid_numbers_regions_and_long_samples_are_rejected(self):
        ...
        from runtime.errors import WorkerError
        p={'asset_id':'v','start_ms':0,'end_ms':1000,'language':'en','sample_ms':500,'min_confidence':.5}
        for patch in ({'sample_ms':True},{'min_confidence':float('nan')},{'end_ms':130000},
                      {'shell':'run'},{'region':{'x':0,'y':0,'width':2,'height':1}}):
...
class ModelAdapterDoubleTests(unittest.TestCase):
    ...
    def session(self, shape=(1,3,512,512)):
        ...
        class SessionDouble:
            def get_inputs(self):
                return [SimpleNamespace(name='image',shape=list(shape),type='tensor(float)'),
    ...
    def test_lama_composites_only_masked_pixels_in_non_square_frame(self):
        ...
        mask=make_mask(160,90,{'x':.25,'y':.2,'width':.5,'height':.4},[],0)
    ...
    def test_empty_mask_keeps_all_pixels_unchanged(self):
        ...
        self.assertTrue(np.array_equal(LamaAdapter(self.session()).erase(image,np.zeros((10,20),dtype=np.uint8)),image))
    ...
    def test_ocr_crop_offsets_and_confidence_filtering(self):
        ...
        def engine(image,**kwargs):
            return SimpleNamespace(boxes=[[[0,0],[10,0],[10,5],[0,5]],[[1,1],[4,1],[4,4],[1,4]]],
                                   txts=['Tiếng Việt','noise'],scores=[.9,.1])
        out=RapidAdapter(engine).detect(np.zeros((100,200,3),dtype=np.uint8),rectangle={'x':.5,'y':.5,'width':.5,'height':.5})
        self.assertEqual(out,[{'text':'Tiếng Việt','confidence':.9,'box':[100,50,110,55]}])
