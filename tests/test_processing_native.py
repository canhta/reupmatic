...
class ProcessingNativeTests(VisionFixture, unittest.TestCase):
    def recipe(self):
        return {"version": 1, "inpaint": {"target": "manual", "padding_px": 0,
                "region": {"x": 0.25, "y": 0.2, "width": 0.5, "height": 0.4}}}

    def request(self, aid, **extra):
        return {"asset_id": aid, "mode": "full", "encoding": "review", "processing": self.recipe(), **extra}

    def test_full_video_crosses_chunk_boundary_preserving_audio_and_cache_identity(self):
        self.source.unlink()
        subprocess.run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i", "color=black:s=160x90:r=24:d=10.5",
                        "-f", "lavfi", "-i", "sine=frequency=600:duration=10.5", "-c:v", "libx264",
        ...
        count = subprocess.check_output(["ffprobe", "-v", "error", "-select_streams", "v:0", "-count_frames",
                                         "-show_entries", "stream=nb_read_frames", "-of", "json", result["path"]])
    ...
    def test_sample_combines_original_timed_subtitles_with_processed_video(self):
        ...
        sid = session.call("asset.register", {"path": str(subtitle), "kind": "subtitle"})["asset_id"]
        ...
        def frame(filename, offset):
            return subprocess.check_output(["ffmpeg", "-v", "error", "-i", filename, "-ss", str(offset),
    ...
    def test_changed_pinned_model_is_rejected_and_does_not_poison_next_job(self):
        ...
        self.assertTrue(session.call("media.render", {"asset_id": aid, "mode": "full", "encoding": "review"})["has_audio"])
    ...
    def test_ocr_missing_serializer_fails_explicitly_without_custom_parser_fallback(self):
        session, aid = self.session()
        args = self.request(aid, processing={"version": 1, "ocr": {"language": "en", "sample_ms": 500, "min_confidence": 0.5}})
