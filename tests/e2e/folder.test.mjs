...
for (const locale of ['en', 'vi']) {
  test(`Electron folder intake, real queued render and desktop UI (${locale})`, {
    timeout: 120000,
  }, async () => {
    ...
    try {
      ...
      const labels =
        locale === 'vi'
          ? {
              nav: 'Tự động hóa',
              folder: 'Nhận file từ thư mục',
              source: 'Chọn folder nguồn',
              output: 'Chọn folder đích',
              scope: 'File mới hoặc thay đổi sau lần kiểm kê đầu',
              save: 'Lưu quy tắc',
          ...
          : {
              nav: 'Automation',
              folder: 'Folder intake',
              source: 'Choose source folder',
              output: 'Choose destination folder',
              scope: 'New or changed files after the first inventory',
              save: 'Save rule',
      ...
      await page.getByRole('tab', { name: labels.folder, exact: true }).click();
      ...
      await page.waitForFunction(async () => {
        const value = await window.reupmatic.folderSnapshot();
        return value.ok && value.data.rules[0]?.state === 'watching';
      });
      const temporary = path.join(source, 'video.mp4.partial');
      await fs.copyFile(prepared, temporary);
      await fs.rename(temporary, path.join(source, 'video.mp4'));
      await page.waitForFunction(
        async () => {
          const value = await window.reupmatic.batchSnapshot();
          return value.ok && value.data.items.length === 1;
        },
        null,
        { timeout: 30000 },
      );
      ...
      await page.screenshot({
        path: path.join(screenshots, `folder-${locale}.png`),
        fullPage: true,
      });
      const snapshot = await page.evaluate(() => window.reupmatic.folderSnapshot());
      assert.equal(snapshot.ok, true);
      assert.equal(snapshot.data.rules[0].state, 'paused');
    } finally {
