...
// Native pickers are controlled fixture input. Renderer, IPC and FFmpeg are real;
// no mocked library storage, import or media is substituted.
...
test('Electron Sources & Library list/detail composition, selection, batch and relink', {
  ...
  await runElectronTest(
    ...
    async ({ application, page }) => {
      ...
      // Ticket 14: the area's top level is exactly SCR-01's two tabs. An asset kind
      // and a label catalogue are not peer navigation destinations.
      ...
      // Empty composition: one action, no reassurance paragraph, no zero-selection
      // label and no disabled pagination.
      ...
      // The transient import result is its own row under the toolbar, not
      // text squeezed beside the primary Import action.
      assert.equal(
        await page
      ...
      // Populated rows: no reserved empty detail column while nothing is selected.
      ...
      // Name header is sortable (UI-CM05); clicking it reorders the table
      // (whole-set ordering itself is proven at the core layer, where a
      // realistic multi-page fixture is cheap — see content-library.test.mjs).
      await panel
      ...
      // Selection bar appears only once something is selected, with an accurate count.
      assert.equal(await panel.getByText(/selected/i).count(), 0);
      ...
      // Prepare batch… opens a sheet; the batch-handoff explanation lives only there.
      assert.equal(
        await page
      ...
      // Selecting one entry opens its actions in the shared DetailSurface.
      ...
      // Focus moves on the next animation frame (DetailSurface's own
      // rAF-deferred focus call), so poll rather than assume it already
      // landed the instant the surface attached.
      await page.waitForFunction(() => document.activeElement?.closest('.detail-surface') !== null);

      // Nothing recorded yet for related files reads as one quiet line, not a
      // heading, an explanatory paragraph and "0–0 of 0 items".
      await surfaceA
      ...
      // Reveal and Forget are lesser actions behind one collapsed "More actions"
      // disclosure; Open in Editor stays pinned.
      ...
      // Ticket 14: related assets are a disclosure *inside* the record's surface —
      // reaching them never leaves the record for a separate assets destination.
      assert.equal(
      ...
      // Escape closes the surface and returns focus to the invoking row.
      ...
      // Drawer at 1050x700 must not clip its last section (redesign-brief:
      // "nothing is clipped at 1050×700 in either locale").
      ...
      // The surface itself may need to scroll (redesign-brief: "everything
      // scrolls inside the surface"), but nothing is unreachable: after
      // scrolling to the end, the last section is fully inside the drawer's
      // own box, not hidden past a hard clip.
      ...
      // Moved original: opening it surfaces the missing-source state, and Locate
      // moved source relinks without losing the library entry.
      ...
      // Scoped to the detail surface: the availability filter's own option list carries the
      // same localized strings, and an unscoped `.first()` can resolve to its hidden option.
      ...
      // The relinked row no longer offers "Locate moved source", so the control that held
      // focus is gone; put focus back inside the surface and Escape must still close it.
      await surfaceB.getByRole('button').first().focus();
      await page.keyboard.press('Escape');
      await surfaceB.waitFor({ state: 'detached' });

      // Vietnamese: composition and copy carry over without losing selection state.
      ...
      // Same drawer, same "nothing clipped" guarantee, in Vietnamese.
      await page.setViewportSize({ width: 1050, height: 700 });
