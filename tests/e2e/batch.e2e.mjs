...
for (const locale of ['en', 'vi']) {
  test(`Electron local batch output and persistent history (${locale})`, { timeout: 120000 }, async () => {
    const temporary = await mkdtemp(path.join(os.tmpdir(), 'reupmatic-batch-e2e-'));
    const dir = await realpath(temporary), output = path.join(dir, 'bản xuất'), userData = path.join(dir, 'app-data');
    await mkdir(output); await mkdir(userData);
    const video = path.join(dir, 'video tự quay.mp4');
    execFileSync(process.env.FFMPEG_PATH || 'ffmpeg', ['-v', 'error', '-f', 'lavfi', '-i', 'testsrc2=size=160x90:rate=12:duration=1', '-c:v', 'libx264', '-threads', '1', '-n', video]);
    const launch = () => electron.launch({ cwd: root,
    ...
    let application;
    try {
      application = await launch(); const page = await application.firstWindow();
      ...
      await application.evaluate(({ dialog }, choices) => {
        ...
        dialog.showOpenDialog = async () => {
          const files = pending.shift(); if (!files) throw new Error('Unexpected picker');
      ...
      const labels = locale === 'vi'
        ? { panel: 'Xử lý lô & tác vụ', add: 'Thêm video', folder: 'Chọn thư mục xuất', enqueue: 'Thêm 1 video vào hàng đợi', start: 'Chạy hàng đợi' }
        : { panel: 'Batch & jobs', add: 'Add videos', folder: 'Choose output folder', enqueue: 'Add 1 video(s) to queue', start: 'Start queue' };
      ...
      const row = page.locator('[data-state="complete"]'); await row.waitFor({ timeout: 60000 });
      const jobId = await row.getAttribute('data-job-id');
      const files = await readdir(output); assert.equal(files.length, 1); assert.match(files[0], /\.mp4$/);
      ...
      assert.equal(snapshot.ok, true); assert.equal(snapshot.data.paused, true);
      assert.equal(snapshot.data.items[0].attempt, 1); assert.deepEqual(await readdir(output), files);
