// Real Electron/Playwright dependencies. Renderer, IPC and catalog storage
// are real; no mocked label save. Ticket 14: shared labels are a Library filter
// facet, not a peer tab — create/rename/archive stay reachable through the
// toolbar's "Manage shared labels…" dialog.
...
test('Shared labels stay manageable from the Library toolbar dialog (UI-CM05)', {
  ...
  await runElectronTest(
    ...
    async ({ page }) => {
      ...
      // The labels catalogue is no longer a peer destination (ticket 14).
      ...
      // Sort descending: page 1 must jump to the whole set's last item, not
      // just reverse the 25 rows currently on screen.
      await manager
