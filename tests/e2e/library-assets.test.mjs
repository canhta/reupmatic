...
// fixture input. Renderer, IPC and library storage are real; no mocked
// asset registration.
...
test('Related-assets table search and sort apply to the whole list (UI-CM05)', {
  ...
  await runElectronTest(
    ...
    async ({ application, page }) => {
      ...
      // Ticket 14: the record's assets disclose in place, inside its surface —
      // no tab switch to an assets destination.
      ...
      // Default attachment kind is audio; switch to Subtitles for these files.
      // Two "Asset type" combobox exist (the toolbar's filter and the attach
      // section's own picker); the attach picker is the second in DOM order.
      ...
      // Sort ascending by File: the whole list (server-paged, sorted by the
      // host, not just the current page) reorders.
      ...
      // Sort descending: still the whole list, not a re-reverse of the page.
      await fileHeaderSort.click();
