...
test('SC-10: one destination post owns exact export and link snapshots', () => {
  ...
  try {
    ...
    assert.throws(
      () => catalog.posts.create({ ...draft, id: 'post_0002' }, exported),
      /LINK_ARCHIVED/,
    );
  } finally {
    catalog.close();
...
test('SC-09/10: per-channel count, retry conflict and cancellation never rewrite usage history', () => {
  ...
  try {
    ...
    assert.deepEqual(catalog.posts.contentDependencies(exported.library_id), {
      posts: 2,
      pending_posts: 1,
    });
    assert.throws(
...
test('catalog boundary rejects unknown fields, credentials, malformed IDs and forged permissions', () => {
  ...
  try {
    assert.throws(
      () => catalog.distribution.saveChannel({ ...channel, can_publish: true }),
      /INVALID_REQUEST/,
    );
    assert.throws(
      () =>
    ...
    assert.throws(
      () => catalog.distribution.saveLink({ ...affiliate, url: 'javascript:alert(1)' }),
      /INVALID_URL/,
    );
    assert.throws(
      () => catalog.posts.list({ ...query, channel_id: '../unsafe' }),
      /INVALID_REQUEST/,
    );
    assert.throws(() => catalog.posts.list({ ...query, limit: 100000 }), /INVALID_REQUEST/);
    assert.throws(
      () => catalog.posts.create({ ...draft, link_ids: [affiliate.id, affiliate.id] }, exported),
      /INVALID_REQUEST/,
    );
    assert.throws(
      () => catalog.posts.create(draft, { ...exported, link_id: 'export_002' }),
      /INVALID_EXPORT/,
    );
  } finally {
    catalog.close();
...
test('shared label archival preserves existing references but prevents new assignment', () => {
  ...
  try {
    ...
    catalog.taxonomy.assignContent({
      id: 'library_001',
      expected_revision: null,
      label_ids: [label.id],
    });
    ...
    assert.throws(
      () => catalog.taxonomy.save({ ...input, expected_revision: 2, kind: 'tag' }),
      /LABEL_KIND_LOCKED/,
    );
  } finally {
    catalog.close();
