...
async function fixture(t) {
  const root = await mkdtemp(path.join(os.tmpdir(), 'library-assets-'));
  ...
  const source = { path: original, sha256: createHash('sha256').update('source').digest('hex'),
    name: path.basename(original), duration_ms: 4000, width: 160, height: 90, has_audio: false };
...
test('derived audio and export remain accessible without the original and detect same-size timestamp changes', async t => {
  ...
  await writeFile(file, 'dcba'); await utimes(file, before.atime, before.mtime);
...
test('asset views filter type, content, literal search, pagination and hidden content', async t => {
  ...
  for (const [index, kind] of ['project', 'subtitle', 'export', 'audio'].entries()) {
    store.link(item.id, kind, path.join(root, `${kind}.bin`), { sha256: String(index).repeat(64), size_bytes: index + 1 });
  }
  const second = store.add(randomUUID(), { ...source, name: 'Separate content' }, 'reference');
  const audio = path.join(root, 'voice.wav'); await writeFile(audio, 'voice');
...
test('project association verifies the source content before it is recorded', async t => {
  ...
  await saveProject(file, createProject({ path: source.path, sha256: 'f'.repeat(64) }, snapshot), []);
  await assert.rejects(assets.register(item.id, 'project', file), /LIBRARY_ASSET_SOURCE/);
  assert.equal(store.assets(query).total, 0);
  await saveProject(file, createProject({ path: source.path, sha256: source.sha256 }, snapshot), []);
...
test('a composition project may belong to a member source, but not unrelated content', async t => {
  ...
  const member = { path: source.path, name: source.name, sha256: source.sha256, duration_ms: source.duration_ms };
  const composition = { version: 1, canvas: { width: 320, height: 180, fps: 30 }, clips: [
    { id: 'member', source: member, start_ms: 0, end_ms: 2000, speed: 1 },
  ] };
  const project = createProject({ path: path.join(root, 'anchor.mp4'), sha256: 'e'.repeat(64) }, {
    cues: [], sample: { start_ms: 0, end_ms: 1000 }, composition,
  });
