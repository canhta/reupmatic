...
test('real saved workflow -> shared SQLite queue -> FFmpeg -> Library export -> one local destination post', {
  ...
  const service = new WorkflowService(catalog.workflows, {
    queue,
    resolveLibrary: (id) => library.resolve(id),
    async checkOutput() {},
    async resolveModels() {
  ...
  try {
    ...
    const label = catalog.taxonomy.save({
      id: 'label_native_01',
    ...
    const workflow = catalog.workflows.save(
      {
    ...
    await new LibraryAssets(libraryStore).register(
      item.id,
      'export',
      job.output.path,
      job.output.sha256,
    ...
    const channel = catalog.distribution.saveChannel({
    ...
    const link = catalog.distribution.saveLink({
      id: 'link_native_01',
      expected_revision: null,
      name: 'Link thủ công',
    ...
    const post = catalog.posts.create(
      {
        id: 'post_native_01',
        expected_revision: null,
        title: 'Nội dung thử',
      ...
      },
      {
        ...
        name: exported.name,
        path: exported.path,
        sha256: await hashFile(exported.path),
      },
    );
    ...
    try {
      ...
      assert.equal(
        reopened.posts.list({ search: '', view: 'published', offset: 0, limit: 25 }).total,
        0,
      );
    } finally {
      reopened.close();
