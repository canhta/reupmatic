...
test('restart retains queued/done rows and recovers running versus acknowledged cancellation', async (t) => {
  ...
  const first = new BatchQueue(new ControlledWorker(input.video.sha256), new ControlledRenderer(), {
    databasePath: path.join(directory, 'queue.sqlite'),
    protectSource: (filename) => protectedPaths.add(filename),
  });
  cleanup.push(() => first.close());
  assert.equal(first.recovered, 2);
  assert.deepEqual(
    first.snapshot().items.map((row) => row.state),
    ['complete', 'interrupted', 'cancelled', 'queued'],
  );
  assert.deepEqual([...protectedPaths], [input.video.path]);
  await first.close();
  const second = new BatchQueue(
    new ControlledWorker(input.video.sha256),
    new ControlledRenderer(),
    { databasePath: path.join(directory, 'queue.sqlite') },
  );
...
test('a batch recipe carrying a logo placement is rejected, not silently ignored', async (t) => {
  const { store, input } = await fixture(t);
  assert.throws(
    () =>
      store.enqueue('request-logo', [
        {
          ...input,
          processing: {
...
async function controlled(t, count = 2) {
  ...
  const queue = new BatchQueue(worker, renderer, {
    databasePath: path.join(f.directory, 'queue.sqlite'),
  });
  queue.enqueue(
    'request-controlled',
...
test('queue starts paused, isolates a bad item, then exports good item through same renderer', async (t) => {
  ...
  const rows = queue.snapshot().items.map((item) => queue.get(item.id));
