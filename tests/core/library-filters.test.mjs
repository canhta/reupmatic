...
test('media type and origin filters select exactly the requested kinds', async (t) => {
  ...
  // An empty selection means "no facet applied", not "nothing matches".
...
test('a facet a row lacks excludes nothing: images survive a duration filter', async (t) => {
  ...
  // The image has no duration at all; filtering on duration must not drop it.
...
test('resolution is the orientation-independent short edge, and ignores non-video rows', async (t) => {
  ...
  // 320x180 -> short edge 180; 1080x1920 -> short edge 1080. The image row has no resolution
  // and is retained rather than treated as zero.
  assert.deepEqual(
...
test('published_at is scoped to origins that have it, and never drops local rows', async (t) => {
  ...
  // The local and image rows carry no publish date; they stay in the result rather than
  // disappearing, because filtering on a facet a row lacks excludes nothing.
...
test('sorting by a key a row lacks keeps the row, and filters never reorder ties', async (t) => {
  ...
  // Rows with a real date sort by it; the rows with no date keep one deterministic, defensible
  // position at the end rather than being dropped or treated as epoch zero.
  assert.deepEqual(
    byPublished.items.map((item) => item.id),
    ['content-douyin', 'content-old', 'content-image', 'content-local'],
  );

  // Ties keep the storage's own stable order (rowid) instead of being randomly reordered.
  ...
  // Ties keep one deterministic order (storage rowid, newest first) rather than shuffling.
