...
import { UserPreferences } from '../../dist-core/settings/user-preferences.js';
...
test('new settings do not guess a save folder or manufacture account status', async (t) => {
  ...
  const settings = await UserPreferences.open(filename);
  assert.deepEqual(settings.snapshot(), { revision: 0, default_output_dir: null });
});
...
test('output default persists; changing it leaves previous output files in place', async (t) => {
  ...
  assert.equal((await UserPreferences.open(filename)).snapshot().default_output_dir, root);
...
test('corrupt settings are preserved rather than silently replaced', async (t) => {
  const { filename } = await fixture(t);
  for (const value of ['bad json', JSON.stringify({ revision: 'x' })]) {
    await writeFile(filename, value);
    await assert.rejects(UserPreferences.open(filename), /SETTINGS_INVALID|SETTINGS_VERSION/);
