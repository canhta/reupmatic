...
/** One video's detail, whose raw payload carries a per-tier CDN mirror. */
...
/**
 * A local HTTP server that only answers a request once it holds a permit, so a test can hold an
 * item `running` for as long as it needs to observe the queue mid-flight. A permit-counting
 * semaphore, not a single reusable gate: the caller's `open()` and the queue's own request race
 * each other (`enqueue()` returns well before the item's `await`-chain reaches the actual `fetch`),
 * so `open()` must still work correctly when called before anyone is waiting on it yet.
 */
async function gatedServer(t) {
  ...
  t.after(() => {
    // `fetch()`'s keep-alive connection otherwise leaves `server.close()`'s callback waiting on
    // an open socket that neither side is about to close.
...
/** Polls the queue until its item list matches `predicate`, or times out. */
...
test('a second enqueue while one item is running appends rather than refusing', async (t) => {
  ...
  // The first item is now `running` (its transfer is held by the gate); a second selection
  // arriving now must be appended, not refused.
...
test('retrying a failed item replaces its row: one entry per aweme_id, counts not doubled', async () => {
  ...
  // The row a table finds by aweme_id must be the retry, not the stale failure.
