...
import { ProjectRecovery } from '../../dist-core/projects/recovery/project-recovery.js';
...
test('recovery persists complete current documents across reopen and rejects stale writes', async () => {
  ...
  try {
    ...
    store = new ProjectRecovery(filename);
...
test('discarding a draft never deletes source media and separate projects keep separate identities', async () => {
  ...
  try {
    ...
    await writeFile(source, 'never delete');
    const input = project();
    input.source.path = source;
    store.save('document-001', 0, input);
    store.save('document-002', 0, input);
...
test('unsupported recovery stores fail without reset or automatic migration', async () => {
  ...
  try {
    ...
    database.exec('CREATE TABLE keep_me(data TEXT); PRAGMA user_version=42;');
    database.close();
    const before = await readFile(filename);
    assert.throws(() => new ProjectRecovery(filename), /RECOVERY_VERSION/);
