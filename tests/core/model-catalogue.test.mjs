...
test('the shipped catalogue offers both text-to-speech engines before any bytes move', async () => {
  ...
  // The verified totals: a turbo graph repo plus the codec in a second Hub repo.
...
test('an entry says which task its model serves, and the task gates the architecture', () => {
  ...
  // Both synthesis architectures are installable through the same catalogue path; the
  // engine name is code the catalogue points at, never a second configuration mechanism.
  ...
  // A real recognition architecture filed under the synthesis task is not executable for that
  // task, so it is refused at configuration time rather than offered and failed later.
  assert.throws(
    () => parseCatalogueModel({ ...synthesis, engine: 'faster-whisper' }),
    /MODEL_ARCHITECTURE_UNIMPLEMENTED/,
  );
  // An architecture this build never implemented is refused the same way.
  assert.throws(
    () => parseCatalogueModel({ ...synthesis, engine: 'some-unshipped-tts' }),
    /MODEL_ARCHITECTURE_UNIMPLEMENTED/,
  );
  // A task this build does not know is malformed, not a forward-compatible entry.
  assert.throws(
...
test('malformed entries are refused with a plain reason and never offered', () => {
  ...
  // The declared download size must be the sum of the files it will actually move.
...
test('the shipped catalogue offers a translation pair and the LaMa inpainting model', async () => {
  ...
  for (const pack of Object.values(ocr.ocr)) {
    ...
    // Every role names a file the entry actually downloads.
    const names = new Set(ocr.files.map((file) => file.name));
    assert.ok(names.has(pack.det) && names.has(pack.rec) && names.has(pack.keys));
  }
  // Vietnamese uses the Latin recognizer, never the Chinese one (local-models.md).
...
test('every offered model describes itself in each UI locale the app ships', async () => {
  ...
  for (const model of catalogue.models) {
    ...
    // Vietnamese that is merely the English string copied across is not a translation.
    assert.notEqual(
      model.purpose.vi,
      model.purpose.en,
...
test('a model that cannot describe itself in every UI locale is refused, never half-translated', () => {
  ...
  // A bare string is the replaced shape; greenfield keeps one current contract, so it fails
  // loudly rather than being read as an English-only entry.
  assert.throws(
    () => parseCatalogueModel(entry({ purpose: 'A purpose' })),
    /MODEL_CATALOGUE_INVALID/,
  );
  // A locale the app does not ship is not a way to smuggle extra keys past the bound.
  assert.throws(
...
test('a translation entry declares one direction and the adapter\u2019s required files', () => {
  ...
  // The source/target vocabulary layout is the other accepted shape.
...
test('a translation entry that cannot install is refused at configuration time', () => {
  ...
  // A bundle cannot translate a language into itself.
  assert.throws(
    () => parseCatalogueModel(translationEntry({ target_language: 'en' })),
    /MODEL_CATALOGUE_INVALID/,
  );
  // Both vocabulary layouts at once is ambiguous, not two ways to read one bundle.
  const both = [
  ...
  // A file the adapter never reads is refused rather than downloaded.
  const extra = [...translationEntry().files, file('notes.txt', 1)];
  assert.throws(
    () => parseCatalogueModel(translationEntry(sized(extra))),
    /MODEL_CATALOGUE_INVALID/,
  );
  // A real architecture filed under the wrong task is not executable there.
  assert.throws(
...
test('a vision entry that cannot install is refused at configuration time', () => {
  ...
  // A role naming a file the entry does not download.
  assert.throws(
    () =>
      parseCatalogueModel(
        visionEntry({
  ...
  // A version or height the adapter does not accept.
  assert.throws(
    () =>
      parseCatalogueModel(
  ...
  // An unreferenced downloaded file is dead weight.
