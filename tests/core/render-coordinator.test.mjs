...
const soundtrack = {
  ...
  fade_in_ms: 100,
  fade_out_ms: 200,
  duck: { enabled: true, amount_db: 9, release_ms: 400 },
  muted: false,
};
...
test('soundtrack is snapshotted, registered and dispatched by exact content through the shared renderer', async () => {
  ...
  assert.deepEqual(worker.calls[1].params.soundtrack.duck, {
    enabled: true,
    amount_db: 9,
    release_ms: 400,
  });
...
test('a verified voice track is registered and dispatched as the third audio branch', async () => {
  const worker = new ControlledWorker();
  const coordinated = new RenderCoordinator(worker, async (track) => {
    assert.equal(track.artifact.artifact_id, voiceTrack.artifact.artifact_id);
    return '/speech/verified.wav';
  });
  const original = {
    ...input(),
    voice: structuredClone(voiceTrack),
  };
  const ticket = coordinated.start(original);
  await tick();
  assert.equal(worker.calls[0].method, 'asset.register');
  assert.deepEqual(worker.calls[0].params, { path: '/speech/verified.wav', kind: 'audio' });
  original.voice.mode = 'replace';
  worker.calls[0].resolve({ asset_id: 'voice-1', sha256: voiceTrack.artifact.sha256 });
  await tick();
  assert.equal(worker.calls[1].method, 'media.render');
  assert.equal(worker.calls[1].params.voice.asset_id, 'voice-1');
  assert.equal(worker.calls[1].params.voice.mode, 'mix');
  assert.equal(worker.calls[1].params.voice.sample_rate, 48000);
  assert.deepEqual(worker.calls[1].params.voice.lines, [
    { offset_ms: 0, rate: 1, start_frame: 0, end_frame: 24000 },
    { offset_ms: 1500, rate: 1.25, start_frame: 30000, end_frame: 54000 },
  ]);
  assert.equal(worker.calls[1].params.voice.artifact_id, undefined);
  worker.calls[1].resolve(output);
  await ticket.result;
  await coordinated.close();
});

test('a voice track with no admitted artifact or a changed hash never dispatches encoding', async () => {
  const worker = new ControlledWorker();
  const coordinator = new RenderCoordinator(worker);
  const value = {
    ...input(),
  ...
  // No admission hook: the render refuses rather than trust a caller-supplied path.
  const refused = coordinator.start(value);
  await assert.rejects(refused.result, { code: 'VOICE_TRACK_STALE' });
  assert.equal(worker.calls.length, 0);
  await coordinator.close();

  const changedWorker = new ControlledWorker();
  const changed = new RenderCoordinator(changedWorker, async () => '/speech/verified.wav');
  const ticket = changed.start(value);
  await tick();
  changedWorker.calls[0].resolve({ asset_id: 'voice-1', sha256: 'c'.repeat(64) });
  await assert.rejects(ticket.result, { code: 'SOURCE_CHANGED' });
  assert.equal(changedWorker.calls.length, 1);
  await changed.close();
});

test('a lossless voice render is refused before any work starts', async () => {
  const worker = new ControlledWorker();
  const coordinator = new RenderCoordinator(worker, async () => '/speech/verified.wav');
  assert.throws(
    () =>
      coordinator.start({
        ...input(),
