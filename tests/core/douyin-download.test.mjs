...
/** One video's detail, whose raw payload carries a per-tier CDN mirror (the expiring URL). */
...
/** A local HTTP server holding one small body, with a live URL per test. */
...
/** A download seam that really fetches bytes over HTTP, standing in for the Python worker. */
...
test('a whole selection downloads, reports transitions, and records the direct path and tier', async (t) => {
  ...
  // The first emitted snapshot has every candidate queued before any network call.
  assert.deepEqual(
...
test('a tier whose declared dataSize disagrees with the served bytes still completes', async (t) => {
  // Douyin's own `data_size` is a claim, not a completion fact (live evidence, coordinator,
  //: a real, playable tier reported a `data_size` the CDN's own bytes disagreed with).
  // The worker's Content-Length check and its FFmpeg probe are the real verification; a
  // deliberately wrong dataSize here must never turn a good transfer into a manufactured
  // DOWNLOAD_SIZE_MISMATCH, and `expected_size` must never even reach the request.
  const server = await mediaServer(t);
  const requests = [];
  const result = await runDouyinDownloads(
...
test('one failing item does not stop the rest and reports exactly one failure', async (t) => {
  ...
  // The stage is what the Diagnostic log needs to tell a refused page from a refused CDN.
...
test('every mirror refusing re-resolves once through reobserve and retries the fresh mirrors', async (t) => {
  ...
  const result = await runDouyinDownloads(
    ...
    baseDeps({
      ...
      commit: async (commit) => {
        // The fresh detail (post-reobserve) is what gets committed, not the stale one.
...
test('a fresh reobserved mirror set still refusing is an honest terminal transfer failure', async (t) => {
  const server = await mediaServer(t);
  // The mirror URL itself is fine (`server.url`); `download` is what unconditionally refuses, both
  // before and after the reobserve. D-62 removed the browser page entirely, so there is nothing
  // left to fall back to — a refusal that survives the one re-resolve must stand as a real
  // `transfer` failure, never a silently claimed success.
...
test('findContentByAwemeId is identity; filename equality is never consulted', async (t) => {
  ...
  // A different filename with the same aweme_id still resolves; a different aweme_id does not.
  ...
  // A forgotten item is no longer held.
