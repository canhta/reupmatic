...
test('inserting a clip at an index places it before the clip already there and shifts later cues', () => {
  ...
  // The cue sat over clip-a; inserting a 2000 ms clip before it moves that cue by 2000 ms.
  assert.deepEqual(
    inserted.cues.map((c) => [c.start_ms, c.end_ms]),
    [[2500, 3500]],
  );
  assert.equal(compositionDuration(inserted.composition), 5000);
  // Appending at the end through insert is the same list as append.
  assert.deepEqual(
...
test('placing past MAX_CLIPS is refused by its own limit code, never truncated', () => {
  ...
  // The placement is refused by the same limit `parseComposition` owns, under a
  // distinct code the UI renders as its own message (ticket 05 review).
  assert.throws(
    () =>
...
test('the remap judges conflicts by the shipped timing policy, never a second literal', async () => {
  ...
  // The same edit remaps the line into a 500 ms slot: 400 ms of speech fits and 800 ms overruns.
  // The expected conflict set is derived from the exported policy, so a remap that hardcoded its
  // own threshold would disagree the moment ticket 07 raises the bound from measurement.
...
test('disabling a clip keeps its placement and trims and is one undo step', () => {
  ...
  // The clip keeps its exact source range and stays on the clock: a disabled clip leaves its
  // place, it does not ripple the clips after it.
  ...
  // Preview and seek never land on a disabled clip.
  ...
  // Re-enabling restores the original document byte for byte.
...
test('cues only render over enabled clips and keep every stored cue', () => {
  ...
  // The stored cues are untouched, so re-enabling brings them all back.
  assert.deepEqual(
    wide.map((c) => c.id),
