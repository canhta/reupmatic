...
const workflow = { id: 'workflow_001', expected_revision: null, name: 'Quy trình',
  item_ids: ['library_001'], processing: null, archived: false };
const video = { path: path.resolve('source.mp4'), name: 'Tiếng Việt.mp4', sha256: 'a'.repeat(64),
  duration_ms: 5000, width: 640, height: 360, has_audio: true };
...
function fixture() {
  ...
  const host = {
    queue: {
      enqueue(id, inputs) { batch.enqueue(id, inputs); return this.snapshot(); },
      snapshot() { return { items: batch.list().map(job => ({ ...job, batch_id: job.batch_id })) }; },
    },
    async resolveLibrary() { resolves++; return video; },
    async resolveModels() { return { ocr_en: 'b'.repeat(64) }; },
    async checkOutput() {},
  };
  const saved = catalog.workflows.save(workflow, directory);
  const service = new WorkflowService(catalog.workflows, host);
  return { catalog, batch, service, saved, host, get resolves() { return resolves; },
    async close() { await service.close(); catalog.close(); batch.close(); } };
}
...
test('SC-08: workflow save has no execution side effects and duplicate run admission is stable', async () => {
  ...
  try {
    ...
    const [one, two] = await Promise.all([
      f.service.run(f.saved.id, 1, 'request_001'), f.service.run(f.saved.id, 1, 'request_001'),
    ]);
...
test('SC-07/08: prepared admission reuses captured recipe and model versions after edits', async () => {
  const f = fixture();
  try {
    const processing = { version: 1, ocr: { language: 'en', sample_ms: 500, min_confidence: 0.7 } };
    const saved = f.catalog.workflows.save({ ...workflow, expected_revision: 1, processing }, directory);
    const input = { video: { path: video.path, name: video.name, sha256: video.sha256 },
      library_id: 'library_001', output_dir: directory, encoding: 'review', processing,
      processing_models: { ocr_en: 'c'.repeat(64) } };
    f.catalog.workflows.prepare('request_002', saved, [input]);
    f.catalog.workflows.save({ ...workflow, expected_revision: 2, name: 'New settings', archived: true }, directory);
    ...
    const state = workflowRunState(f.catalog.workflows.runs()[0], f.batch.list());
...
test('SC-08: reusing a prepared request for another workflow is rejected', async () => {
  ...
  try {
    ...
    const second = f.catalog.workflows.save({ ...workflow, id: 'workflow_002' }, directory);
...
test('SC-08: stale/archived or missing-input workflows cannot partially enqueue', async () => {
  ...
  try {
    ...
    f.host.resolveLibrary = async () => { throw new Error('SOURCE_UNAVAILABLE'); };
    await assert.rejects(f.service.run(f.saved.id, 1, 'request_005'), /SOURCE_UNAVAILABLE/);
    assert.equal(f.batch.list().length, 0);
    assert.equal(f.catalog.workflows.runs().length, 0);
    f.catalog.workflows.save({ ...workflow, expected_revision: 1, archived: true }, directory);
...
test('SC-08: change during source resolution is detected before a run is saved', async () => {
  ...
  try {
    f.host.resolveLibrary = async () => {
      f.catalog.workflows.save({ ...workflow, expected_revision: 1, name: 'Changed mid-flight' }, directory);
      return video;
    };
...
test('SC-07: missing jobs and mixed outcomes cannot be called a completed workflow', () => {
  ...
  assert.equal(workflowRunState(run, [{ id: 'job_0001', state: 'complete' }, { id: 'job_0002', state: 'failed' }]), 'partial');
  assert.equal(workflowRunState(run, [{ id: 'job_0001', state: 'complete' }, { id: 'job_0002', state: 'complete' }]), 'complete');
});
...
test('SC-07/08: a crash after queue commit but before run update cannot create duplicate jobs', async () => {
  ...
  try {
    const input = { video: { path: video.path, name: video.name, sha256: video.sha256 },
      library_id: 'library_001', output_dir: directory, encoding: 'review' };
    const prepared = f.catalog.workflows.prepare('request_crash_01', f.saved, [input]);
