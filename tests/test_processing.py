...
from vision.service import MAX_OCR_EDGE, MAX_SEGMENT_BYTES, DISK_RESERVE


class RecipeTests(unittest.TestCase):
    def setUp(self):
        self.recipe = {'version': 1, 'ocr': {'language': 'vi', 'sample_ms': 500, 'min_confidence': 0.5},
    ...
    def test_strict_recipe_and_independent_snapshot(self):
        ...
        for bad in [None, [], {}, {'version': True, 'ocr': self.recipe['ocr']}, {'version': 1},
                    {**self.recipe, 'shell': 'command'}, {**self.recipe, 'ocr': {**self.recipe['ocr'], 'sample_ms': True}},
...
class ChunkCompositionTests(unittest.TestCase):
    def test_windows_preserve_sampling_grid_and_fit_worst_case_decoded_frames(self):
        for sample in [100, 333, 500, 700, 2000]:
            ...
            self.assertLessEqual(window // sample * MAX_OCR_EDGE**2 * 3 + DISK_RESERVE, MAX_SEGMENT_BYTES)
    ...
    def test_ocr_merges_only_adjacent_equal_text_across_chunks(self):
        ...
        class VisionDouble:
            ...
            def run(self, request, *, staging, emit_progress):
                p = request['params']; first, last = p['start_ms'], p['end_ms']
                observations.append((first, last))
                analysis_id = str(len(observations))
                (staging / f'{analysis_id}.json').write_text('{}')
                cues = [{'id': 'source-cue', 'start_ms': first, 'end_ms': last, 'text': 'Same text'}]
                if len(observations) == 3:
                    cues[0]['start_ms'] += 500  # A blank observation breaks a cue.
                return {'width': 160, 'height': 90, 'observations': [], 'analysis_id': analysis_id, 'cues': cues, 'model_fingerprints': {'ocr': pins['ocr_en']}}
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('processing.ocr_scan.VisionService', VisionDouble), patch('subtitles.service.cue_document', document):
                track, count = scan_subtitles(host, req, options, 250, 250 + 3 * window, staging, pins)
...
class ProcessingSchemaTests(unittest.TestCase):
    def setUp(self):
        ...
        self.schemas = {file.name: json.loads(file.read_text()) for file in (ROOT / 'contracts').glob('*.schema.json')}

    def test_embedded_recipes_stay_identical_to_the_standalone_contract(self):
        base = self.schemas['processing.schema.json']
        shape = {key: value for key, value in base.items() if key not in ('$schema', '$id', '$comment', 'title', 'description')}
        def find_recipe(value):
            if isinstance(value, dict):
                if value.get('properties', {}).get('version') == {'const': 1} and {'ocr', 'inpaint'} <= value.get('properties', {}).keys():
                    yield value
    ...
    def test_processing_request_is_data_only_and_plain_render_stays_strict(self):
        ...
        request = {'v': 1, 'id': 'test', 'revision': 0, 'method': 'media.process',
                   'params': {'asset_id': 'source', 'mode': 'full', 'encoding': 'review', 'processing': recipe}}
        ...
        self.assertFalse(validator.is_valid(request))
        request['method'] = 'models.resolve'; request['params'] = {'processing': recipe}
        self.assertTrue(validator.is_valid(request))
