...
class VisionContracts(unittest.TestCase):
    ...
    def valid(self, method, params):
        return self.validator.is_valid({'v': 1, 'id': 'contract-test', 'revision': 2,
    ...
    def test_model_configuration_and_status_are_explicit_verbs(self):
        ...
        self.assertFalse(self.valid('models.configure', {'path': '/native/selected.json', 'download': True}))
        self.assertFalse(self.valid('models.status', {'path': '/unrequested/path'}))

    def test_ocr_bounds_and_unknown_fields_match_runtime(self):
        params = {'asset_id': 'known-asset', 'start_ms': 0, 'end_ms': 2000,
                  'language': 'vi', 'sample_ms': 500, 'min_confidence': 0.5}
    ...
    def test_manual_and_text_inpainting_have_distinct_requirements(self):
        ...
        manual = common | {'target': 'manual', 'region': {'x': 0, 'y': 0.7, 'width': 1, 'height': 0.2}}
    ...
    def test_relational_time_bounds_remain_runtime_validation(self):
        params = {'asset_id': 'known-asset', 'start_ms': 5000, 'end_ms': 1000,
                  'language': 'en', 'sample_ms': 500, 'min_confidence': 0.5}
        with self.assertRaises(WorkerError):
