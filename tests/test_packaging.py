...
class SourcePackagingTests(unittest.TestCase):
    ...
    def package(self):
        return subprocess.run(
            [sys.executable, str(PACKAGER), '--root', str(self.root),
    ...
    def test_archive_version_root_and_embedded_hashes_match_the_sources(self):
        ...
        with zipfile.ZipFile(archive) as bundle:
            ...
            for name, digest in manifest['files'].items():
                self.assertEqual(hashlib.sha256(bundle.read('reupmatic/' + name)).hexdigest(), digest)
    ...
    def test_dependencies_secrets_fonts_models_and_archives_are_excluded(self):
        excluded = [
            'node_modules/library/index.js', 'dist-core/example.js',
            '.env', 'app/.env.local', 'worker/__pycache__/worker.pyc',
            'worker/model.onnx', 'app/ui/font.woff2', 'research/backup.zip',
            '.git/config', 'workspace/private.mp4', 'worker/local.pem',
        ]
    ...
    def test_reserved_modules_and_future_cloud_delivery_boundaries_survive_packaging(self):
        markers = ['app/core/downloads/.gitkeep', 'worker/speech/.gitkeep',
                   'services/cloud/consent/.gitkeep', 'packaging/windows/.gitkeep',
                   'contracts/entitlements/.gitkeep', 'tests/acceptance/publishing/.gitkeep']
        for name in markers:
            self.write(name, '')
        result = self.package()
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def declare_scope(self):
        self.write('docs/architecture/system-map.json', json.dumps({
            'source_version': '0.6.0', 'modules': [{'reserved': ['app/core/downloads']}],
        }))
        self.write('docs/architecture/retained-paths.json', json.dumps({
            'paths': ['app/core/example.ts'], 'moves': {},
        }))
        self.write('docs/planning/scope-traceability.json', json.dumps({
            'source_version': '0.6.0', 'features': [{'scope': f'SC-{index:02d}'} for index in range(1, 15)],
        }))
    ...
    def test_current_system_map_versions_and_scope_must_match_release(self):
        self.declare_scope()
        self.write('docs/planning/scope-traceability.json', json.dumps({
            'source_version': '0.6.0', 'features': [{'scope': 'SC-01'}],
        }))
