...
class SoundtrackNativeTests(unittest.TestCase):
    def setUp(self):
        ...
        subprocess.run(['ffmpeg', '-v', 'error', '-f', 'lavfi', '-i', 'color=s=160x90:r=24:d=4',
                        '-c:v', 'libx264', '-threads', '2', str(self.video)], check=True)
        subprocess.run(['ffmpeg', '-v', 'error', '-f', 'lavfi', '-i', 'sine=frequency=880:duration=3',
                        '-c:a', 'pcm_s16le', str(self.audio)], check=True)
        self.session = Session(self.root / 'workspace')
        self.source = self.session.call('asset.register', {'path': str(self.video), 'kind': 'video'})['asset_id']
        registered = self.session.call('asset.register', {'path': str(self.audio), 'kind': 'audio'})
        self.track = {'asset_id': registered['asset_id'], 'sha256': registered['sha256'], 'mode': 'replace',
                      'start_ms': 0, 'end_ms': 2000, 'offset_ms': 1000, 'gain_db': 0,
    ...
    def render(self, **params):
        return self.session.call('media.process', {'asset_id': self.source, 'mode': 'full', 'encoding': 'review',
            'processing': {'version': 1, 'editing': {'speed': 1}}, 'soundtrack': self.track, **params})

    def level(self, filename, start, duration=.25):
        data = array.array('f', subprocess.check_output(['ffmpeg', '-v', 'error', '-ss', str(start), '-i', filename,
            '-t', str(duration), '-map', '0:a:0', '-ac', '1', '-f', 'f32le', '-']))
    ...
    def test_replace_pads_silence_preserves_duration_and_cache_identity(self):
        ...
        self.assertLess(self.level(result['path'], .2), .001)
        self.assertGreater(self.level(result['path'], 1.4), .04)
        self.assertLess(self.level(result['path'], 3.5), .001)
        ...
        self.assertAlmostEqual(self.level(quieter['path'], 1.4) / self.level(result['path'], 1.4), 10 ** (-12/20), delta=.04)
    ...
    def test_sample_uses_output_clock_after_trim_and_speed(self):
        processing = {'version': 1, 'editing': {'trim': {'start_ms': 1000, 'end_ms': 4000}, 'speed': 2}}
        ...
        self.assertLess(self.level(full['path'], .2), .001)
        self.assertGreater(self.level(sample['path'], .2, .2), .04)
    ...
    def test_mix_contains_both_sources_while_muting_only_suppresses_original(self):
        ...
        subprocess.run(['ffmpeg', '-v', 'error', '-i', str(self.video), '-f', 'lavfi',
            '-i', 'sine=frequency=440:duration=4', '-c:v', 'copy', '-c:a', 'aac', str(audible)], check=True)
        self.source = self.session.call('asset.register', {'path': str(audible), 'kind': 'video'})['asset_id']
        track = {**self.track, 'mode': 'mix', 'offset_ms': 0, 'end_ms': 3000}
        mixed = self.render(soundtrack=track)
        muted = self.render(soundtrack=track, processing={'version': 1, 'editing': {'audio': {'muted': True, 'gain_db': 0}}})
        self.assertGreater(self.level(mixed['path'], 1), self.level(muted['path'], 1) * 1.3)
        self.assertGreater(self.level(mixed['path'], 3.5), .04)
        self.assertLess(self.level(muted['path'], 3.5), .001)
    ...
    def test_plain_renderer_supports_soundtrack_and_fades(self):
        ...
        result = self.session.call('media.render', {'asset_id': self.source, 'mode': 'full',
                    'encoding': 'review', 'soundtrack': track})
        self.assertLess(self.level(result['path'], .01, .1), self.level(result['path'], .8, .1) * .2)
        self.assertLess(self.level(result['path'], 1.85, .1), self.level(result['path'], .8, .1) * .3)
        self.assertEqual(result['duration_ms'], 4000)
