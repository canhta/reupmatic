...
FILES = ['onnx/' + name for name in ('vieneu_prefill.onnx', 'vieneu_decode_step.onnx',
    'vieneu_acoustic_cached.onnx', 'vieneu_backbone_shared.data', 'vieneu_v3_heads.npz', 'config.json', 'tokenizer.json')]
FILES += ['codec/moss_audio_tokenizer_decode_full.onnx', 'codec/moss_audio_tokenizer_decode_shared.data']
...
def bundle(root: Path) -> tuple[Path, Path]:
    ...
    (directory / 'voices.json').write_text(json.dumps({'version': 1, 'voices': [
        {'id': 'test-voice', 'label': 'Controlled voice', 'speaker_emb': [0.5] * 192, 'ref_codes': [[1] * 8] * 2}]}))
    manifest = root / 'manifest.json'
    manifest.write_text(json.dumps({'version': 1, 'engine': 'vieneu-v3-turbo-onnx',
        'directory': str(directory), 'languages': ['en', 'vi'],
        'files': {name: hashlib.sha256((directory / name).read_bytes()).hexdigest() for name in FILES + ['voices.json']}}))
...
def sdk(root: Path) -> Path:
    ...
    directory.mkdir()
    for name, version in [('vieneu', '3.7.1'), ('onnxruntime', '0.0.0'), ('sea_g2p', '0.0.0'), ('tokenizers', '0.0.0')]:
        pkg = directory / name
        pkg.mkdir()
    ...
    (utils / 'phonemize_text.py').write_text('def phonemize_text_with_emotions(text): return text\n')
    return directory


def params(model_id: str) -> dict:
    return {'source_layer': 'spoken', 'source_token': 'spoken-12345678', 'model_id': model_id,
            'voice_id': 'test-voice', 'language': 'vi', 'cues': [
