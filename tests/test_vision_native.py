...
class ControlledVisionPipelineTests(VisionFixture, unittest.TestCase):
    def test_controlled_ocr_returns_timed_cues_and_separate_persisted_evidence(self):
        s,aid=self.session()
        data=s.call('media.ocr',{'asset_id':aid,'start_ms':0,'end_ms':1000,
                               'language':'en','sample_ms':500,'min_confidence':.5},revision=9)
        self.assertEqual([(c['start_ms'],c['end_ms'],c['text']) for c in data['cues']],[(0,1000,'Fixture OCR')])
        self.assertEqual(len(data['observations']),2)
        self.assertEqual(data['source_sha256'],self.source_hash)
        saved=json.loads((self.workspace/'analyses'/f"{data['analysis_id']}.json").read_text())
    ...
    def test_controlled_lama_exports_actual_mp4_with_audio_without_touching_original(self):
        ...
        raw=subprocess.check_output(['ffmpeg','-v','error','-i',data['path'],'-frames:v','1','-f','rawvideo','-pix_fmt','rgb24','-threads','1','-'])
    ...
    def test_cancel_reaps_inference_child_and_publishes_no_output(self):
        ...
        while True:
            ...
            if message['event'] == 'error': self.fail(str(message))
            if message['event']=='progress' and (message['data'].get('fraction') or 0)>0: break
...
try:
    ...
    print(e.code)
"""
        result=subprocess.run([sys.executable,'-c',script],cwd=root/'worker',capture_output=True,text=True,check=True)
