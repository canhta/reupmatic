"""Composition's current persisted shape and path-free worker transport."""
import copy
...
class CompositionContracts(unittest.TestCase):
    def setUp(self):
        ...
        self.source = {'path': '/video.mp4', 'name': 'video.mp4', 'sha256': 'a' * 64, 'duration_ms': 2000}
        self.clip = {'id': 'clip-a', 'source': self.source, 'start_ms': 0, 'end_ms': 2000, 'speed': 1}
        self.composition = {'version': 1, 'canvas': {'width': 320, 'height': 180, 'fps': 30}, 'clips': [self.clip]}
    ...
    def test_persisted_project_and_transport_are_distinct_and_strict(self):
        ...
        project = {'format': 'reupmatic.project', 'version': 5, 'source': {'path': '/video.mp4', 'sha256': 'a' * 64},
                   'cues': [], 'sample': {'start_ms': 0, 'end_ms': 1000}, 'composition': self.composition}
        self.validator(json.loads((ROOT / 'contracts/project.schema.json').read_text())).validate(project)
        transport = copy.deepcopy(self.composition)
        transport['clips'][0]['source'] = {'asset_id': 'registered-a', 'sha256': 'a' * 64, 'duration_ms': 2000}
        document, spans, duration = parse_composition(transport)
        self.assertEqual(duration, 2000)
        self.assertEqual(len(spans), 1)
        request = {'v': 1, 'id': 'render-a', 'revision': 0, 'method': 'media.render', 'params': {
            'asset_id': 'registered-a', 'mode': 'full', 'encoding': 'review', 'composition': document}}
        validator = self.validator(json.loads((ROOT / 'contracts/worker-request.schema.json').read_text()))
    ...
    def test_semantics_reject_duplicate_ids_short_ranges_and_invalid_numbers(self):
        transport = copy.deepcopy(self.composition)
        transport['clips'][0]['source'] = {'asset_id': 'registered-a', 'sha256': 'a' * 64, 'duration_ms': 2000}
        for modify in [lambda value: value['clips'].append(copy.deepcopy(value['clips'][0])),
