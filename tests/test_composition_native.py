...
class CompositionNativeTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i", "color=red:s=320x180:r=30:d=3",
                        "-f", "lavfi", "-i", "sine=frequency=440:sample_rate=48000:duration=3",
                        "-c:v", "libx264", "-threads", "2", "-c:a", "aac", "-shortest", "-n", str(cls.first)], check=True)
        subprocess.run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i", "color=blue:s=180x320:r=24:d=3",
                        "-c:v", "libx264", "-threads", "2", "-an", "-n", str(cls.second)], check=True)
        cls.originals = {path: hashlib.sha256(path.read_bytes()).hexdigest() for path in (cls.first, cls.second)}
    ...
    def setUp(self):
        ...
        for path in (self.first, self.second):
            ...
            self.sources.append({"asset_id": asset["asset_id"], "sha256": asset["sha256"], "duration_ms": info["duration_ms"]})
        self.document = {"version": 1, "canvas": {"width": 320, "height": 180, "fps": 30}, "clips": [
            {"id": "red-clip", "source": self.sources[0], "start_ms": 500, "end_ms": 2500, "speed": 1},
            {"id": "blue-clip", "source": self.sources[1], "start_ms": 500, "end_ms": 2500, "speed": 2}]}
    ...
    def render(self, **kwargs):
        params = {"asset_id": self.sources[0]["asset_id"], "mode": "full", "encoding": "review", "composition": self.document, **kwargs}
        return self.session.call("media.process" if "processing" in params else "media.render", params)

    def pixel(self, filename, second):
        data = subprocess.check_output(["ffmpeg", "-v", "error", "-ss", str(second), "-i", filename,
                                        "-vf", "crop=2:2:158:88,format=rgb24", "-frames:v", "1", "-f", "rawvideo", "-"])
        return tuple(data[:3])

    def rms(self, filename, second, duration=.25):
        data = subprocess.check_output(["ffmpeg", "-v", "error", "-ss", str(second), "-i", filename,
                                       "-t", str(duration), "-vn", "-ac", "1", "-ar", "48000", "-f", "f32le", "-"])
    ...
    def test_mixed_dimensions_rates_audio_and_clip_order_are_encoded(self):
        ...
        self.assertGreater(self.pixel(result["path"], .5)[0], 200)
        self.assertGreater(self.pixel(result["path"], 2.5)[2], 200)
        self.assertGreater(self.rms(result["path"], .5), .03)
        self.assertLess(self.rms(result["path"], 2.5), .001)
        cached = self.render()
        ...
        self.assertGreater(self.pixel(reversed_result["path"], .5)[2], 200)
    ...
    def test_sample_straddles_cut_and_materializes_only_its_interval(self):
        ...
        self.assertGreater(self.pixel(result["path"], .2)[0], 200)
        self.assertGreater(self.pixel(result["path"], .8)[2], 200)
        self.assertGreater(self.rms(result["path"], .1), .03)
        self.assertLess(self.rms(result["path"], .7), .001)
        intermediates = list((self.workspace / "renders").glob("*/output.mkv"))
        self.assertEqual(len(intermediates), 1)
        asset = self.session.call("asset.register", {"path": str(intermediates[0]), "kind": "video"})
        info = self.session.call("media.probe", {"asset_id": asset["asset_id"]})
    ...
    def test_global_trim_speed_and_source_audio_use_the_composition_clock(self):
        result = self.render(mode="sample", start_ms=1000, end_ms=3000,
                             processing={"version": 1, "editing": {"trim": {"start_ms": 500, "end_ms": 2500}, "speed": .5}})
    ...
    def test_captions_burn_on_composition_time_for_late_sample(self):
        ...
        captioned = self.render(mode="sample", start_ms=2000, end_ms=3000, subtitle_id=asset["asset_id"])
        def frame(path):
            return subprocess.check_output(["ffmpeg", "-v", "error", "-ss", "0.4", "-i", path,
    ...
    def test_replacement_audio_sample_uses_global_output_offset_and_fades(self):
        ...
        if not soundtrack.exists():
            subprocess.run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i",
                            "sine=frequency=880:sample_rate=48000:duration=5", "-n", str(soundtrack)], check=True)
        asset = self.session.call("asset.register", {"path": str(soundtrack), "kind": "audio"})
        track = {"asset_id": asset["asset_id"], "sha256": asset["sha256"], "mode": "replace",
                 "start_ms": 0, "end_ms": 4000, "offset_ms": 500, "gain_db": 0,
                 "fade_in_ms": 1000, "fade_out_ms": 500}
        processing = {"version": 1, "editing": {"trim": {"start_ms": 500, "end_ms": 3000}, "speed": .5}}
        full = self.render(processing=processing, soundtrack=track)
        sample = self.render(mode="sample", start_ms=1000, end_ms=2500, processing=processing, soundtrack=track)
        ...
        self.assertAlmostEqual(self.rms(full["path"], 1.2), self.rms(sample["path"], .2), delta=.003)
        self.assertAlmostEqual(self.rms(full["path"], 3.2), self.rms(sample["path"], 2.2), delta=.003)
        self.assertGreater(self.rms(sample["path"], 2.2), .03)

    def test_fractional_clip_durations_share_one_frame_grid(self):
        self.document["clips"] = [
            {"id": f"cut-{index}", "source": self.sources[index % 2], "start_ms": 111, "end_ms": 412, "speed": 1}
            for index in range(12)]
        result = self.render()
        self.assertAlmostEqual(result["duration_ms"], 3612, delta=40)
        self.assertGreater(self.pixel(result["path"], .1)[0], 200)
        self.assertGreater(self.pixel(result["path"], .4)[2], 200)
    ...
    def test_validation_and_cancel_do_not_poison_next_request(self):
        ...
        request = self.session.send("media.render", {"asset_id": self.sources[0]["asset_id"],
                                    "mode": "full", "encoding": "review", "composition": self.document})
