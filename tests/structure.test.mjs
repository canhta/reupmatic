...
import { projectRoot, readSystemMap, scaffold, checkStructure } from '../scripts/system-structure.mjs';
...
function fixture(t) {
  ...
  cpSync(path.join(projectRoot, 'docs/architecture/system-map.json'), path.join(root, 'docs/architecture/system-map.json'));
  return root;
}
function update(root, change) {
  const filename = path.join(root, 'docs/architecture/system-map.json');
  const map = JSON.parse(readFileSync(filename, 'utf8')); change(map); writeFileSync(filename, JSON.stringify(map));
}
...
test('the delivered tree retains source paths and 14 scope groups without counting placeholders as code', () => {
  ...
  assert.equal(report.reserved, readSystemMap().modules.flatMap(module => module.reserved).length);
  const retained = JSON.parse(readFileSync(path.join(projectRoot, 'docs/architecture/retained-paths.json'), 'utf8'));
  assert.equal(retained.baseline, '0.14.0');
  assert.equal(retained.paths.length, 893);
  assert.equal(retained.paths.filter(name => name.endsWith('/.gitkeep')).length, 96);
  assert.ok(readSystemMap().modules.filter(module => module.status === 'planned').every(module => !module.implemented.length));
});

test('scaffolding is idempotent and never truncates existing markers or real code', t => {
  const root = fixture(t);
  const created = scaffold(root);
  assert.equal(created.length, readSystemMap(root).modules.flatMap(module => module.reserved).length);
...
test('routing is owned by Automation and an empty planned module cannot be relabeled as implemented', t => {
  const root = fixture(t);
  update(root, map => map.modules.find(module => module.id === 'distribution').reserved.push('app/core/distribution/routing'));
  assert.throws(() => scaffold(root), /Automation owns routing/);
  const other = fixture(t);
  update(other, map => map.modules.find(module => module.status === 'planned').status = 'partial');
