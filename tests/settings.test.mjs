...
async function fixture(t) {
  const root = await mkdtemp(path.join(os.tmpdir(), 'reupmatic-settings-'));
...
test('invalid directories do not replace the last valid settings', async t => {
  ...
  await assert.rejects(settings.setOutputDirectory(path.join(root, 'missing')), /OUTPUT_DIRECTORY_MISSING/);
