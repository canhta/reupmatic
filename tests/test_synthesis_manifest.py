...
SPEC = importlib.util.spec_from_file_location(
    "synthesis_manifest", Path(__file__).parents[1] / "scripts/synthesis_manifest.py"
)
...
class SynthesisManifestTests(unittest.TestCase):
    def test_generated_manifest_is_accepted_by_the_current_registry(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(
                SynthesisRegistry(target).read()["model_id"],
                SynthesisRegistry(manifest).read()["model_id"],
            )
            original = target.read_bytes()
            with self.assertRaises(FileExistsError):
                MODULE.create_manifest(
                    directory, target, json.loads(manifest.read_text())["languages"]
                )
