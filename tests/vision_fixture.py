...
HAS_NATIVE = bool(shutil.which('ffmpeg') and shutil.which('ffprobe')
                  and importlib.util.find_spec('numpy') and importlib.util.find_spec('cv2'))
...
class VisionFixture:
    def setUp(self):
        ...
        (sdk/'rapidocr/utils/typings.py').write_text('EngineType = LangRec = ModelType = OCRVersion = str\n')
...
''')
        ...
        entry = {'det':artifact,'rec':artifact,'keys':artifact,'det_version':'PP-OCRv4',
                 'rec_version':'PP-OCRv4','rec_height':48}
        manifest = self.root/'manifest.json'
        manifest.write_text(json.dumps({'version':1,'ocr':{'en':entry},'inpainting':{'model':artifact}}))
        self.env = {**os.environ, 'REUPMATIC_MODEL_MANIFEST':str(manifest), 'PYTHONPATH':str(sdk)}
        self.source = self.root/'video nguồn.mp4'
        subprocess.run(['ffmpeg','-v','error','-f','lavfi','-i','color=black:s=160x90:r=24:d=1',
                        '-f','lavfi','-i','sine=frequency=600:duration=1','-c:v','libx264','-threads','2',
                        '-c:a','aac','-shortest','-n',str(self.source)],check=True)
    ...
    def session(self, slow=False):
        ...
        aid = session.call('asset.register',{'path':str(self.source),'kind':'video'})['asset_id']
        return session,aid
    def inpaint(self, aid, target='manual'):
        p={'asset_id':aid,'start_ms':0,'end_ms':1000,'target':target,'padding_px':0}
        p.update({'region':{'x':.25,'y':.2,'width':.5,'height':.4}} if target=='manual' else {'language':'en'})
        return p
