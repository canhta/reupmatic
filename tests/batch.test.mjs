...
import { outputFilename, publishBatchOutput } from '../dist-core/batch/batch-output.js';
...
async function until(predicate) {
  ...
  while (!predicate()) { if (Date.now() > end) throw new Error('condition timeout'); await sleep(); }
}
async function fixture(t) {
  const temp = await mkdtemp(path.join(os.tmpdir(), 'reupmatic-batch-'));
  const cleanup = [];
  t.after(async () => {
    try { for (const callback of [...cleanup].reverse()) await callback(); }
    finally { await rm(temp, { recursive: true, force: true }); }
  });
  const directory = await realpath(temp);
  const input = { video: { path: path.join(directory, 'nguồn.mp4'), sha256: digest('input'), name: 'nguồn.mp4' }, output_dir: directory, encoding: 'review' };
...
test('SQLite idempotent batch admission keeps one snapshot, rejects reused key with changed data', async t => {
  ...
  const jobs = store.enqueue('request-0001', [input]); input.video.name = 'changed';
  assert.equal(jobs.length, 1); assert.equal(store.list()[0].input.video.name, 'nguồn.mp4');
...
test('claim increments attempt; explicit retry retains job identity and input', async t => {
  ...
  assert.equal(store.claim().attempt, 1); assert.equal(store.claim(), null);
  store.finish(job.id, 'failed', 'INVALID_MEDIA'); store.retry(job.id);
  const retried = store.claim(); assert.equal(retried.id, job.id); assert.equal(retried.attempt, 2);
  assert.deepEqual(retried.input, job.input); store.finish(job.id, 'complete', null, {
    path: path.join(input.output_dir, 'result.mp4'), sha256: digest('output'), duration_ms: 2000, cache_hit: false,
  });
...
test('restart retains queued/done rows and recovers running versus acknowledged cancellation', async t => {
  ...
  store.claim(); store.finish(rows[0].id, 'complete', null, { path: path.join(directory, 'done.mp4'), sha256: digest('done'), duration_ms: 1, cache_hit: false });
  store.claim(); store.claim(); store.cancel(rows[2].id); store.close();
  const reopened = new BatchStore(path.join(directory, 'queue.sqlite')); cleanup.push(() => reopened.close());
  assert.equal(reopened.recover(), 2);
  assert.deepEqual(reopened.list().map(row => row.state), ['complete', 'interrupted', 'cancelled', 'queued']);
  assert.equal(reopened.recover(), 0);
});

test('invalid batch does not partly enqueue and relative paths are rejected', async t => {
  const { store, input } = await fixture(t);
  assert.throws(() => store.enqueue('request-0004', [input, { ...input, output_dir: 'relative' }]), { code: 'INVALID_REQUEST' });
...
class ControlledWorker {
  ...
  request(method, params) {
    ...
    return { id: `registration-${this.calls.length}`, result: Promise.resolve({ asset_id: `asset-${this.calls.length}`, sha256: this.hash }), cancel: async () => ({ requested: true }) };
  }
}
class ControlledRenderer extends EventEmitter {
  ...
  start(input, subtitle, modelFingerprints) {
    ...
    this.activeCount++;
    const finish = () => { this.activeCount--; this.emit('idle'); };
    const call = {
      input, subtitle, modelFingerprints,
      resolve: data => { finish(); resolve(data); },
      reject: error => { finish(); reject(error); },
    };
    this.calls.push(call);
    return { id: input.request_id, result, cancel: async () => { call.reject(new RemoteError('CANCELLED')); return { requested: true }; } };
  }
}
async function controlled(t, count = 2) {
  const f = await fixture(t);
  const artifact = path.join(f.directory, 'cache.mp4'); await writeFile(artifact, 'native-result');
  const renderer = new ControlledRenderer(), worker = new ControlledWorker(f.input.video.sha256);
  const queue = new BatchQueue(f.store, worker, renderer);
  queue.enqueue('request-controlled', Array.from({ length: count }, () => f.input));
  const output = { path: artifact, sha256: digest('native-result'), artifact_id: 'cache-artifact', duration_ms: 2000, cache_hit: false };
  f.cleanup.push(async () => { queue.beginClose(); await queue.finishClose(); });
...
test('queue starts paused, isolates a bad item, then exports good item through same renderer', async t => {
  ...
  await sleep(); assert.equal(renderer.calls.length, 0);
  queue.resume(); await until(() => renderer.calls.length === 1);
  ...
  assert.equal(rows[0].state, 'failed'); assert.equal(rows[1].state, 'complete');
...
test('pause allows current output, keeps next queued; closing the UI is not cancellation', async t => {
  ...
  queue.resume(); await until(() => renderer.calls.length === 1); queue.pause();
  renderer.calls[0].resolve(output); await until(() => queue.snapshot().items[0].state === 'complete');
  assert.equal(renderer.calls.length, 1); assert.equal(queue.snapshot().items[1].state, 'queued');
  queue.resume(); await until(() => renderer.calls.length === 2); renderer.calls[1].resolve(output);
  await until(() => queue.snapshot().items[1].state === 'complete');
});
...
test('cancel running and queued entries; retry never creates a second job', async t => {
  ...
  await queue.cancel(ids[1]); assert.equal(queue.snapshot().items[1].state, 'cancelled');
  queue.resume(); await until(() => renderer.calls.length === 1);
  await queue.cancel(ids[0]); await until(() => queue.snapshot().items[0].state === 'cancelled');
  queue.retry(ids[0]); await until(() => renderer.calls.length === 2); renderer.calls[1].resolve(output);
  await until(() => queue.snapshot().items[0].state === 'complete');
  assert.equal(queue.snapshot().items.length, 2); assert.equal(queue.snapshot().items[0].attempt, 2);
});
...
test('a changed source cannot reach render; runtime failure pauses the waiting group', async t => {
  ...
  worker.hash = digest('modified'); queue.resume();
  ...
  queue.pause(); queue.retry(queue.snapshot().items[0].id); queue.retry(queue.snapshot().items[1].id);
  worker.request = () => ({ id: 'failed-runtime', result: Promise.reject(new RemoteError('WORKER_EXITED')), cancel: async () => ({ requested: false }) });
  queue.resume(); await until(() => queue.snapshot().fault === 'WORKER_EXITED');
  assert.equal(queue.snapshot().paused, true); assert.equal(queue.snapshot().items[1].state, 'queued');
});
...
test('existing preview has dispatch priority before the next batch item', async t => {
  ...
  renderer.activeCount = 1; queue.resume(); await sleep(); assert.equal(renderer.calls.length, 0);
  renderer.activeCount = 0; renderer.emit('idle'); await until(() => renderer.calls.length === 1);
  renderer.calls[0].resolve(output); await until(() => queue.snapshot().items[0].state === 'complete');
});

test('output commit is exclusive, retries reconcile identical files and preserve conflicts', async t => {
  const { store, directory } = await fixture(t);
  const source = path.join(directory, 'render.mp4'); await writeFile(source, 'expected');
  const destination = await publishBatchOutput(source, directory, 'safe.mp4', digest('expected'), () => {});
  assert.equal(await hashFile(destination), digest('expected'));
  assert.equal(await publishBatchOutput(source, directory, 'safe.mp4', digest('expected'), () => {}), destination);
  await writeFile(path.join(directory, 'conflict.mp4'), 'user-owned');
  await assert.rejects(publishBatchOutput(source, directory, 'conflict.mp4', digest('expected'), () => {}), { code: 'OUTPUT_CONFLICT' });
  assert.equal(await readFile(path.join(directory, 'conflict.mp4'), 'utf8'), 'user-owned');
  await assert.rejects(publishBatchOutput(source, directory, '../escape.mp4', digest('expected'), () => {}), { code: 'INVALID_REQUEST' });
});

test('cancel before output commit produces no final export, does not delete the source', async t => {
  const { store, directory } = await fixture(t);
  const source = path.join(directory, 'render.mp4'); await writeFile(source, 'expected');
  await assert.rejects(publishBatchOutput(source, directory, 'cancelled.mp4', digest('expected'), () => { throw new RemoteError('CANCELLED'); }), { code: 'CANCELLED' });
...
test('queue shutdown retains interrupted state and does not start the next item', async t => {
  ...
  queue.resume(); await until(() => renderer.calls.length === 1); queue.beginClose();
  await until(() => queue.snapshot().items[0].state === 'interrupted');
  assert.equal(queue.snapshot().items[1].state, 'queued'); assert.equal(renderer.calls.length, 1);
});
...
test('library provenance is durable and part of idempotent batch admission', async t => {
  ...
  assert.throws(() => store.enqueue('library-batch-001', [{ ...request, library_id: 'library-separate-002' }]),
  ...
  assert.throws(() => reopened.enqueue('invalid-library-001', [{ ...input, library_id: '../outside' }]),
...
test('shared queue forwards the saved recipe, model pins and stage progress on every attempt', async t => {
  ...
  const snapshot = queue.enqueue('processing-queued', [{ ...f.input, processing, processing_models }]);
  ...
  renderer.emit('job', { v: 1, id: renderer.calls[0].input.request_id, revision: 1, event: 'progress',
    data: { phase: 'processingInpaint', fraction: 0.5 } });
  assert.deepEqual(queue.snapshot().items[0].progress, { phase: 'processingInpaint', fraction: 0.5 });
  ...
  queue.retry(id); await until(() => renderer.calls.length === 2);
