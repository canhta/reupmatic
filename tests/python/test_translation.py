...
from speech.translation.contracts import apply_rules, parse_options, validate_output
...
class TranslationValidationTests(unittest.TestCase):
    ...
    def test_rules_are_literal_ordered_target_edits_with_expansion_bounds(self):
        self.assertEqual(
            apply_rules(
                "a.b aXb", [{"find": "a.b", "replace": "$1"}, {"find": "$1", "replace": "Tên"}]
            ),
            "Tên aXb",
        )
...
class TranslationManifestTests(unittest.TestCase):
    ...
    def test_configuration_normalizes_local_paths_without_loading_runtime(self):
        ...
        with patch("speech.translation.models.runtime_available", return_value=False):
            status = configure_translation(
                host, {"id": "configure", "params": {"path": str(self.manifest)}}
            )
    ...
    def test_unconfigure_forgets_only_the_configured_directory(self):
        workspace = self.root / "workspace"
        workspace.mkdir()
        host = SimpleNamespace(workspace=workspace, cancelled=lambda _: None)
        configure_translation(host, {"id": "configure", "params": {"path": str(self.manifest)}})
        ...
        # A directory this bundle does not live under removes nothing.
        unconfigure_translation(host, {"params": {"directory": str(self.root / "elsewhere")}})
        self.assertTrue(target.exists())
        status = unconfigure_translation(host, {"params": {"directory": str(self.model)}})
