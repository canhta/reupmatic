...
from media.editing.recipe import parse_editing  # noqa: E402
...
class CompositionNativeTests(unittest.TestCase):
    ...
    def mean_luma(self, filename, second):
        data = subprocess.check_output(
            [
                "ffmpeg",
                "-v",
    ...
    def pixel_at(self, filename, second, x, y):
        data = subprocess.check_output(
            [
                "ffmpeg",
                "-v",
    ...
    def test_disabled_clip_keeps_its_clock_place_and_renders_black(self):
        document = copy.deepcopy(self.document)
        document["clips"][1]["enabled"] = False
        result = self.render(composition=document)
        # The disabled clip keeps its place on the output clock: the montage is not shortened.
        self.assertAlmostEqual(result["duration_ms"], 3000, delta=40)
        self.assertGreater(self.pixel(result["path"], 0.5)[0], 200)
        self.assertLess(self.pixel(result["path"], 2.5)[0], 20)
        self.assertLess(self.pixel(result["path"], 2.5)[2], 20)
        self.assertGreater(self.rms(result["path"], 0.5), 0.03)
        self.assertLess(self.rms(result["path"], 2.5), 0.001)

    def test_composition_applies_global_rotate_fade_and_logo(self):
        """The Edit panel's global framing/colour/fades/logo apply to the assembled
        output, not only to a single video. The same recipe the panel writes is
        rendered over a two-clip composition: the filter graph carries the rotate,
        fade and logo stages, and a sampled frame shows the fade and the logo."""
        ...
        logo_asset = self.session.call("asset.register", {"path": str(logo_path), "kind": "image"})
        logo = {
            "asset_id": logo_asset["asset_id"],
            "sha256": logo_asset["sha256"],
        }
        editing = {
            "rotate": 180,
            "fade": {"in_ms": 500, "out_ms": 500, "audio": False},
            "logo": {"anchor": "top-left", "margin": 0.02, "scale": 0.25, "opacity": 1},
        }

        # Graph: the one global recipe resolves to the rotate, fade and overlay stages.
        parsed = parse_editing(editing)
        geometry, _ = geometry_filters(
            parsed, {"width": 320, "height": 180, "has_audio": True, "frame_rate": "30"}
        )
        self.assertTrue(
            any("hflip" in value and "vflip" in value for value in geometry),
            geometry,
        )
        self.assertEqual(video_fade_filters(parsed, 3000)[0], "fade=t=in:st=0:d=0.500")
        overlay, overlay_x, overlay_y = logo_overlay(parsed["logo"], (320, 180))
        self.assertTrue(any(value.startswith("scale=") for value in overlay))
        self.assertEqual((overlay_x, overlay_y), ("6", "6"))

        result = self.render(processing={"editing": editing}, logo=logo)
        # The global edits do not change the assembled output's duration.
        self.assertAlmostEqual(result["duration_ms"], 3000, delta=40)
        self.assertEqual((result["width"], result["height"]), (320, 180))

        # Sampled frame: the head fade darkens the first 500 ms of the output…
        head = self.mean_luma(result["path"], 0.05)
        middle = self.mean_luma(result["path"], 1.5)
        self.assertGreater(middle, 40)
        self.assertLess(head, middle * 0.5)
        # …and the logo (green) is painted over the red first clip at its anchor.
        red, green, blue = self.pixel_at(result["path"], 1.5, 8, 8)
