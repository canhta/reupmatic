...
def speech(*, mode="mix", sample_rate=24000, rate=1, gain_db=0, fade_in_ms=0, fade_out_ms=0):
    """One line at offset 500 ms whose speech is 24 000 frames — one second at 24 kHz."""
    return {
        "mode": mode,
...
class VoiceMixingArguments(unittest.TestCase):
    """The audio arrangement is asserted by the arguments it builds, never by encoding audio.
    Every line is placed at its own plan offset and trimmed by its own frame span over the
    recording's reported rate."""
    ...
    def test_voice_with_original_audio_mixes_both_branches(self):
        ...
        # No soundtrack: original audio is branch one and voice branch two, summed by amix.
        self.assertIn("[original][voice]amix=inputs=2", value)
        # With music set to mix too, all three join the mix.
    ...
    def test_voice_replaces_original_audio_while_music_still_sums(self):
        # A dub: voice mode replace drops original audio, but the music branch remains.
    ...
    def test_voice_level_and_fades_are_adjustable_on_the_track(self):
        ...
        # fade-out starts on the placed speech's end: 500 ms + 1000 ms - 300 ms.
...
class VoicePlacementExecutesThePlan(unittest.TestCase):
    """The recording's internal spacing is not the timing: each line is trimmed by its own span and
    delayed to its own planned offset, so changing the producer's GAP_MS cannot move any line."""
    ...
    def test_two_lines_are_placed_independently_not_concatenated(self):
        ...
        # Each line trimmed to its speech, never the whole recording.
        self.assertIn("atrim=start=0.000000000:end=1.000000000", value)
    ...
    def test_a_compressed_line_is_placed_at_the_slot_length_the_rate_was_computed_for(self):
        # The plan computes rate = speech / slot; the worker executes it with atempo, so the placed
        # duration is span / rate. A 1.0 s line at rate 2 lands in the 0.5 s slot it was rated for.
        voice = {
            "mode": "mix",
            "sample_rate": 24000,
            "gain_db": 0,
        ...
        # The fade-out begins on the placed end: 500 ms + 1000/2 ms - 100 ms = 0.900000 s.
