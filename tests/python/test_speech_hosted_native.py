"""Real worker protocol against a local stub endpoint standing in for DashScope — never a real
provider. Covers the hosted-protocol adapter's boundary behaviour: what a
hosted job is accepted or refused for, that the credential reaches only the hosted child's
environment and never the on-disk job file, and that cancellation reaps that child exactly as it
already does for the local one.
"""
...
class SpeechHostedNativeTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # A real, self-signed TLS listener stands in for DashScope's real HTTPS endpoint (the
        # adapter always speaks https://, per its own wire shape — never a config toggle down to
        # plain HTTP just for a test double). The hosted child trusts this one certificate only,
        # via SSL_CERT_FILE in its inherited environment (see `session`), exactly the way a real
        # deployment would trust a real CA — nothing in the adapter itself is aware of a test.
    ...
    def session(self):
        # `SSL_CERT_FILE` here is the worker process's own environment, read by
        # `worker/runtime/process.py`'s `os.environ` merge when it spawns the hosted child —
        # exactly the mechanism a real deployment's own OS/CA trust store fills normally.
    ...
    def test_hosted_transcription_succeeds_and_the_stub_receives_only_the_bearer_credential(self):
        ...
        # The credential travelled as the Authorization header value, never as request content.
    ...
    def test_a_request_over_its_models_own_duration_bound_is_refused_before_any_audio_moves(self):
        session, asset = self.session()
        # 2000ms requested against a 1000ms model bound (1000 is itself the lowest a model may
        # ever declare — app/core/speech/providers.ts's parseModelDraft) — strictly over, never
        # merely at, the bound.
    ...
    def test_missing_credential_and_missing_provider_are_both_a_plain_bad_request(self):
        ...
        del base["provider"]
        del base["credential"]
        # A local-shaped request naming a model_id nothing local configured is refused the same
        # way it always was — this ticket adds a path, not a change to that existing one.
