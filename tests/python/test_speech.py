...
class ManifestTests(unittest.TestCase):
    """The hand-authored, single-engine candidate manifest a user points the app at."""
    ...
    def test_manifest_is_strict_and_resolves_local_files(self):
        ...
        # Pinned against the pre-descriptor formula so an engine-seam refactor
        # cannot silently change how existing bundles are identified.
        self.assertEqual(
    ...
    def test_qwen3_asr_manifest_validates_and_its_digest_differs_from_faster_whisper(self):
        ...
        # Same declared languages, wholly different engine/bundle: the digest
        # must not collide, per the pluggable-engine spec's identity rule.
    ...
    def test_qwen3_forced_aligner_manifest_validates_with_the_same_file_set_as_qwen3_asr(self):
        ...
        # listing: identical filenames to the qwen3-asr 0.6B bundle, no sharded
        # variant. Still a distinct engine with its own digest, because the
        # digest also covers the declared `engine` name, not just the files.
...
class SpeechEnginesTests(unittest.TestCase):
    """The workspace-persisted, multi-engine store `configure_speech` writes."""
    ...
    def test_reconfiguring_one_engine_does_not_change_the_others_model_id(self):
        with (
            ...
            # Reconfigure faster-whisper with a different bundle; qwen3-asr's
            # recorded identity, and any draft resolved against it, is untouched.
    ...
    def test_old_single_engine_store_shape_fails_loudly_never_migrated(self):
        # A single-engine store (one engine at the top level) does not partially
        # resolve as the multi-engine shape: it is refused outright, on both status
        # (non-throwing) and configure (throwing), and is never silently upgraded.
    ...
    def test_optional_is_none_when_a_companion_engine_is_simply_not_there_to_help(self):
        ...
        # Configured, but for a language the bundle does not declare.
        self.assertIsNone(engines.optional("qwen3-forced-aligner", "en"))
        # Configured and language matches, but its runtime is absent.
    ...
    def test_unconfigure_forgets_only_the_engine_at_that_directory(self):
        ...
        # A directory no engine points at removes nothing.
...
class AdapterDispatchTests(unittest.TestCase):
    """The forced aligner is a companion engine: it never runs on its own."""
...
class SpeechServiceTests(unittest.TestCase):
    ...
    def test_word_timings_are_ordered_and_bounded_within_their_cue(self):
        # A companion aligner's word timings, as qwen3_asr's adapter attaches them:
        # seconds relative to the same clock as the segment's own start/end.
    ...
    def test_a_segment_with_no_words_attribute_produces_no_word_timings(self):
        # faster-whisper's segments never carry `.words`; behaviour is unchanged.
    ...
    def test_word_timing_validation_rejects_out_of_order_or_out_of_bounds_words(self):
        cases = [
            # Starts before the cue's own start.
            [{"text": "a", "start": -0.1, "end": 0.2}],
            # Overlaps/precedes the previous word.
            [
    ...
    def test_word_timings_fail_fast_on_size_before_the_outer_result_limit(self):
        # Each word individually fits comfortably; a large enough run must still
        # be refused explicitly rather than growing past the shared result budget.
...
class CueSegmentationTests(unittest.TestCase):
    """Ticket 11: an engine that returns no segments gets cues derived from the
    aligner's word timings, without ever altering the recognised words.

    Boundaries only ever fall between aligner word tokens, so a space inside a
    Vietnamese word (a syllable pair) or the absence of spaces in Chinese never
    drives a split.
    """

    def test_an_engine_that_provides_its_own_segments_is_never_re_segmented(self):
        # A segment carrying words with a wide gap would split if derivation ran.
        # Because this engine reports its own boundaries, it comes through with
        # exactly one cue and untouched bounds.
        segment = SimpleNamespace(
            start=0.0,
            end=3.0,
            text=" A B ",
    ...
    def test_english_unsegmented_engine_derives_ordered_cues_that_rebuild_the_transcript(self):
        ...
        # The words are never rewritten or dropped: concatenating the cue text
        # rebuilds the engine's transcript character for character.
    ...
    def test_chinese_derives_without_whitespace_and_rebuilds_the_transcript(self):
        # No spaces at all: naive whitespace splitting cannot work, and the join
        # that proves preservation must be empty-string, not a space. The break
        # comes from the silence gap between the two runs of characters.
    ...
    def test_vietnamese_syllables_are_kept_whole_at_an_aligner_word_boundary(self):
        # "Việt" + "Nam" are two syllables of one word. Boundaries come from the
        # aligner tokens, so a split falls between words, never inside one.
        segment = self.segment(
            "Việt Nam học sinh",
            [
                ("Việt", 0.1, 0.6),
    ...
    def test_unmappable_word_texts_fall_back_to_the_single_engine_cue(self):
        # An aligner that normalises text it cannot place back in the transcript
        # must not force a split that could drop or rewrite a character: the
        # engine's own single cue is returned instead, words intact.
        segment = self.segment(
            "Hello world",
            [
                ("hello", 0.1, 0.5),
    ...
    def test_adjacent_limit_break_never_overlaps_the_next_cue(self):
        # A character-limit break lands exactly where the next word starts, with
        # no silence. The first cue is shorter than the published minimum and is
        # extended only as far as the next cue's start: never past it.
