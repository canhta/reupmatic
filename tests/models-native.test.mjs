...
test('real Python protocol configures local files, rejects bad checksum and persists across restart; no inference', async t => {
  ...
  const directory = await mkdtemp(path.join(os.tmpdir(), 'reupmatic-model-config-'));
  ...
  const good = { version: 1, inpainting: { model: { path: 'test-only.onnx', sha256 } } };
  await writeFile(manifest, JSON.stringify(good));
  const start = () => new WorkerClient(pythonExecutable(root), path.join(root, 'worker/main.py'), workspace);
  let worker = start();
  try {
    ...
    await assert.rejects(worker.request('models.configure', { path: manifest }).result, { code: 'MODEL_HASH_MISMATCH' });
