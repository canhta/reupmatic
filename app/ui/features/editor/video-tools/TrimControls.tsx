...
export function TrimControls() {
  ...
  return (
    <div className="business-form">
      <CheckboxInput
        label={t(editor.composition ? 'compositionTrim' : 'editTrim')}
      ...
      {editing?.trim && (
        <div className="business-toolbar">
          ...
          <Button
            label={t('editMarkIn')}
            isDisabled={disabled}
            onClick={() => update({ ...trim, start_ms: editor.clock })}
          />
          <Button
            label={t('editMarkOut')}
            isDisabled={disabled}
            onClick={() => update({ ...trim, end_ms: editor.clock })}
          />
        </div>
      )}
      <p className="field-help">
        {t(editor.composition ? 'compositionClock' : 'editSourceTimeHint')}
      </p>
      <span role="status">
