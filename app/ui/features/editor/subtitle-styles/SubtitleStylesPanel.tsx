  ...
  return (
    <Collapsible trigger={t('styleTitle')} defaultIsOpen={false}>
      <Selector
        ...
        options={[
          { value: 'global', label: t('styleGlobal') },
          { value: 'cue', label: t('styleOneCue') },
        ]}
        onChange={(value) => {
          setScope(value);
          setTarget(editor.selected);
        }}
      />
      {scope === 'cue' && (
        <Selector
          ...
          value={target}
          isDisabled={disabled || dirty}
          options={editor.cues.map((cue, index) => ({
            value: cue.id,
            label: `${index + 1}. ${cue.text.slice(0, 60)}`,
          }))}
          onChange={setTarget}
        />
      )}
