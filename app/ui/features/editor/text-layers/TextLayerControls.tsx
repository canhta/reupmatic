    ...
    <div className="business-form">
      <div className="business-toolbar">
        <Selector
          ...
          options={options}
          onChange={(value) => {
            if (textLayerNames.includes(value as TextLayerName))
              editor.selectTextLayer(value as TextLayerName);
          }}
        />
        <Selector
          label={t('textLayerLanguage')}
          value={layer.language ?? 'unknown'}
          options={['unknown', 'en', 'vi', 'zh'].map((value) => ({
            value,
            label: value === 'unknown' ? t('textLanguageUnknown') : t(`visionLanguage_${value}`),
          }))}
          onChange={(value) => {
            if (['unknown', 'en', 'vi', 'zh'].includes(value))
              editor.changeLayerCues(layer.cues, to, {
      ...
      <p role="status">
        {t('textLayerOrigin', { origin: t(`textOrigin_${layer.origin.kind}`) })}
        {layer.edited ? ` · ${t('textManuallyEdited')}` : ''}
      </p>
      {layer.stale && (
        <Banner
          status="warning"
      ...
      {(to === 'spoken' || to === 'translated') && (
        <p className="field-help">{t('textServicesUnavailable')}</p>
      )}
        ...
        {preview && (
          <>
            ...
            <Table density="compact" aria-label={t('rulesComparison')}>
              ...
              <TableBody>
                {preview.value.cues.slice(0, 25).map((cue, index) => (
                  <TableRow key={cue.id}>
                    <TableCell>
                      <span className="rule-comparison-text">
                        {previousCues[index]?.text ?? '—'}
                      </span>
                    </TableCell>
                    <TableCell>
