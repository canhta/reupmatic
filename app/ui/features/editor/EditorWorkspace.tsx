...
const errorKeys: Record<string, string> = {
  ...
  PROJECT_VERSION: 'projectVersionError',
...
export function EditorWorkspace() {
  ...
  return (
    <div className="editor-workspace">
      <RecoveryPanel />
      {media && (
        <small role="status">{editor.dirty ? t('projectUnsaved') : t('projectClean')}</small>
      )}
      ...
      {!media ? (
        ...
        <>
          ...
          {editor.composition ? (
            <p className="notice">{t('compositionAiUnavailable')}</p>
          ) : (
            <VisionPanel
              key={media.asset_id}
              ...
              onApply={(result, captured) => {
                if (!canApplyOcr(result, captured, media.asset_id, editor.rev.current))
                  return false;
