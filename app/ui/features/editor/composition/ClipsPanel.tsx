...
/**
 * The inspector's Clips tab: composition order/range/speed as numbers, one
 * trim concept, one clip speed. When there is no clip composition, the
 * whole video is one implicit clip: its in/out *is* the trim and its own
 * speed is the only speed field, instead of a second "playback speed"
 * control duplicating the same idea.
 *
 * The global framing/colour/fades/logo sections sit below the clip list in
 * both modes: the recipe they write is global and the worker applies it to
 * the assembled output, so a two-clip composition keeps rotate, crop, colour,
 * fades and the logo — they must not vanish with the single-video controls.
 */
...
function WholeVideoClip() {
  ...
  try {
    ...
    /* Invalid edits remain visible for correction; rendering validates them. */
  }
  function update(patch: Partial<EditingRecipe>) {
    const next: EditingRecipe = { ...editing, ...patch };
    for (const key of Object.keys(next) as (keyof EditingRecipe)[]) {
