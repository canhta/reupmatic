...
export function TextRulePanel() {
  ...
  const [rule, setRule] = useState<TextRule>({
    mode: 'literal',
    find: '',
    replacement: '',
    case_sensitive: true,
  ...
  const applicable =
    preview && preview.revision === editor.revision && preview.settings === settings;
    ...
    <div className="business-form">
      <Collapsible trigger={t('rulesTitle')} defaultIsOpen={false}>
        <p className="field-help">
          {t('rulesLayerNamed', { layer: t(`textLayer_${editor.activeTextLayer}`) })}
        </p>
        ...
        <div className="business-toolbar">
          <TextInput
            label={t('find')}
            value={rule.find}
            onChange={(find) => setRule({ ...rule, find })}
          />
          <TextInput
            label={t('replace')}
            value={rule.replacement}
            onChange={(replacement) => setRule({ ...rule, replacement })}
          />
          <Button
        ...
        {error && (
          <Banner
            status="error"
            title={t(error === 'TEXT_RULE_TIMEOUT' ? 'rulesTimeout' : 'rulesInvalid')}
        ...
        {preview && (
          <>
            <span role="status">
              {t('rulesResult', {
                count: preview.result.matched_cues,
                matches: preview.result.replacements,
              })}
            </span>
            ...
            <Table density="compact" aria-label={t('rulesComparison')}>
              <TableHeader>
                <TableRow isHeaderRow>
                  ...
                  <TableHeaderCell>{t('rulesAfter')}</TableHeaderCell>
                </TableRow>
              </TableHeader>
              <TableBody>
                {preview.result.changes.slice(0, 25).map((change) => (
                  <TableRow key={change.id}>
                    <TableCell>
                      <span className="rule-comparison-text">{change.before}</span>
                    </TableCell>
                    <TableCell>
                      <span className="rule-comparison-text">{change.after}</span>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
            ...
            <Button
              label={t('rulesApply')}
              variant="primary"
              isDisabled={!applicable || !preview.result.matched_cues}
              onClick={() => {
