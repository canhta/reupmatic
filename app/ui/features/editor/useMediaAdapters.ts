...
export function useMediaAdapters() {
  ...
  useEffect(() => {
    ...
    try {
      instance = new JASSUB({
        video: video.current, subContent: '[Script Info]\nScriptType: v4.00+\n', queryFonts: false,
      });
    } catch { report(new Error('PREVIEW_UNAVAILABLE')); return; }
    renderer.current = instance;
    void instance.ready.catch(() => { if (alive) report(new Error('PREVIEW_UNAVAILABLE')); });
    return () => {
      alive = false;
      renderer.current = null;
      void instance.destroy();
    };
  }, [composition, media?.asset_id, report, video]);
  ...
  useEffect(() => {
    ...
    }).catch(() => { if (alive) report(new Error('PREVIEW_UNAVAILABLE')); });
    return () => { alive = false; };
  }, [ass, rev, report]);
  ...
  useEffect(() => {
    ...
    void unwrap<{ peaks: number[]; duration_ms: number }>(
      ...
      instance = WaveSurfer.create({
        container: wave.current, media: video.current, peaks: [data.peaks],
        duration: data.duration_ms / 1000, height: 56, normalize: false,
      });
      instance.on('error', () => report(new Error('PREVIEW_UNAVAILABLE')));
    }).catch(reason => { if (alive) report(reason); });
    return () => { alive = false; instance?.destroy(); };
