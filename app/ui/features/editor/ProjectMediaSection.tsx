...
interface Row {
  ...
  used: boolean;
  missing: boolean;
  /** Subtitle only: the text layer the file was imported into, if any. */
  importedLayer?: TextLayerName;
  /** Present only for the stored files whose path this section can relink. */
  stored?: ProjectMedia;
}
...
/**
 * Project media (ED-P01, D-63): the project's videos, soundtrack and imported
 * subtitle files, one row each, above the permanent cue list. Adding never uses
 * media — a row shows a used marker once it is on the timeline; excluding and
 * removing follow the studio conventions of study §9, never a checkbox. The
 * primary video, composition clips and soundtrack rows are derived from the
 * document; only added-but-unplaced videos and subtitle files are stored.
 */
export function ProjectMediaSection() {
  ...
  if (editor.media) {
    rows.push({
      ...
      // Deleting the clip from the timeline keeps this row; the used marker clears once no
      // placement of the source video remains (ED-P01, D-63).
      used:
        !editor.composition ||
  ...
  for (const item of editor.projectMedia) {
    // A stored video that ticket 05 later places on the timeline is already
    // shown by its composition-clip row; never list the same file twice.
  ...
  // The marker's accessible name and tooltip, not a bare colour: an SRT names
  // the layer it reached, everything else is simply on the timeline.
  ...
  // A stored video not yet on the timeline is the one row this section places:
  // it appends at the end here, or is dragged onto the clips lane (ticket 05).
  // The source video, composition clips and used rows are already placed.
  ...
  // The row's own actions. An unused video row offers Add to timeline; an unused
  // subtitle row offers Import. Both appear in the row's overflow and context menu.
  ...
  // Only a placeable stored video starts a drag; the band's drop target reads the
  // row id back from the payload and inserts it at the drop position.
  ...
  return (
    <div className="project-media" onContextMenuCapture={capture}>
      {/* The panel's one primary action heads its body, the way a media bin's
          import control does (owner — it used to float mid-panel
          between a collapsible trigger and the list). */}
      <Button
        label={t('mediaAdd')}
        size="sm"
        icon={<Icon icon={Plus} size="sm" />}
        width="100%"
        // With no project open this adds the first video and creates one
        // (D-63); audio and subtitle imports still need an open project.
          ...
          <List density="compact" hasDividers>
            {rows.map((row) => {
              ...
              return (
                <ListItem
                  key={row.key}
                  // The marker rides the row root so a right-click
                  // anywhere on the row can resolve its own actions.
                  ...
                  endContent={
                    <span className="project-media-actions">
                      ...
                      {canPlace && (
                        // Clipchamp's "+": a compact icon button whose accessible
                        // name and tooltip carry the action, leaving the row's own
                        // file name the width it needs (ticket 05 review).
                        <IconButton
                          label={t('mediaAddToTimeline')}
