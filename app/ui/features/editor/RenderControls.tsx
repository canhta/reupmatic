...
export function RenderControls() {
  ...
  const compositionBlocked = Boolean(editor.composition && (editor.processing?.ocr || editor.processing?.inpaint));
  const unavailable = compositionBlocked || editor.busy || editor.opening || !editor.cap?.ffmpeg
    || ((editor.cues.length > 0 || editor.processing?.ocr) && !editor.cap?.pysubs2)
    || Boolean(editor.cues.length && editor.processing?.ocr);
  return <>
    {compositionBlocked && <p className="warning" role="status">{t('compositionAiUnavailable')}</p>}
    <TrimControls />
    <ProfilePicker onApply={editor.changeProcessing} disabled={editor.busy || editor.opening}
      hasSubtitles={editor.cues.length > 0} />
    <ProcessingOptions showSubtitleStyle={false} value={editor.processing} onChange={editor.changeProcessing}
      disabled={editor.busy || editor.opening} hasSubtitles={editor.cues.length > 0} />
    <footer className="render-controls">
      ...
      {editor.job && <Button
        ...
        type="button"
        onClick={() => {
        if (editor.job) void unwrap(window.reupmatic.cancel(editor.job.id)).catch(editor.report);
      }}
      />}
    </footer>
