...
function draftOf(label?: Label): SaveLabel {
  return { id: label?.id ?? crypto.randomUUID(), expected_revision: label?.revision ?? null,
    name: label?.name ?? '', kind: label?.kind ?? 'tag', archived: label?.archived ?? false };
}
...
export function LabelManager() {
  ...
  const rows = (catalog.snapshot?.labels ?? []).filter(label => label.name.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="business-grid">
      <div className="business-list">
        <div className="business-toolbar">
          ...
          <Button label={t('labelCreate')} isDisabled={disabled} onClick={() => void form.choose(draftOf())} />
        </div>
        {!rows.length ? <EmptyState title={t('labelsEmpty')} description={t('labelsEmptyHelp')} /> :
          <Table density="compact" aria-label={t('catalogLabels')}>
            ...
            <TableBody>{rows.slice(offset, offset + 25).map(label => <TableRow key={label.id}>
              <TableCell>{label.name}{label.archived && <p>{t('catalogArchived')}</p>}</TableCell>
              <TableCell>{t(`labelKind_${label.kind}`)}</TableCell>
              <TableCell><Button label={t('catalogEdit')} isDisabled={disabled}
                onClick={() => void form.choose(draftOf(label))} /></TableCell>
            </TableRow>)}</TableBody>
          </Table>}
      ...
      <div className="business-form">
        ...
        <Selector label={t('labelKind')} value={draft.kind} isDisabled={disabled || draft.expected_revision !== null}
          options={['tag', 'category', 'group'].map(value => ({ value, label: t(`labelKind_${value}`) }))}
        ...
        <div className="action-row">
          <Button label={t('catalogSave')} variant="primary" isDisabled={disabled || !draft.name.trim() || (!form.dirty && !!draft.expected_revision)}
            onClick={() => void save()} />
