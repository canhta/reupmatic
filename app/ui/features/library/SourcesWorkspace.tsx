...
/**
 * SCR-01's two tabs, and only those two (ticket 14): Downloads and Library. A record's
 * child assets are nested inside its DetailSurface, and label management lives in a dialog
 * off the Library toolbar — neither is a peer destination.
 */
export function SourcesWorkspace({ batchBusy, onBatch, onEditor }: Props) {
  ...
  // The Library's labels live in the catalog's taxonomy, not on the content record, so the list
  // resolves ids to names here and hands the table plain maps to render.
  ...
  /**
   * Every command a Library row offers, for the context menu. Fluent documents this as the
   * required path — a toolbar or hover button is an accelerator on top of it, never the only
   * route — so these are the same handlers the detail surface and the selection bar already use,
   * reached a second way rather than reimplemented.
   */
  function contextCommands(item: ContentEntry, scope: 'row' | 'selection'): ContextMenuOption[] {
    ...
    return [
      // Only a video opens as an Editor source; audio and subtitle rows have no Editor surface.
      ...
      { type: 'divider' },
      {
        ...
        variant: 'destructive',
        isDisabled: disabled,
        // The same confirmation the detail surface gates this on; the menu is a new path to the
        // command, never a way around its confirmation.
        onClick: () =>
          void library.action(async () => {
              ...
              >
                {active && (
                  <>
                    ...
                    {/* SL-R05: a record's children stay under the parent. This disclosure
                        expands in place, inside the record — it never moves the user to a
                        separate assets destination (ticket 14). */}
