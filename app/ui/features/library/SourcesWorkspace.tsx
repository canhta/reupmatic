...
import { type AssetContent, LibraryAssetBrowser } from './assets/LibraryAssetBrowser';
...
export function SourcesWorkspace({ batchBusy, onBatch, onEditor }: Props) {
  ...
  return (
    <Section variant="transparent" padding={0} className="sources-workspace">
      ...
      >
        ...
        <div className="library-toolbar">
          ...
          />
          <Button
            label={t('libraryProcess', { count: selected.size })}
            isDisabled={disabled || batchBusy || !selected.size}
        ...
        {page && page.items.length > 0 && (
          <div className="library-grid" aria-busy={loading}>
            <div className="library-main">
              ...
              <div className="action-row library-pagination">
                ...
                <Button
                  ...
                  isDisabled={disabled || page.offset === 0}
                  onClick={() => library.changePage(page.offset - page.limit)}
                />
                <Button
                  label={t('libraryNext')}
                  isDisabled={disabled || page.offset + page.limit >= page.total}
                  onClick={() => library.changePage(page.offset + page.limit)}
                />
              </div>
            </div>
            <LibraryDetails
              ...
              onOpen={open}
              onAssets={(item) => {
                setAssetContent({ id: item.id, name: item.name });
                setTab('assets');
              }}
            />
          </div>
      ...
      >
        <EmptyState
          title={t('downloadsUnavailable')}
          description={t('downloadsUnavailableHelp')}
