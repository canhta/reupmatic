...
// ContentAssetView is a plain named type with no index signature, so it
// needs this wrapper per Table's own documented pattern.
...
/**
 * A content record's child assets. Scoped to exactly one `content` on purpose:
 * the former global, all-content mode was the flattened list SL-R05 forbids and
 * the IA study found in 0 of 8 references (ticket 14). Callers nest this in the
 * record's detail surface rather than navigating to it.
 */
export function LibraryAssetBrowser({ contentId, disabled, onOpen }: Props) {
  ...
  // Not memoized: assets.select/inspect close over per-render state, and
  // re-deriving four column defs each render is cheap.
