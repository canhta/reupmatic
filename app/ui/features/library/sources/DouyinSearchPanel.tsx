...
interface Props {
  ...
  connected: boolean;
  disabled: boolean;
  /** Opens the headed Douyin surface and resolves when the user closes it. */
  onReconnect(): Promise<unknown>;
  /** Opens the challenged page headed so the user solves the verification there, then resolves. */
  onVerify(): Promise<unknown>;
}

/**
 * Ticket 05's one Search action, inside the Downloads tab.
 *
 * A Douyin link (or a full share text, from which the link is extracted) is typed into the
 * always-visible field and searched; the clipboard is a one-click shortcut for the same action.
 * The result is the working list on the same surface — the exact item a video link pointed at,
 * then the channel and its listed videos — and the count / "more may be available" /
 * partial-result phrasing make it clear that this is what one scan retrieved, not whole-channel
 * coverage. Discovery downloads nothing.
 */
export function DouyinSearchPanel({ search, connected, disabled, onReconnect, onVerify }: Props) {
  ...
  // A search that returns a channel saves it host-side; pick that new channel up once the
  // outcome lands rather than making the user reload.
  useEffect(() => {
  ...
  /** Every candidate the current result set knows, so a selection id resolves to its item. */
  ...
  /**
   * SL-R04: this is the only path that starts a download, and it is an explicit user action on
   * the selection. Discovery has already finished; nothing here runs automatically. The chosen
   * tier is the best one the source offered for each candidate. `saveTo` is D-62's per-run
   * override — the default is the primary action; "Save to…" is the secondary one beside it.
   */
  ...
  // The text of the search that is running or last ran, so solving a verification challenge can
  // resume it instead of making the user paste the link again (SL-P01: return to the interrupted
  // operation).
  ...
  // Solving the challenge where the interrupted search ran (rather than on the home page) is what
  // lets the resumed search pass; the host remembers the captured response too.
  ...
  /**
   * SL-P01's return-to-the-interrupted-work for a download: open the challenged page, wait for the
   * user to clear it, then re-run only the items Douyin's verification stopped. The host has
   * remembered the captured item, so the retry reads it instead of visiting again.
   */
  ...
  // The one working list (SL-R02/ticket 05): the exact item the link pointed at comes first, then
  // the channel's videos, deduped by id because a channel listing normally includes the exact
  // item too. One list means one set of filters and one selection, exact item included.
  ...
  // A candidate Douyin's verification challenge stopped: the one action re-runs that page headed
  // and retries only those items, rather than making the user select and download again.
      ...
      <section className="downloads-search" aria-label={t('douyinSearchTitle')}>
        <VStack gap={2}>
          <HStack gap={2} vAlign="center">
            ...
            />
            {/* Icon-only and only when the clipboard actually holds text: with nothing to read
                the shortcut has no action to offer. */}
            {clipboard.hasText && (
              <IconButton
                variant="secondary"
