...
export function FolderRules({ snapshot, busy, onControl }: Props) {
  ...
  return (
    <Section variant="transparent" padding={0} className="folder-rules"
      ...
      {!snapshot.rules.length ? (
        ...
        <div className="folder-scroll">
          <Table density="compact" verticalAlign="top" aria-label={t('folderRules')}>
            ...
            <TableBody>{snapshot.rules.map(rule => (
              <TableRow key={rule.id} data-state={rule.state}>
                <TableCell>
                  <strong className="folder-path">{rule.source_name}</strong>
                  <span className="folder-path">{t('folderTo', { folder: rule.output_name })}</span>
                  {rule.processing && <small>{t(processingSummaryKey(rule.processing))}</small>}
                  <small>{t(rule.include_existing ? 'folderAllExisting' : 'folderNewOnly')}
                    {rule.recursive ? ` · ${t('folderRecursive')}` : ''}</small>
                </TableCell>
                <TableCell>
                  <span>{t(`folderState_${rule.state}`)}</span>
                  {rule.last_scan !== null && <small>{t('folderLastScan', {
                    time: new Date(rule.last_scan).toLocaleTimeString(i18n.language),
                  })}</small>}
                  {rule.error_code && <p className="inline-error">
                    {t(folderErrorKey(rule.error_code))} <code>{rule.error_code}</code>
                    {rule.error_file && <small>{rule.error_file}</small>}
                  </p>}
                </TableCell>
                <TableCell><span className="numeric">{rule.admitted}</span></TableCell>
                <TableCell>
                  <Button label={t(rule.state === 'paused' ? 'folderStart' : 'folderPause')}
                    isDisabled={busy || (rule.state === 'paused' && !snapshot.available)}
                    onClick={() => void onControl(rule.id, rule.state === 'paused' ? 'start' : 'pause')} />
                </TableCell>
              </TableRow>
            ))}</TableBody>
          </Table>
