...
// The decision lives beside the capability seam it questions, where it is asserted at the core
// boundary; the dialog keeps importing it from here.
export { problemCode };
...
interface Active {
  ...
  // The document the run belongs to. Captured per request, not per hook mount:
  // the provider this hook now lives in is long-lived across opening a video,
  // which changes the document id.
  documentId: string;
}
...
export function useSpeechJob(context: SpeechContext) {
  ...
  useEffect(() => {
    ...
    const off = window.reupmatic.onSpeechJob((message) => {
      ...
      // The run's own document, not this hook's mount-time one: the provider
      // outlives opening a video, and a result from a previous document must
      // not land in the current one.
      if (
  ...
  async function start(language: SpeechLanguage, scope: 'sample' | 'full', engineId: string) {
    ...
    try {
      ...
      // The dialog's own picker (ticket 05) already narrowed this to one job-scoped choice;
      // re-resolve it here rather than trusting a stale prop, the same defence the previous
      // automatic first-match had.
      const engine = models
