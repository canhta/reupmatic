...
export function useTranslationJob(context: Context) {
  ...
  useEffect(() => {
    ...
    const offModels = window.reupmatic.onTranslationModelsChanged(() => {
      void refresh();
    });
    void refresh();
    return () => {
      ...
      operation.current = null;
      if (request)
        void window.reupmatic.translationCancel(request.input.request_id).catch(() => undefined);
      if (configuring.current)
        void window.reupmatic.translationCancelSetup().catch(() => undefined);
    };
  }, [refresh, report]);
  ...
  async function configure() {
    ...
    setError('');
    try {
      await unwrap(window.reupmatic.translationConfigure());
    } catch (reason) {
      report(reason);
    } finally {
  ...
  async function start(options: Omit<TranslationOptions, 'request_id' | 'revision' | 'model_id'>) {
    ...
    try {
      ...
      if (
        models.source_language !== options.source_language ||
        models.target_language !== options.target_language
      ) {
        throw new Error('MODEL_LANGUAGE_UNAVAILABLE');
      }
      const input = prepareTranslation(c.snapshot, {
        ...options,
  ...
  return {
    ...
    cancel,
    report,
    consume: (captured: TranslationDraft) =>
      setDraft((value) => (value === captured ? null : value)),
    cancelSetup: async () => {
      try {
        await unwrap(window.reupmatic.translationCancelSetup());
      } catch (reason) {
        report(reason);
      }
    },
