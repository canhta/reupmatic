    ...
    >
      ...
      <p role="status">
        {job.checking
          ...
          : job.models?.available
            ? t('speechReady')
            : t(speechErrorKey(job.models?.code || 'MODEL_MISSING'))}
      </p>
      ...
      <div className="business-toolbar">
        <Selector
          ...
          onChange={(value) => {
            if (value === 'en' || value === 'vi' || value === 'zh') setLanguage(value);
          }}
        />
        <Selector
          label={t('speechScope')}
        ...
        <Button
          ...
          variant="primary"
          isDisabled={
            busy || editor.opening || !available || !media?.has_audio || Boolean(editor.composition)
          }
          ...
          {draft.data.cues.length === 0 ? (
            ...
            <>
              ...
              <Table density="compact" aria-label={t('speechDraft')}>
                ...
                <TableBody>
                  {draft.data.cues.slice(0, 25).map((cue, index) => (
                    <TableRow key={cue.id}>
                      <TableCell>
                        {cue.start_ms / 1000}–{cue.end_ms / 1000}
                      </TableCell>
                      <TableCell>
                        <span className="rule-comparison-text">{before[index]?.text ?? '—'}</span>
                      </TableCell>
                      <TableCell>
