...
import { parseSynthesisStatus, validateSynthesisResult, type SynthesisInput,
  type SynthesisResult, type SynthesisStatus } from '../../../../core/speech/synthesis/contracts';
...
interface Active { input: SynthesisInput; documentId: string; phase: string; fraction: number | null }
export interface SynthesisDraft { input: SynthesisInput; result: SynthesisResult; documentId: string }
...
export function useSynthesisJob(context: Context) {
  ...
  useEffect(() => {
    ...
    const offModels = window.reupmatic.onSynthesisModelsChanged(() => { void refresh(); });
    void refresh();
    return () => {
      alive.current = false; off(); offModels();
      const request = operation.current;
      operation.current = null;
      if (request) void window.reupmatic.synthesisCancel(request.input.request_id).catch(() => undefined);
  ...
  async function configure() {
    ...
    try { await unwrap(window.reupmatic.synthesisConfigure()); }
    catch (reason) { report(reason); }
    finally {
  ...
  async function start(options: Omit<SynthesisOptions, 'request_id' | 'revision' | 'model_id'>) {
    ...
    try {
      ...
      if (!models.languages.includes(options.language)) throw new Error('MODEL_LANGUAGE_UNAVAILABLE');
      if (!models.voices.some(voice => voice.id === options.voice_id)) throw new Error('SYNTHESIS_VOICE_UNAVAILABLE');
      const input = prepareSynthesis(c.snapshot, { ...options, request_id: requestId,
  ...
  return { models, checking, settingUp, active, draft, error, refresh, configure, start, cancel, report,
    cancelSetup: async () => {
      try { await unwrap(window.reupmatic.synthesisCancelSetup()); } catch (reason) { report(reason); }
    },
  };
}
