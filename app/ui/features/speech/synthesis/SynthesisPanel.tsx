...
export function SynthesisPanel() {
  ...
  const [language, setLanguage] = useState<SynthesisLanguage>(
    source.language === 'en' ? 'en' : 'vi',
  );
  ...
  return (
    <Section
      ...
      aria-label={t('synthesisTitle')}
    >
      <div className="action-row">
        <h2>{t('synthesisTitle')}</h2>
        <Button
      ...
      <p className="field-help">{t('synthesisLimits')}</p>
      <p role="status">
        {job.checking
          ? t('visionChecking')
          : job.models?.available
      ...
      <div className="business-toolbar">
        <Selector
          label={t('synthesisLanguage')}
          value={language}
          isDisabled={busy}
          options={(['vi', 'en'] as const).map((value) => ({
            value,
            label: t(`visionLanguage_${value}`),
          }))}
          onChange={(value) => {
            if (value === 'en' || value === 'vi') setLanguage(value);
          }}
        />
        <Selector
      ...
      {scope === 'selected' && !selectionValid && (
        <p role="status">{t('synthesisSelectionHelp')}</p>
      )}
