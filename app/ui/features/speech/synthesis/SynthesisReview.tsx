...
export function SynthesisReview({
  ...
  // Commits the exact draft and plan the user just reviewed to the project's voice track.
  // `applyVoiceResult` owns the freshness refusal: it re-checks the spoken token and text against
  // the current document, so a stale draft is surfaced here rather than pre-empted. The settings
  // are the neutral starting point the track ships with; the user tunes mode, level and fades
  // afterwards. Applying again replaces the current track, the same attach-or-replace the project
  // write does everywhere else.
  function apply() {
  ...
  const planned = new Map(plan.lines.map((entry) => [entry.cue_id, entry])),
    ...
    // How much of the line's own length the rate factor removed, as a percentage: `1.5` plays the
    // line 50% faster, which is 33.3% shorter, and the user acts on "this line is 33% shorter to
    // fit". `rate` is an exact ratio and is only rounded here for display; it is never stored back.
    ...
    <div className="business-form">
      ...
      <Text as="p" type="body">
        {t('synthesisCaptured', {
          ...
          // The engine a user recognises, never the raw build fingerprint or
          // the manifest's own model id (engine-name.ts).
