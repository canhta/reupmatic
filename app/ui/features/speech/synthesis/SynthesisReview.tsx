...
export function SynthesisReview({
  ...
  return (
    <div className="business-form">
      ...
      <p>
        {t('speechModelId')} <code>{p.model_id}</code>
      </p>
      ...
      <Table density="compact" aria-label={t('synthesisComparison')}>
        <TableHeader>
          <TableRow isHeaderRow>
            <TableHeaderCell>{t('synthesisWords')}</TableHeaderCell>
            <TableHeaderCell>{t('synthesisSourceTime')}</TableHeaderCell>
            <TableHeaderCell>{t('synthesisActualTime')}</TableHeaderCell>
            <TableHeaderCell>{t('synthesisDuration')}</TableHeaderCell>
          </TableRow>
        </TableHeader>
        <TableBody>
          {p.cues.slice(page * 25, (page + 1) * 25).map((cue, index) => {
            const span = result.segments[page * 25 + index];
            return (
              <TableRow key={cue.id}>
                <TableCell>
                  <span className="rule-comparison-text">{cue.text}</span>
                </TableCell>
                <TableCell>
                  {cue.start_ms / 1000}–{cue.end_ms / 1000}
                </TableCell>
                <TableCell>
                  {(span.start_frame / 48000).toFixed(3)}–{(span.end_frame / 48000).toFixed(3)}
                </TableCell>
                <TableCell>
                  {((span.end_frame - span.start_frame) / 48000).toFixed(3)} /{' '}
                  {((cue.end_ms - cue.start_ms) / 1000).toFixed(3)}
                </TableCell>
              </TableRow>
            );
          })}
      ...
      <div className="action-row">
        ...
        />
        <span>
          {t('translationPage', { page: page + 1, total: Math.ceil(p.cues.length / 25) })}
        </span>
        <Button
          label={t('translationNext')}
          isDisabled={(page + 1) * 25 >= p.cues.length}
          onClick={() => setPage(page + 1)}
        />
      </div>
      <Button
