...
/**
 * Speech recognition's setup form, resident in the Transcribe tool panel
 * (ED-P01, ticket 02): range, and language only when the Transcript layer has
 * none declared. Running it produces the review `CuePanel` renders in the cue
 * column — the job itself lives in `EditorGeneratorsProvider`, shared by both
 * surfaces, so a run started here is the draft reviewed there.
 */
export function SpeechSetup() {
  ...
  // What this job's picker actually offers: every entry the capability seam presents (via
  // presentableSpeechEngines, used unchanged below for the setup-vs-language-mismatch message),
  // narrowed to the declared language. Picking one of these is a per-job choice — it is never
  // written back to the transcript layer, the sample range or any persisted setting.
  ...
  return (
    <InspectorPanelSection title={t('speechTitle')}>
      {/* A real flex column with gap tokens: the engine line between the range
          select and the Run action used to be an inline `Text` whose line box
          bled into both neighbours at narrow widths and in Vietnamese. Every
          child is now a block-level flex item with its own gap. */}
      <Stack direction="vertical" gap={3}>
        ...
        {(job.checking || !available) && (
          <>
            ...
            {/* Acquisition is Settings-only (D-69): this panel names what is wrong and links
                there, instead of offering a second download of its own. */}
...
/**
 * The result of a speech run, rendered in the cue column until accepted or
 * discarded (ED-SUB-P01: a late result never overwrites manual edits without
 * an explicit review-and-apply).
 */
export function SpeechReview() {
  ...
  return (
    <div className="generator-review">
      ...
      {/* Summary and provenance are separate block rows; inline `Text` used to
