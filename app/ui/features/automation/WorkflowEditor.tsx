...
// Astryx Stepper collapses to a current-stage summary at narrow widths on its own.
export function WorkflowEditor({
  ...
  // This canvas mounts fresh each time it opens (it replaces the list rather
  // than toggling open like DetailSurface), so move focus in the same way
  // DetailSurface does for its own open transition — otherwise focus stays
  // on whatever invoked it (now unmounted) and Escape below never bubbles
  // through an unfocused subtree.
  useEffect(() => {
  ...
  return (
    // A confirmation <dialog> opened from this keydown is cancelled by Chromium in the same tick,
    // the same race DetailSurface guards against for its own Escape handler.
    <section
      ref={canvas}
      aria-label={title}
