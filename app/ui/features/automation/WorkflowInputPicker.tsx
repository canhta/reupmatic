...
export function WorkflowInputPicker({
  ...
  useEffect(() => {
    ...
    void unwrap(
      window.reupmatic.libraryList({
        ...
        offset,
        limit: PAGE_SIZE,
        // A workflow resolves a video source, so only video records are offered here: an audio
        // or subtitle item would be selectable and then fail admission.
        media_kinds: ['video'],
        ...toSortQuery(sort),
      }),
    )
  ...
  // Not memoized: onChange closes over per-render state, and re-deriving
  // two column defs each render is cheap.
