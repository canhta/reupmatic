...
export function WorkflowInputPicker({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    void unwrap(window.reupmatic.libraryList({ search, offset, limit: 25 }))
      .then((result) => {
    ...
    <div className="business-form">
      ...
      {page && page.items.length > 0 && (
        <div className="business-table">
          <Table density="compact" aria-label={t('workflowInputs')}>
            <TableHeader>
              <TableRow>
                ...
                <TableRow key={item.id}>
                  <TableCell>
                    <CheckboxInput
                      ...
                      isDisabled={
                        disabled || loading || (!value.includes(item.id) && value.length >= 100)
                      }
                      onChange={(checked) =>
                        onChange(
      ...
      <div className="business-pagination">
        <Button
          ...
          isDisabled={disabled || loading || offset === 0}
          onClick={() => setOffset(Math.max(0, offset - 25))}
        />
        <Button
          label={t('libraryNext')}
          isDisabled={disabled || loading || !page || offset + 25 >= page.total}
