...
// The editor replaces the list rather than docking beside it: its stages need the full width.
export function WorkflowManager({
  ...
  // The row/toolbar control that opened the editor unmounts along with the
  // rest of the list (the editor replaces it, unlike DetailSurface's docked
  // drawer), so a plain DOM ref would go stale. Track *which* control to
  // refocus by a stable data attribute instead, and look it up again once
  // the list has remounted.
  ...
  // Shares RunHistory's run-state function so both views derive an outcome identically.
  ...
  // Once the list has remounted, return focus to the control that opened
  // the editor — looked up fresh (see focusReturn's own comment above),
  // since a menu-triggered open (no in-page invoker) still resolves to the
  // toolbar's own Create action.
  useEffect(() => {
    if (editing) return;
  ...
  // Native Automation-menu "New Workflow" opens the same create
  // draft as the toolbar button below, gated the same way (isDisabled).
  useEffect(
    () =>
