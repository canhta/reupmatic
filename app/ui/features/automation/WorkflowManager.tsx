...
export function WorkflowManager({
  ...
  const workflows = (catalog.snapshot?.workflows ?? []).filter((item) =>
    ...
    <div className="business-workspace">
      ...
      <div className="business-grid">
        <div className="business-list">
          ...
          {!workflows.length ? (
            <EmptyState title={t('workflowsEmpty')} description={t('workflowsEmptyHelp')} />
          ) : (
            <div className="business-table">
              <Table density="compact" aria-label={t('workflowsTitle')}>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {workflows.slice(offset, offset + 25).map((workflow) => (
                    <TableRow key={workflow.id}>
                      <TableCell>
                        {workflow.name}
                        {workflow.archived && <p>{t('catalogArchived')}</p>}
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="action-row">
                          <Button
                            label={t('catalogEdit')}
                            isDisabled={disabled}
                            onClick={() => void draft.choose(draftOf(workflow))}
                          />
                          <Button
          ...
          <div className="business-pagination">
            ...
            <Button
              label={t('libraryNext')}
              isDisabled={offset + 25 >= workflows.length}
