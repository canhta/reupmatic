...
interface Draft { id: string; storedRevision: number; savedEditorRevision: number }
interface Status { key: string; updatedAt?: number; error?: string }

export function useAutosave(context: Context) {
  const latest = useRef(context);
  latest.current = context;
  const draft = useRef<Draft>({ id: context.documentId, storedRevision: 0, savedEditorRevision: -1 });
  ...
  useEffect(() => setStatus({ key: 'recoveryIdle' }), [context.documentId]);
  ...
  const flush = useCallback((): Promise<void> => {
    ...
    const write = async () => {
      ...
      try {
        const result = await unwrap(window.reupmatic.recoverySave({
          id: record.id, expected_revision: record.storedRevision, asset_id, snapshot,
        }));
        record.storedRevision = result.revision;
        record.savedEditorRevision = captured.revision;
        if (draft.current === record) setStatus({ updatedAt: result.updated_at,
          key: latest.current.revision === captured.revision ? 'recoverySaved' : 'recoveryWaiting' });
      } catch (error) {
        if (draft.current === record) setStatus({ key: 'recoveryFailed',
  ...
  const clearSaved = useCallback((revision: number): Promise<void> => {
    ...
    const remove = async () => {
      ...
      if (record.storedRevision > 0) {
        await unwrap(window.reupmatic.recoveryDiscard({ id: record.id, expected_revision: record.storedRevision }));
      }
  ...
  }, [context.documentId, context.revision, context.dirty, context.opening, context.media, flush]);

  useEffect(() => window.reupmatic.onRecoveryFlush(({ request_id }) => {
    void flush().then(() => window.reupmatic.recoveryFlushResult(request_id, true),
      () => window.reupmatic.recoveryFlushResult(request_id, false)).catch(() => undefined);
  }), [flush]);
