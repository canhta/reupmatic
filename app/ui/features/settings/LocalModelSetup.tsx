...
export function LocalModelSetup({ disabled = false, overridden = false }: {
  disabled?: boolean;
  overridden?: boolean;
}) {
  ...
  return <div className="local-model-setup">
    <p>{t('settingsModelsIntro')}</p>
    {overridden && <Banner status="warning" title={t('settingsModelsOverride')} />}
    <div className="action-row">
      <Button label={t('settingsChooseModels')} isDisabled={disabled || overridden || busy}
        onClick={() => void configure()} />
      {busy && <Button label={t(cancelling ? 'cancelling' : 'cancel')} isDisabled={cancelling}
        onClick={() => void cancel()} />}
    </div>
    {busy && <ProgressBar label={t('settingsModelsChecking')} isIndeterminate />}
    {saved && <p role="status">{t('settingsModelsSaved')}</p>}
    {error && <Banner status={error === 'CANCELLED' ? 'info' : 'error'}
      title={t(error === 'MODEL_CONFIG_OVERRIDE' ? 'settingsModelsOverride' : 'settingsModelsFailure')}
      description={<span>{t(visionErrorKey(error))} <code>{error}</code></span>} />}
