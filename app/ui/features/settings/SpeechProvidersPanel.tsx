...
/**
 * The BYOK/BYO-model registry surface (D-55, ticket 08): several hosted providers, several
 * models under each, added/edited/removed here without a code change. Panel content only — no
 * window lifecycle assumption (D-57 removes the Settings window; ticket 10 rehosts this panel).
 * A credential is entered here and never displayed again: the list only ever reports whether one
 * is currently set, via `has_credential`, and offers replace/remove rather than showing it.
 */
export function SpeechProvidersPanel() {
  ...
  // A provider can only be added once some protocol is implemented in code. With none, an Add
  // affordance could never succeed, so the section is absent rather than a door that refuses —
  // and carries no copy explaining why. Already-stored providers still render, so
