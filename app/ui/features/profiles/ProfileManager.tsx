...
export function ProfileManager({ currentRecipe }: { currentRecipe?: ProcessingRecipe }) {
  ...
  const mediaSpecific =
    value.processing?.inpaint?.target === 'manual' || Boolean(value.processing?.editing?.trim);
  const profiles = (catalog.snapshot?.profiles ?? []).filter((item) =>
        ...
        <div className="business-list">
          <div className="business-toolbar">
            ...
            <Button
              label={t('profileNew')}
              isDisabled={disabled}
              onClick={() => void draft.choose(draftOf())}
            />
            <Button
          ...
          {!profiles.length ? (
            <EmptyState title={t('profilesEmpty')} description={t('profilesEmptyHelp')} />
          ) : (
            <div className="business-table">
              <Table density="compact" aria-label={t('profilesTitle')}>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {profiles.slice(offset, offset + 25).map((profile) => (
                    <TableRow key={profile.id}>
                      <TableCell>
                        {profile.name}
                        {profile.archived && <p>{t('catalogArchived')}</p>}
                      </TableCell>
                      <TableCell>
                        <div className="action-row">
                          <Button
                            label={t('catalogEdit')}
                            isDisabled={disabled}
                            onClick={() => void draft.choose(draftOf(profile))}
                          />
                          <Button
          ...
          <div className="business-pagination">
            ...
            <Button
              label={t('libraryNext')}
              isDisabled={offset + 25 >= profiles.length}
        ...
        <div className="business-form">
          ...
          <Button
            ...
            onClick={() => {
              ...
              draft.setValue({
                ...value,
                processing: processing && Object.keys(processing).length > 1 ? processing : null,
              });
