...
export function VisionPanel(props: Props) {
  ...
  return (
    ...
    >
      ...
      {missing && (
        <Banner status="warning" title={t('visionMissing')} description={t('visionSetup')} />
      )}
      ...
      <div className="ocr-controls">
        ...
        <Button
          label={t('visionOcr')}
          variant="primary"
          isDisabled={busy || !hasOcr}
          onClick={() =>
            void job.start('media.ocr', { language, sample_ms: sample, min_confidence: confidence })
          }
        />
        <Button
      ...
      <InpaintControls
        ...
        hasOcr={hasOcr}
        onStart={(settings) =>
          job.start('media.inpaint', {
            ...settings,
            ...(settings.target === 'text' ? { language } : {}),
          })
        }
      />
      {job.active && (
