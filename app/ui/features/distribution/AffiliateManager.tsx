...
function draftOf(record?: AffiliateLink): SaveAffiliate {
  return { id: record?.id ?? crypto.randomUUID(), expected_revision: record?.revision ?? null,
    name: record?.name ?? '', url: record?.url ?? '', label_ids: record?.label_ids ?? [], archived: record?.archived ?? false };
}
...
export function AffiliateManager({ onPosts }: { onPosts(id: string): void }) {
  ...
  const rows = (catalog.snapshot?.links ?? []).filter(link => `${link.name} ${link.url}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <div className="business-grid">
    <div className="business-list">
      <div className="business-toolbar">
        ...
        <Button label={t('affiliateNew')} isDisabled={disabled} onClick={() => void form.choose(draftOf())} />
      </div>
      <p>{t('affiliateUsageHelp')}</p>
      {!rows.length ? <EmptyState title={t('affiliateEmpty')} description={t('affiliateEmptyHelp')} /> :
        <Table density="compact" aria-label={t('affiliateTab')}>
          ...
          <TableBody>{rows.slice(offset, offset + 25).map(link => <TableRow key={link.id}>
            <TableCell>{link.name}{link.archived && <p>{t('catalogArchived')}</p>}</TableCell>
            <TableCell>{catalog.snapshot?.link_usage[link.id] ?? 0}</TableCell>
            <TableCell><div className="action-row">
              <Button label={t('catalogEdit')} isDisabled={disabled} onClick={() => void form.choose(draftOf(link))} />
    ...
    <div className="business-form">
      ...
      <FormLayout>
        ...
        <TextInput label={t('affiliateUrl')} value={draft.url} isDisabled={disabled}
          onChange={url => setDraft({ ...draft, url })} />
      </FormLayout>
      <p className="field-help">{t('affiliateUrlHelp')}</p>
      <LabelPicker value={draft.label_ids} disabled={disabled} onChange={label_ids => setDraft({ ...draft, label_ids })} />
      ...
      <div className="action-row">
        <Button label={t('catalogSave')} variant="primary" isDisabled={disabled || !draft.name.trim() || !draft.url || (!dirty && draft.expected_revision !== null)}
          onClick={() => void save()} />
