...
export function ChannelsWorkspace() {
  ...
  function posts(next: PostFilter) { setFilter(next); setTab('posts'); }
  return <div className="business-workspace">
    <TabList value={tab} onChange={setTab} role="tablist" aria-label={t('channels')}>
      ...
      <Tab value="affiliate" label={t('affiliateTab')} id="affiliate-tab" panelId="affiliate-panel" />
      <Tab value="posts" label={t('postsTab')} id="posts-tab" panelId="posts-panel" />
    </TabList>
    <div role="tabpanel" id="channels-panel" aria-labelledby="channels-tab" hidden={tab !== 'channels'}>
      <ChannelManager onPosts={(channel_id, view) => posts({ channel_id, view })} />
    </div>
    <div role="tabpanel" id="affiliate-panel" aria-labelledby="affiliate-tab" hidden={tab !== 'affiliate'}>
