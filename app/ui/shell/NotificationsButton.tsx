...
/**
 * The status bar's notification centre: a bell carrying the unread count, opening the session
 * list newest-first. `JobsButton` is the shape it follows — a right-side status-bar control that
 * opens its own surface — but its surface is an Astryx `Popover`, which already owns focus
 * trapping, light dismiss and Escape, so this control draws no surface of its own. Opening marks
 * what the list shows as read; the badge is the only count that still needs attention.
 */
    ...
    <Popover
      ...
      content={
        <Stack direction="vertical" gap={2}>
          {/* The header's own close control takes focus on open, so the first Escape dismisses
              the list itself rather than a timestamp's hover card underneath it. */}
