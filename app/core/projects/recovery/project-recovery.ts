...
/** Durable current-format editor drafts. The SQLite schema and optimistic
 * revision transaction stay behind this project-recovery interface.
 */
export class ProjectRecovery {
  ...
  save(
    ...
    try {
      ...
      if (
        !row &&
        Number(this.db.prepare('SELECT count(*) AS count FROM editor_drafts').get()?.count) >=
          MAX_DRAFTS
      ) {
      ...
      const revision = expectedRevision + 1,
        updated_at = Date.now();
      this.db
        .prepare(`INSERT INTO editor_drafts VALUES(?,?,?,?,?)
        ON CONFLICT(id) DO UPDATE SET revision=excluded.revision, updated_at=excluded.updated_at,
        document=excluded.document`)
        .run(id, revision, updated_at, project.source.sha256, document);
      this.db.exec('COMMIT');
