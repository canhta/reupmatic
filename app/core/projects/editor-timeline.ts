...
/** Native media is known at open/save time, not while editing an incomplete draft. */
export function validateProjectTimeline(snapshot: EditorSnapshot, sourceDuration: number): void {
  const duration = snapshot.composition ? compositionDuration(snapshot.composition) : sourceDuration;
  if (snapshot.sample.end_ms > duration) throw new Error('INVALID_PROJECT');
  resolveEditWindow(snapshot.processing?.editing, duration);
  if (snapshot.composition && textLayerNames.some(name =>
    getTextLayer(snapshot, name).cues.some(cue => cue.end_ms > duration))) throw new Error('INVALID_CUES');
}
