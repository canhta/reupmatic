...
import { parseProcessingRecipe, type ProcessingRecipe } from '../processing/recipe.js';
...
export function assertEditorSnapshot(value: unknown): asserts value is EditorSnapshot {
  if (!object(value) || !onlyKeys(value, ['cues', 'sample', ...('processing' in value ? ['processing'] : []), ...('soundtrack' in value ? ['soundtrack'] : []), ...('composition' in value ? ['composition'] : []), ...('text_layers' in value ? ['text_layers'] : [])])) throw new Error('INVALID_PROJECT');
  ...
  if ('processing' in value) {
    try { parseProcessingRecipe(value.processing, value.cues.length > 0); }
    catch { throw new Error('INVALID_PROJECT'); }
  }
  const sample = value.sample;
  if (!object(sample) || !onlyKeys(sample, ['start_ms', 'end_ms'])
    || !Number.isInteger(sample.start_ms) || !Number.isInteger(sample.end_ms)
    || Number(sample.start_ms) < 0 || Number(sample.end_ms) <= Number(sample.start_ms)
    || Number(sample.end_ms) > 86400000) throw new Error('INVALID_PROJECT');
...
export function parseProject(value: unknown): ProjectFile {
  ...
  if (!onlyKeys(value, ['format', 'version', 'source', 'cues', 'sample', ...('processing' in value ? ['processing'] : []), ...('soundtrack' in value ? ['soundtrack'] : []), ...('composition' in value ? ['composition'] : []), ...('text_layers' in value ? ['text_layers'] : [])])) {
    throw new Error('INVALID_PROJECT');
  }
  const source = value.source;
  if (!object(source) || !onlyKeys(source, ['path', 'sha256'])
    || typeof source.path !== 'string' || !source.path || source.path.length > 4096
    || source.path.includes('\0') || /^[a-z]+:\/\//i.test(source.path)
    || typeof source.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(source.sha256)) {
    throw new Error('INVALID_PROJECT');
  }
  assertEditorSnapshot({ cues: value.cues, sample: value.sample,
...
export async function saveProject(
  filename: string, project: ProjectFile, protectedSources: Iterable<string> = [],
): Promise<void> {
  ...
  const sources = [...protectedSources, valid.source.path, ...(valid.soundtrack ? [valid.soundtrack.source.path] : []), ...(valid.composition?.clips.map(clip => clip.source.path) ?? [])];
