...
export interface EditorSnapshot {
  /** The project's display name. Optional in the live document (an untouched
   *  untitled project has none; the header shows its own localized placeholder);
   *  a saved project always carries one (parseProject requires it). */
  name?: string;
...
export interface ProjectFile extends EditorSnapshot {
  ...
  /** A saved project always has one; the live snapshot may not. */
  name: string;
...
/** The default project name: the source file's own basename, without extension. */
...
/** A project always carries a name; the caller may supply one (the Editor's own
 *  session does, from the first video or an in-place rename). A snapshot without
 *  one is named after the source file, so no reader ever fills in a default. */
