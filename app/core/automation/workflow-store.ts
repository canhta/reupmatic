...
import { validateBatchInput } from '../batch/batch-input.js';
...
export class WorkflowStore {
  ...
  list(): Workflow[] {
    return this.db.list<WorkflowData>('workflow');
  }
  get(id: string): Workflow {
    return this.db.require<WorkflowData>('workflow', id);
  }

  save(input: unknown, outputDir: string): Workflow {
    const value = object(
      input,
      ['id', 'expected_revision', 'name', 'item_ids', 'processing', 'archived'],
      ['output_id'],
    );
    if (!path.isAbsolute(outputDir) || outputDir.includes('\0') || outputDir.length > 32768)
    ...
    return this.db.save('workflow', identifier(value.id), revision(value.expected_revision), {
      ...
      item_ids,
      processing: value.processing === null ? null : parseReusableRecipe(value.processing),
      output_dir: outputDir,
      archived: boolean(value.archived),
    });
  }

  findRun(id: string): WorkflowRun | null {
    return this.db.find<WorkflowRunData>('run', id);
  }

  runs(): WorkflowRunView[] {
    return this.db.list<WorkflowRunData>('run').map(({ inputs, ...run }) => ({
      ...run,
      input_names: inputs.map((input) => input.video.name),
      output_dir: inputs[0]?.output_dir ?? '',
    }));
  }
  ...
  prepare(id: string, workflow: Workflow, inputs: BatchJobInput[]): WorkflowRun {
    ...
    if (
      inputs.length !== workflow.item_ids.length ||
      inputs.some((input, index) => input.library_id !== workflow.item_ids[index])
    )
      throw new Error('INVALID_REQUEST');
  ...
  admitted(id: string, jobs: string[]): WorkflowRun {
    ...
    if (current.admission === 'admitted') {
      if (JSON.stringify(current.job_ids) !== JSON.stringify(jobs))
        throw new Error('DUPLICATE_REQUEST');
      return current;
    }
    const { id: _id, revision: rev, created_at: _created, updated_at: _updated, ...data } = current;
    return this.db.save<WorkflowRunData>('run', id, rev, {
      ...data,
