...
/**
 * A recipe reused away from its project — batch, folders, workflows. The logo's
 * placement names a project media image that never travels with a reusable recipe
 * (D-63), so carrying one is a loud error rather than a silent no-op render.
 */
...
/** Content equality, independent of key order or object identity — the header's "· modified"
 * label (§5.8) compares the live recipe against the one an applied profile last set, so undoing
 * back to that exact content clears it instead of a revision counter staying "modified" forever. */
