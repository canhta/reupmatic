...
function validateSource(
  ...
  if (layer.language !== null && layer.language !== language)
...
export function previewTranslation(
  ...
  policy: TranslationPolicy = 'keep-existing',
): TranslationPreview {
  const input = parseTranslationInput(value),
    result = validateTranslationResult(response, input),
    p = input.params;
  ...
  if (source.token !== p.source_token || !sameText(source.cues, p.cues))
    throw new Error('STALE_OPERATION');
  const target = getTextLayer(snapshot, 'translated'),
    keep = policy === 'keep-existing';
  if (keep && target.cues.length && target.language !== p.target_language)
    throw new Error('TRANSLATION_TARGET_LANGUAGE_MISMATCH');
  const existing = new Set(target.cues.map((cue) => cue.id)),
    incoming = new Set(result.cues.map((cue) => cue.id));
  const additions = result.cues.filter((cue) => !existing.has(cue.id));
  const cues = keep
    ? [...target.cues, ...additions].sort((a, b) => a.start_ms - b.start_ms)
    : result.cues;
  return structuredClone({
    ...
    before: target.cues,
    cues,
    kept: keep ? target.cues.length : 0,
...
export function applyTranslation<T extends TextSnapshot>(
  snapshot: T,
  preview: TranslationPreview,
): T {
  let expected: TranslationPreview;
  try {
    expected = previewTranslation(snapshot, preview.input, preview.result, preview.policy);
  } catch {
    throw new Error('STALE_OPERATION');
  }
  if (JSON.stringify(expected) !== JSON.stringify(preview)) throw new Error('STALE_OPERATION');
  const p = preview.input.params;
  const next = editTextLayer(snapshot, 'translated', preview.cues, {
    ...
    origin: {
      ...
      model_id: p.model_id,
      runtime: preview.result.runtime,
      rules: p.rules,
  ...
  if (preview.kept) {
    ...
    next.text_layers.translated.edited = true;
  }
  return next;
}
