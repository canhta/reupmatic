...
export function validateSynthesisResult(value: unknown, input: SynthesisInput): SynthesisResult {
  ...
  if (
    ...
    !integer(value.frames, 1, 48000 * 600) ||
    value.duration_ms !== Math.ceil(value.frames / 48) ||
    value.timing !== 'natural-sequential' ||
    value.gap_ms !== 250 ||
  ...
  for (const [index, segment] of value.segments.entries()) {
    if (
      !object(segment) ||
      !exact(segment, ['cue_id', 'start_frame', 'end_frame']) ||
      segment.cue_id !== input.params.cues[index].id ||
      segment.start_frame !== nextStart ||
      !integer(segment.end_frame, nextStart + 1, Math.min(nextStart + 48000 * 60 - 1, value.frames))
    )
      return fail();
...
export function parseSynthesisStatus(value: unknown): SynthesisStatus {
  if (
    !object(value) ||
    !exact(value, ['available', 'code', 'model_id', 'languages', 'voices', 'verified']) ||
