...
/** Rates this build writes and reads as canonical mono PCM16. The producer reports its own. */
export const SUPPORTED_SAMPLE_RATES = [16000, 22050, 24000, 32000, 44100, 48000] as const;
export interface SynthesisStatus {
  available: boolean;
  code: string | null;
  model_id: string | null;
  /** The synthesis architecture the configured bundle names. The voice timing plan consults this
   *  capability rather than assuming which engine produced a job's audio. */
  engine: string | null;
...
/** The parameter keys a persisted receipt carries; a reopened artifact reconstructs the input
 *  correlation from these alone, since a receipt is the params plus the producer's own facts. */
...
export function validateSynthesisResult(value: unknown, input: SynthesisInput): SynthesisResult {
  ...
  if (
    ...
    return fail();
  // A well-formed rate this build cannot produce is its own refusal, not a generic malformed
  // response: the producer is telling the truth about a rate we do not support.
