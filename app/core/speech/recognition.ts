...
/** A word-level timing from a companion forced aligner. It references its cue by
 * `cue_id` rather than nesting under the cue itself, so `SpeechResult.cues` stays
 * exactly `Cue[]` — the narrow shape every text layer (displayed/translated/spoken)
 * trusts via `assertCues` — whether or not an aligner ran. */
...
export interface SpeechResult extends Record<string, unknown> {
  ...
  timing: 'segment';
  cues: Cue[];
  /** Always present, empty when no aligner ran. Ordered per cue and within its
   * cue's bounds; never changes cue count, cue boundaries or recognised words. */
  words: WordTiming[];
  /** Identity of the configured aligner bundle that produced `words`; non-null
   * exactly when `words` is nonempty. */
...
/** Replaces the previous single-engine status object: several engines can be
 * configured at once, so the worker reports one entry per known engine rather
