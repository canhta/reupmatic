...
export interface SpeechInput { request_id: string; revision: number; params: SpeechParams }
...
export function parseSpeechInput(value: unknown): SpeechInput {
  if (!object(value) || !exact(value, ['request_id', 'revision', 'params'])
    || typeof value.request_id !== 'string' || !/^[a-zA-Z0-9_-]{8,128}$/.test(value.request_id)
    || !integer(value.revision, 0, 2 ** 31 - 1) || !object(value.params)) {
    throw new RemoteError('INVALID_REQUEST');
  }
  const p = value.params;
  if (!exact(p, ['asset_id', 'start_ms', 'end_ms', 'language', 'model_id'])
    || typeof p.asset_id !== 'string' || !p.asset_id || p.asset_id.length > 128 || p.asset_id.includes('\0')
    || !integer(p.start_ms, 0, 86400000) || !integer(p.end_ms, 1, 86400000)
    || Number(p.end_ms) <= Number(p.start_ms) || typeof p.language !== 'string'
    || !['en', 'vi', 'zh'].includes(p.language)
    || !hash(p.model_id)) throw new RemoteError('INVALID_REQUEST');
...
export function validateSpeechResult(value: unknown, input: SpeechInput, sourceHash: string): SpeechResult {
  if (!object(value) || !exact(value, ['kind', 'asset_id', 'source_sha256', 'start_ms', 'end_ms',
    'language', 'model_id', 'runtime', 'clock', 'timing', 'cues'])
    || value.kind !== 'stt' || value.clock !== 'source' || value.timing !== 'segment'
    || value.asset_id !== input.params.asset_id || value.source_sha256 !== sourceHash || !hash(sourceHash)
    || value.model_id !== input.params.model_id || value.language !== input.params.language
  ...
  for (const cue of value.cues) {
    if ('style' in cue || !cue.text.trim() || cue.start_ms < previous || cue.end_ms > input.params.end_ms) {
      throw new RemoteError('INVALID_WORKER_RESPONSE');
    }
    previous = cue.end_ms;
  }
...
export function parseSpeechStatus(value: unknown): SpeechStatus {
  if (!object(value) || !exact(value, ['available', 'code', 'model_id', 'languages', 'verified'])
    || typeof value.available !== 'boolean' || typeof value.verified !== 'boolean'
    ...
    || value.languages.length > 3 || new Set(value.languages).size !== value.languages.length
  ...
  return structuredClone(value) as unknown as SpeechStatus;
}
