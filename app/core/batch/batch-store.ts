...
import { parseModelFingerprints, parseProcessingRecipe } from '../processing/recipe.js';
...
export class BatchStore {
  ...
  enqueue(requestId: string, inputs: BatchJobInput[]): BatchJob[] {
    ...
    const snapshots = inputs.map((input) => ({
      ...
      ...(input.processing
        ? {
            processing: parseProcessingRecipe(input.processing),
  ...
  finish(
    ...
    if (
      state === 'complete' &&
      (!output || !SHA.test(output.sha256) || !path.isAbsolute(output.path))
    )
