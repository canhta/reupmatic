...
import type { BatchJob, BatchJobInput, BatchOutput, BatchState } from './batch-contracts.js';
import { isSha256, validateBatchInput } from './batch-input.js';

const MAX_JOBS = 2000;

/** One coordinator owns this journal; media never enters SQLite. */
export class BatchStore {
  ...
  constructor(filename: string) {
    this.db = openCurrentDatabase(
      filename,
      {
        tables: ['batch_groups', 'batch_jobs'],
        sql: `
  ...
  list(): BatchJob[] {
    return this.db
      .prepare('SELECT * FROM batch_jobs ORDER BY created_at, rowid')
      .all()
  ...
  findRequest(requestId: string): BatchJob[] {
    return this.db
      .prepare('SELECT * FROM batch_jobs WHERE batch_id=? ORDER BY position')
      .all(requestId)
      .map((row) => this.decode(row));
  }
  enqueue(requestId: string, inputs: BatchJobInput[]): BatchJob[] {
    ...
    const snapshots = inputs.map((input) => ({
      ...
      ...(input.subtitle
        ? {
            subtitle: {
              ...
              sha256: input.subtitle.sha256,
              name: input.subtitle.name,
            },
          }
        : {}),
      ...
      ...(input.processing
        ? {
            processing: parseReusableRecipe(input.processing),
    ...
    return this.transaction(() => {
      const existing = this.db
        .prepare('SELECT signature FROM batch_groups WHERE id=?')
        .get(requestId);
      if (existing) {
      ...
      snapshots.forEach((input, position) => {
        insert.run(randomUUID(), requestId, position, JSON.stringify(input), now, now);
      });
      return this.db
        .prepare('SELECT * FROM batch_jobs WHERE batch_id=? ORDER BY position')
        .all(requestId)
  ...
  recover(): number {
    return this.transaction(() => {
      ...
      const interrupted = this.db
        .prepare(
          "UPDATE batch_jobs SET state='interrupted',error_code='INTERRUPTED',updated_at=? WHERE state='running'",
        )
        .run(now);
      const cancelled = this.db
        .prepare(
          "UPDATE batch_jobs SET state='cancelled',error_code='CANCELLED',updated_at=? WHERE state='cancelling'",
        )
        .run(now);
  ...
  claim(): BatchJob | null {
    return this.transaction(() => {
      const row = this.db
        .prepare("SELECT id FROM batch_jobs WHERE state='queued' ORDER BY created_at,rowid LIMIT 1")
        .get();
      if (!row) return null;
      this.db
        .prepare(
          "UPDATE batch_jobs SET state='running',attempt=attempt+1,error_code=NULL,updated_at=? WHERE id=? AND state='queued'",
        )
        .run(Date.now(), row.id);
  ...
  finish(
    id: string,
    state: 'complete' | 'failed' | 'cancelled' | 'interrupted',
    error: string | null = null,
    output: BatchOutput | null = null,
  ): void {
    if (
      state === 'complete' &&
      (!output || !isSha256(output.sha256) || !path.isAbsolute(output.path))
    )
      throw new RemoteError('INVALID_WORKER_RESPONSE');
    const result = this.db
      .prepare(`UPDATE batch_jobs SET state=?,error_code=?,output_json=?,updated_at=?
      WHERE id=? AND state IN ('running','cancelling')`)
      .run(state, error, output ? JSON.stringify(output) : null, Date.now(), id);
  ...
  cancel(id: string): BatchJob {
    ...
    if (job.state === 'queued' || job.state === 'interrupted') {
      this.db
        .prepare(
          "UPDATE batch_jobs SET state='cancelled',error_code='CANCELLED',updated_at=? WHERE id=?",
        )
        .run(Date.now(), id);
    } else if (job.state === 'running') {
      this.db
        .prepare("UPDATE batch_jobs SET state='cancelling',updated_at=? WHERE id=?")
        .run(Date.now(), id);
    }
    return this.get(id);
  }
  retry(id: string): void {
    const result = this.db
      .prepare(`UPDATE batch_jobs SET state='queued',error_code=NULL,updated_at=?
