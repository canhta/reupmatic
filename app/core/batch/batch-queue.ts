...
export class BatchQueue extends EventEmitter {
  ...
  constructor(private readonly store: BatchStore, private readonly worker: WorkerPort, private readonly renderer: RenderPort) {
    super();
  ...
  get pendingCount(): number { return this.store.list().filter(job => ['queued', 'running', 'cancelling', 'interrupted'].includes(job.state)).length; }
  get(id: string): BatchJob { return this.store.get(id); }
  snapshot(): BatchSnapshot {
    return {
      version: this.version, paused: this.paused, active_id: this.active?.job.id ?? null,
      recovered: this.recovered, fault: this.fault,
      items: this.store.list().map(job => ({
        id: job.id, batch_id: job.batch_id, name: job.input.video.name,
        ...(job.input.processing ? { processing: job.input.processing } : {}),
        subtitle_name: job.input.subtitle?.name ?? null, state: job.state, attempt: job.attempt,
        error_code: job.error_code, output_name: job.output ? path.basename(job.output.path) : null,
  ...
  enqueue(requestId: string, inputs: BatchJobInput[]): BatchSnapshot {
    ...
    this.changed(); this.wake(); return this.snapshot();
  }
  ...
  admitFolder(requestId: string, input: BatchJobInput): string {
    ...
    if (existing.length) {
      ...
      if (saved.input.video.sha256 !== input.video.sha256 || saved.input.output_dir !== input.output_dir
        || saved.input.encoding !== input.encoding || saved.input.subtitle
        || JSON.stringify(saved.input.processing) !== JSON.stringify(input.processing)
        || JSON.stringify(saved.input.processing_models) !== JSON.stringify(input.processing_models)) throw new RemoteError('DUPLICATE_REQUEST');
      return saved.id;
    }
    const job = this.store.enqueue(requestId, [input])[0];
    this.changed(); this.wake(); return job.id;
  }
  outputPaths(): string[] {
    return this.store.list().map(job => path.join(job.input.output_dir, outputFilename(job.input.video.name, job.id)));
  }
  pause(): BatchSnapshot { this.paused = true; this.changed(); return this.snapshot(); }
  resume(): BatchSnapshot {
    if (this.closing) throw new RemoteError('QUEUE_CLOSED');
    this.fault = null; this.paused = false; this.changed(); this.wake(); return this.snapshot();
  }
  retry(id: string): BatchSnapshot {
    if (this.closing) throw new RemoteError('QUEUE_CLOSED');
    this.store.retry(id); this.changed(); this.wake(); return this.snapshot();
  }
  ...
  async finishClose(): Promise<void> {
    try { await this.active?.done; }
    finally {
      this.renderer.off('job', this.onProgress); this.renderer.off('idle', this.wake);
      this.store.close();
    }
  }
  ...
  private readonly onProgress = (event: Envelope): void => {
    ...
    const phase = typeof event.data.phase === 'string' && ['queued', 'running', 'rendering', 'processingModels', 'processingOcr', 'processingInpaint',
      'processingJoining', 'processingEncoding', 'processingVerifying'].includes(event.data.phase) ? event.data.phase : 'running';
    const fraction = typeof event.data.fraction === 'number' && Number.isFinite(event.data.fraction)
      ? Math.min(1, Math.max(0, event.data.fraction)) : null;
    this.active.progress = { phase, fraction }; this.changed();
  };
  private readonly wake = (): void => {
    ...
    setImmediate(() => {
      ...
      try {
        ...
        if (!job) { this.changed(); return; }
        const active: Active = { job, cancelRequested: false, progress: { phase: 'preparing', fraction: null } };
        this.active = active; this.changed();
        active.done = this.execute(active);
        void active.done.catch(error => { this.fault = code(error); this.paused = true; this.emit('fault', this.fault); try { this.changed(); } catch { /* Preserve the journal for diagnosis. */ } });
      } catch (error) { this.fault = code(error); this.paused = true; this.changed(); }
    });
  };
  ...
  private async register(active: Active, kind: 'video' | 'subtitle') {
    this.checkActive(active);
    const identity = kind === 'video' ? active.job.input.video : active.job.input.subtitle!;
  ...
  private async execute(active: Active): Promise<void> {
    const { job } = active;
    try {
      const videoId = await this.register(active, 'video');
      const subtitleId = job.input.subtitle ? await this.register(active, 'subtitle') : undefined;
      this.checkActive(active);
      // Fresh operation ID for each attempt; persistent job ID/input remain stable.
      const ticket = this.renderer.start({
        request_id: randomUUID(), asset_id: videoId, cues: [], revision: job.attempt,
        mode: 'full', encoding: job.input.encoding,
        ...(job.input.processing ? { processing: job.input.processing } : {}),
      }, subtitleId, job.input.processing_models);
      active.step = ticket; active.progress = { phase: 'rendering', fraction: null }; this.changed();
      const rendered = await ticket.result;
      this.checkActive(active);
      active.step = undefined; active.progress = { phase: 'exporting', fraction: null }; this.changed();
      const digest = await hashFile(rendered.path);
      if (rendered.sha256 !== digest) throw new RemoteError('OUTPUT_CHANGED');
      const destination = await publishBatchOutput(rendered.path, job.input.output_dir,
        outputFilename(job.input.video.name, job.id), digest, () => this.checkActive(active));
      this.store.finish(job.id, 'complete', null, {
        path: destination, sha256: digest, duration_ms: rendered.duration_ms, cache_hit: rendered.cache_hit,
      });
    } catch (error) {
      const reason = active.cancelRequested ? 'CANCELLED' : this.closing ? 'INTERRUPTED' : code(error);
      this.store.finish(job.id, reason === 'CANCELLED' ? 'cancelled' : reason === 'INTERRUPTED' ? 'interrupted' : 'failed', reason);
      ...
      if (['WORKER_EXITED', 'WORKER_START_FAILED', 'INVALID_WORKER_RESPONSE'].includes(reason)) {
        this.fault = reason; this.paused = true;
      }
    } finally {
      this.active = undefined; this.changed(); this.wake();
    }
  }
}
