...
export interface RenderInput {
  ...
  voice?: VoiceTrack;
  /** The project media image the editing recipe's logo placement draws; project-specific
   *  media the coordinator registers for the worker, never part of a profile. */
  logo?: ProjectMedia;
}
...
export class RenderCoordinator extends EventEmitter {
  ...
  constructor(
    ...
    /** Resolves the project's voice artifact through the same inspection the save dialog uses,
     *  returning the path the worker may register. Absent when this session has no synthesis. */
