...
export class LibraryStore {
  ...
  constructor(filename: string) {
    this.db = openCurrentDatabase(
      filename,
      {
        ...
        -- The Douyin intake projection beside its untouched raw payload, one row per item. Both
        -- are JSON: the projection is the typed, queryable-by-decode shape, the raw is the one
        -- retained copy a later facet can be read from without a second fetch.
  ...
  /**
   * Identity for a connector item is its `aweme_id`, never a filename (SL-R07). A re-run that finds
   * the item here reuses the held copy instead of re-downloading; filename equality is never
   * consulted, so two distinct items that happen to share a name stay distinct and a renamed item
  ...
  /**
   * Persists the Douyin intake projection beside its untouched raw payload. The item must be a
   * Douyin-origin record, and the projection must pass `assertDouyinIntake` — notably a counter
   * cannot be written without the instant it was captured. One row per item: the raw payload is
   * retained exactly once, and re-intake replaces it rather than accumulating copies.
   */
  ...
  /**
   * `contentIds`, when supplied, restricts the page to those ids — the Library resolves a label
   * facet through `app/core/taxonomy` and passes the matching ids here, so this store never reads
   * a second owner's tables. An empty array is a real empty result, not "no filter".
   */
  listContent(query: ContentStoreQuery, contentIds?: readonly string[]): ContentPage {
    // Whitelist: sort_by must never reach the SQL string by any path other
    // than this exact column lookup (never interpolate the raw value). Record<ContentSortKey, …>
    // keeps this map exhaustive against CONTENT_SORT_KEYS at compile time.
    ...
    // `label_ids` is deliberately absent: it belongs to the taxonomy, and `ContentLibrary`
    // resolves it before this call, so passing it here must fail rather than filter nothing.
    const allowedKeys = [
      'search',
      'offset',
      'limit',
    ...
    // Sort applies to the whole matching set before LIMIT/OFFSET slices a
    // page. An explicit sort_by always wins; its absence keeps today's
  ...
  /**
   * Records the outcome of cover generation. `unavailable` is a real state the row renders, not an
   * error: a missing cover never blocks an import from completing (UI-CM06).
   */
  setCover(
    id: string,
  ...
  listAssets(query: ContentAssetQuery): ContentAssetPage {
    // Whitelist: sort_by must never reach the SQL string by any path other than this exact
    // column lookup (never interpolate the raw value). Record<ContentAssetSortKey, …> keeps
    // this map exhaustive against CONTENT_ASSET_SORT_KEYS at compile time.
