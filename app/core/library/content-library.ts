...
/**
 * Produces a local cover file for one item and returns its path. Injected because covers come
 * from FFmpeg, which Python owns — core never shells out to it.
 */
export type GenerateCover = (content: ContentEntry) => Promise<string>;

/**
 * What probing a local file yields, one variant per importable kind. The host owns probing (core
 * cannot shell to FFmpeg), so it returns this union and core maps it onto a `ContentSource`
 * without inferring the kind from an extension it is not allowed to trust.
 *
 * The video variant may carry the media registry's `asset_id` so a later open reuses the probe
 * that already happened; a test probe can omit it and the open path re-registers instead.
 */
...
/** Maps a probed local original onto the store's source shape, facts included per kind. */
...
/**
 * Returns each content id's label ids from `app/core/taxonomy`. Injected so the Library store
 * filters by the one shared label contract without learning about a second owner's tables.
 */
...
/**
 * The wired-in collaborators, named rather than positional: core cannot probe, generate covers or
 * read labels itself, so each is injected, and three optional booleans-worth of `undefined`
 * placeholders at the call site would hide which one a caller meant.
 */
export interface ContentLibraryOptions {
  ...
  /**
   * Attaches a connector item's hashtags as `tag` labels through `app/core/taxonomy`. Injected for
   * the same reason as `contentLabels`: the tags live in the catalog's taxonomy, and the Library
   * must not learn a second owner's tables.
   */
...
export class ContentLibrary {
  ...
  /**
   * Attaches a cover, or records that there is none. A failure here is a designed state, never a
   * failed import: the item is already usable, and a row with a placeholder cover is better than
   * no row (UI-CM06).
   */
  ...
  /** The identity lookup a download re-run uses: a held `aweme_id` is reused, never re-fetched. */
  findContentByAwemeId(awemeId: string): ContentEntry | null {
    return this.#store.findContentByAwemeId(awemeId);
  }

  /**
   * Registers a downloaded Douyin file as one content record, with its origin, ticket 09's typed
   * intake projection and raw payload, its hashtags as taxonomy labels, and a local cover.
   *
   * The intake is validated before the row is created, so a malformed projection cannot leave a
   * half-registered item; a later failure (tag assignment, cover) removes the row again. The cover
   * is best-effort and never fails the handoff: an unavailable cover is a designed state
   * (UI-CM06). No automation starts and nothing is marked publishable (SL-AC04).
   */
  async registerDouyinContent(
    ...
    try {
      ...
      // Tags are part of the persisted contract: dropping them silently when no taxonomy is wired
      // would make an item look complete while missing its hashtags.
  ...
  /**
   * The read/write half of ticket 09's intake contract, beside `registerDouyinContent` which writes
   * the same store on the download path. Kept on the interface so a caller (and the contract's
   * tests) never reaches into `LibraryStore` directly.
   */
