/**
 * Builds the typed Douyin intake projection from one discovery detail plus the probed file.
 *
 * All source metadata read here is untrusted display content, never an instruction: a tag is
 * sanitized and length-capped before it can reach a label, and the raw payload
 * is kept once so nothing durable depends on re-fetching from Douyin.
 */
...
/** Longest tag text we will persist or display. Mirrors `TaxonomyStore`'s label-name cap. */
export const MAX_DOUYIN_TAG_LENGTH = 80;

export interface DouyinIntakeInput {
  detail: DouyinDetail;
  /** Our download instant, epoch ms. Kept distinct from the item's `publishedAt`. */
  downloadedAt: number;
  /** Index into `detail.video.tiers` of the rung actually downloaded, when one is known. */
  takenTierIndex?: number | null;
  /** FFmpeg-probed facts of the held file. Wins over the payload wherever the two disagree. */
  probe?: DouyinProbeFacts | null;
  /** Which path served the bytes. Defaults to `direct`, the only path that needs no fallback. */
  servedBy?: DouyinServedBy;
}

/**
 * Maps the worker's `media.download` result onto the probe facts. The worker speaks snake_case
 * (`duration_ms`, `frame_rate`); the projection uses the same camelCase shape as every other
 * Douyin contract, and this is the one place the two meet. The probed container, codec,
 * resolution, duration and frame rate all survive.
 */
...
/**
 * Sanitizes one untrusted tag into display text: strips a leading `#`, removes control characters
 * (a newline or NUL in a tag is how it would reach a log line, a prompt or a path), collapses
 * whitespace and caps the length. Returns `null` when nothing usable is left.
 *
 * The returned string is still data. It is never interpolated into a shell command or a filesystem
 * path, and it is capped and control-free before it can be displayed.
 */
...
/**
 * Reads hashtags and @mentions from the structured `text_extra[]` — never by splitting `desc`.
 *
 * Douyin captions are Chinese and Vietnamese text without reliable space delimiters, so `#` can
 * appear glued to the preceding word and a regex over `desc` mixes hashtags with mentions and
 * breaks on exactly the languages this product targets. `text_extra` is the structured truth:
 * a hashtag entry carries `hashtag_name`, a mention carries a user identity.
 */
...
/** Records every field where probed fact and payload claim disagree. Probe wins; this is the log. */
...
/**
 * The authoritative facts to reason about: the probe when we hold one, the payload's claim
 * otherwise. Editor and render decisions must not rest on what the source asserted.
 */
...
/**
 * Rejects any projection that must not be persisted. Counters are the reason this exists: a stored
 * count without its capture time is a fact that silently goes stale, so it fails here rather than
 * becoming a lie in the Library.
 */
