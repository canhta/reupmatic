...
export class LibraryService<T extends VideoSource = VideoSource> {
  ...
  async importFile(filename: string, options: LibraryImportOptions): Promise<{ item: LibraryItem; reused: boolean }> {
    if (!options || !['reference', 'copy'].includes(options.mode)
      || !['reuse', 'separate'].includes(options.duplicates)) throw new Error('INVALID_REQUEST');
    ...
    try {
      if (options.mode === 'copy') {
        ...
        if (!['.mp4', '.mov', '.mkv', '.webm', '.avi'].includes(extension)) throw new Error('INVALID_MEDIA');
        const target = path.join(ownedDirectory, `source${extension}`);
        await copyFile(source.path, target, constants.COPYFILE_EXCL);
        if (await hashFile(target) !== source.sha256 || await hashFile(source.path) !== source.sha256) {
  ...
  async resolve(id: string): Promise<T> {
    ...
    try {
      ...
      if (['ENOENT', 'EACCES', 'ENOTDIR'].some(value => code.includes(value)) || code === 'SOURCE_UNAVAILABLE') {
