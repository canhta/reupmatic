...
export class LibraryAssets {
  ...
  async register(itemId: string, kind: LibraryLinkKind, filename: string, expectedHash?: string): Promise<LibraryLink> {
    ...
    if (kind === 'project') {
      ...
      if (project.source.sha256 !== item.sha256 && !project.composition?.clips.some(clip => clip.source.sha256 === item.sha256)) {
