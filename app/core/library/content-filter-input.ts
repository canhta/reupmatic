...
/**
 * Validation/normalization for a Library filter request, shared by the host-bridge boundary (which
 * receives untrusted IPC input) and the store (which re-checks its own query). One owner means the
 * two can never disagree about what a valid facet looks like.
 */

/** A present-but-empty selection means "no value selected", i.e. no facet, not "match nothing". */
