...
import type { FolderConfig, FolderEntry, FolderRule, FolderSnapshot } from './folder-types.js';
...
export async function validateRoots(config: FolderConfig, workspace: string): Promise<void> {
  for (const root of [config.source_dir, config.output_dir]) {
    if (!path.isAbsolute(root) || await fs.realpath(root).catch(() => '') !== root
      || !(await fs.stat(root)).isDirectory()) throw new RemoteError('WATCH_FOLDER_MISSING');
  }
  // A child output is supported and pruned; source inside output is not.
  if (within(config.output_dir, config.source_dir)
    || within(workspace, config.source_dir) || within(config.source_dir, workspace)
    || within(workspace, config.output_dir)) throw new RemoteError('WATCH_OUTPUT_LOOP');
}
async function fileEntry(filename: string): Promise<FolderEntry | null> {
  ...
  return { path: filename, fingerprint: `${stat.dev}:${stat.ino}:${stat.size}:${stat.mtimeMs}:${stat.ctimeMs}` };
}
 ...
 */
export async function collectFiles(rule: FolderConfig, excludedDirs: string[], excludedFiles: Set<string>): Promise<FolderEntry[]> {
  ...
  const walk = async (directory: string, depth: number): Promise<void> => {
    ...
    for await (const entry of stream) {
      ...
      if (entry.name.startsWith('.') || entry.isSymbolicLink()
        || excludedDirs.some(root => within(root, filename)) || excludedFiles.has(filename)) continue;
      if (entry.isDirectory()) { if (rule.recursive) await walk(filename, depth + 1); continue; }
      if (!EXTENSIONS.has(path.extname(entry.name).toLowerCase())) continue;
      try { const value = await fileEntry(filename); if (value) result.push(value); }
...
export type AttachWatcher = (rule: FolderRule, changed: () => void, failed: (error: unknown) => void) => Promise<WatchHandle>;
interface Runtime {
  handle?: WatchHandle;
...
export class FolderMonitor extends EventEmitter {
  ...
  constructor(private readonly store: FolderStore, private readonly queue: Pick<BatchQueue, 'admitFolder' | 'outputPaths' | 'snapshot'>, private readonly options: Options) {
    super(); this.stableMs = options.stableMs ?? 2000; this.now = options.now ?? Date.now;
  }
  get activeCount(): number { return this.active.size + this.starting.size; }
  snapshot(): FolderSnapshot {
    return {
      version: this.version, available: this.options.authorize() && !this.closing, developer_only: true,
      queue_paused: this.queue.snapshot().paused,
      rules: this.store.list().map(rule => {
        const runtime = this.active.get(rule.id);
        return {
          id: rule.id, source_name: rule.source_dir,
          ...(rule.processing ? { processing: rule.processing } : {}), output_name: rule.output_dir,
          include_existing: rule.include_existing, recursive: rule.recursive, initialized: rule.initialized,
          state: runtime ? runtime.error ? 'needs_attention' : !runtime.handle || runtime.scanning ? 'scanning' : 'watching' : 'paused',
          admitted: this.store.count(rule.id), last_scan: runtime?.lastScan ?? null,
          error_code: runtime?.error ?? null, error_file: runtime?.errorFile ?? null,
        };
      }),
    };
  }
  private changed(): void { this.version++; this.emit('changed', this.snapshot()); }
  notifyQueue(): void { if (!this.closing) this.changed(); }
  ...
  async create(config: FolderConfig): Promise<FolderSnapshot> {
    this.allowed(); await validateRoots(config, this.options.workspace);
    this.allowed();
    if (this.store.list().some(rule => within(rule.output_dir, config.source_dir) || within(config.output_dir, rule.source_dir))) {
      throw new RemoteError('WATCH_OUTPUT_LOOP');
    }
    this.store.add(config); this.changed(); return this.snapshot();
  }
  ...
  private async startRule(id: string): Promise<FolderSnapshot> {
    ...
    try {
      ...
      if (!rule.initialized) {
        const baseline = await collectFiles(rule, this.exclusions(), new Set(this.queue.outputPaths()));
        this.allowed();
      ...
      this.allowed();
      if (!this.starting.has(id)) return this.snapshot();
      const runtime: Runtime = { observed: new Map(), checked: new Map(), scanning: false, lastScan: null, error: null, errorFile: null };
      this.active.set(id, runtime);
      try {
        const handle = await this.options.attach(rule, () => this.schedule(id, 100), error => {
          if (this.active.get(id) !== runtime) return;
          runtime.error = safeCode(error); this.changed();
        });
        if (this.active.get(id) !== runtime || this.closing) { await handle.close(); return this.snapshot(); }
        runtime.handle = handle;
        const interval = this.options.reconcileMs ?? 30000;
        if (interval > 0) { runtime.interval = setInterval(() => { void this.reconcile(id); }, interval); runtime.interval.unref(); }
        this.changed(); await this.reconcile(id);
      } catch (error) { await this.pause(id); throw error; }
  ...
  private schedule(id: string, delay: number): void {
    ...
    runtime.timer = setTimeout(() => { runtime.timer = undefined; void this.reconcile(id); }, delay);
    runtime.timer.unref();
  }
  async reconcile(id: string): Promise<void> {
    ...
    runtime.inFlight = this.scan(id, runtime).finally(() => { runtime.inFlight = undefined; });
    return runtime.inFlight;
  }
  private async scan(id: string, runtime: Runtime): Promise<void> {
    const current = () => !this.closing && this.active.get(id) === runtime && this.options.authorize();
    let pending = false;
    runtime.scanning = true; runtime.error = null; runtime.errorFile = null; this.changed();
    try {
      this.allowed();
      const rule = this.store.get(id);
      await validateRoots(rule, this.options.workspace);
      const entries = await collectFiles(rule, this.exclusions(), new Set(this.queue.outputPaths()));
      const baseline = this.store.baseline(id);
      const visible = new Set(entries.map(entry => entry.path));
      for (const name of runtime.observed.keys()) if (!visible.has(name)) { runtime.observed.delete(name); runtime.checked.delete(name); }
      for (const entry of entries) {
        if (!current()) break;
        if (baseline.get(entry.path) === entry.fingerprint || runtime.checked.get(entry.path) === entry.fingerprint) continue;
        const before = runtime.observed.get(entry.path);
        if (!before || before.fingerprint !== entry.fingerprint) {
          runtime.observed.set(entry.path, { fingerprint: entry.fingerprint, since: this.now() }); pending = true; continue;
        }
        if (this.now() - before.since < this.stableMs) { pending = true; continue; }
        try {
          ...
          if (!after || after.fingerprint !== entry.fingerprint) { runtime.observed.delete(entry.path); pending = true; continue; }
          if (!current()) break;
          if (!this.store.seen(id, sha256)) {
            ...
            const jobId = this.queue.admitFolder(admissionId(id, sha256), {
              ...
              output_dir: rule.output_dir, encoding: 'review',
              ...(rule.processing ? { processing: rule.processing, processing_models: rule.processing_models } : {}),
            });
        ...
        } catch (error) { runtime.error = safeCode(error); runtime.errorFile = path.basename(entry.path); }
      }
      runtime.lastScan = this.now();
    } catch (error) { runtime.error = safeCode(error); }
    finally {
      runtime.scanning = false;
      if (this.active.get(id) === runtime) { this.changed(); if (pending) this.schedule(id, this.stableMs); }
    }
  }
  async pause(id: string): Promise<FolderSnapshot> {
    this.store.get(id); this.starting.delete(id);
    const runtime = this.active.get(id);
    this.active.delete(id);
    if (runtime) {
      clearTimeout(runtime.timer); clearInterval(runtime.interval);
      try { await runtime.handle?.close(); } finally { await runtime.inFlight; }
    }
    this.changed(); return this.snapshot();
  }
  async close(): Promise<void> {
    this.closing = true; this.starting.clear();
