...
import type { FolderConfig } from './folder-contracts.js';
...
/** Config plus intake receipts; the batch journal owns jobs. */
export class FolderStore {
  ...
  constructor(filename: string) {
    this.db = openCurrentDatabase(
      filename,
      {
        tables: ['folder_rules', 'folder_baseline', 'folder_receipts'],
        sql: `
  ...
  list(): FolderRule[] {
    return this.db
      .prepare('SELECT * FROM folder_rules ORDER BY created_at,rowid')
      .all()
      .map((row) => ({
  ...
  add(config: FolderConfig): FolderRule {
    ...
    if (this.list().length >= 25) throw new RemoteError('WATCH_LIMIT');
    if (
      this.list().some(
        (rule) => rule.source_dir === config.source_dir && rule.output_dir === config.output_dir,
      )
    )
      throw new RemoteError('WATCH_EXISTS');
    const id = randomUUID();
    this.db
      .prepare('INSERT INTO folder_rules(id,config_json,created_at) VALUES(?,?,?)')
      .run(id, JSON.stringify(config), Date.now());
    return this.get(id);
  }
  private parseConfig(config: FolderConfig): FolderConfig {
    if (config?.processing !== undefined) {
      const processing = parseReusableRecipe(config.processing);
      config = {
        ...config,
        processing,
        processing_models: parseModelFingerprints(config.processing_models, processing),
      };
    } else if (config?.processing_models !== undefined)
      throw new RemoteError('INVALID_PROCESSING_MODELS');
    if (
      !config ||
      !path.isAbsolute(config.source_dir) ||
      !path.isAbsolute(config.output_dir) ||
      config.source_dir.includes('\0') ||
      config.output_dir.includes('\0') ||
      typeof config.include_existing !== 'boolean' ||
  ...
  baseline(id: string): Map<string, string> {
    return new Map(
      this.db
        .prepare('SELECT path,fingerprint FROM folder_baseline WHERE rule_id=?')
        .all(id)
  ...
  seen(id: string, sha256: string): boolean {
    return !!this.db
      .prepare('SELECT 1 FROM folder_receipts WHERE rule_id=? AND sha256=?')
      .get(id, sha256);
  }
  ...
  count(id: string): number {
    return Number(
      this.db.prepare('SELECT count(*) AS n FROM folder_receipts WHERE rule_id=?').get(id)?.n ?? 0,
    );
  }
}
