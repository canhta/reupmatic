...
/** Native filesystem APIs for reconciliation only; Chokidar remains the watcher. */
export async function collectFiles(
  ...
  const walk = async (directory: string, depth: number): Promise<void> => {
    ...
    // Reject link/junction changes between observations.
...
/** Admission into BatchQueue only; no job execution or AI here. */
  ...
  private async scan(id: string, runtime: Runtime): Promise<void> {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        try {
          // Size stability is a heuristic; recheck after hashing, worker verifies again.
