...
import { type ClipSpan, type Composition, compositionSpans } from './document.js';
...
export function remapCompositionCues(
  ...
  assertCues(cues);
  const previous = compositionSpans(before),
    next = compositionSpans(after);
  ...
  for (const cue of cues) {
    ...
    for (const old of previous) {
      const first = Math.max(cue.start_ms, old.start_ms),
        last = Math.min(cue.end_ms, old.end_ms);
      ...
      for (const target of descendants.get(old.clip.id) ?? []) {
        const start = Math.max(sourceStart, target.clip.start_ms),
          end = Math.min(sourceEnd, target.clip.end_ms);
        if (end <= start) continue;
        const start_ms =
          target.start_ms + Math.round((start - target.clip.start_ms) / target.clip.speed);
