...
/** One still image's placement on the output frame. `media_id` references a Project
 * media `image` row and is project-specific, so a saved profile keeps the placement
 * but drops the reference (D-63, "Profiles never carry project media"). */
export interface LogoPlacement {
  media_id?: string;
  anchor: LogoAnchor;
  /** Inset from the anchored edge, as a fraction of the output width. */
  margin: number;
  /** Logo width as a fraction of the output width; the height keeps the image ratio. */
  scale: number;
  opacity: number;
}
/** A new logo's placement before the user moves it: a small mark in the bottom-right. */
...
/** A Project media id: the reference a logo placement keeps to its project's image row. */
