import { validTranslationOrigin, type TranslationOrigin } from './translation-origin.js';
...
export type LayerOrigin =
  ...
  | { kind: 'stt' | 'ocr'; request_id: string; source_sha256: string;
      start_ms: number; end_ms: number; model_id?: string; runtime?: string };
...
export interface TextLayer extends LayerMetadata { cues: Cue[] }
...
export interface TextSnapshot { cues: Cue[]; text_layers?: TextLayers }
...
function validOrigin(value: unknown): boolean {
  ...
  if (value.kind === 'copy') return exact(value, ['kind', 'layer', 'token'])
    && textLayerNames.includes(value.layer as TextLayerName) && token(value.token);
  return (value.kind === 'stt' || value.kind === 'ocr')
    && exact(value, ['kind', 'request_id', 'source_sha256', 'start_ms', 'end_ms',
      ...('model_id' in value ? ['model_id'] : []), ...('runtime' in value ? ['runtime'] : [])])
    && token(value.request_id) && hash(value.source_sha256)
    && Number.isInteger(value.start_ms) && Number.isInteger(value.end_ms)
    && Number(value.start_ms) >= 0 && Number(value.end_ms) > Number(value.start_ms)
    && Number(value.end_ms) <= 86400000
    && (!('model_id' in value) || hash(value.model_id))
    && (!('runtime' in value) || (typeof value.runtime === 'string' && value.runtime.length > 0
      && value.runtime.length <= 128 && !value.runtime.includes('\0')))
...
export function parseTextLayers(value: unknown): TextLayers {
  ...
  for (const name of textLayerNames) {
    ...
    if (!object(item) || !exact(item, ['token', 'language', 'origin', 'edited', 'stale',
      ...(name === 'displayed' ? [] : ['cues'])]) || !token(item.token)
      || ![null, 'en', 'vi', 'zh'].includes(item.language as TextLanguage)
      || !validOrigin(item.origin) || typeof item.edited !== 'boolean' || typeof item.stale !== 'boolean') {
      throw new Error('INVALID_TEXT_LAYERS');
    }
    if (name !== 'displayed') {
      try { assertCues(item.cues); } catch { throw new Error('INVALID_TEXT_LAYERS'); }
...
export function createTextLayers(): TextLayers {
  const metadata = (name: TextLayerName): LayerMetadata => ({
    token: `initial-${name}`, language: null, origin: { kind: 'manual' }, edited: false, stale: false,
  });
  return { version: 2, displayed: metadata('displayed'),
...
export function getTextLayer(snapshot: TextSnapshot, name: TextLayerName): TextLayer {
  ...
  return structuredClone(name === 'displayed' ? { ...layers.displayed, cues: snapshot.cues } : layers[name]);
}
