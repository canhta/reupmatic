...
export interface LayerMetadata {
  ...
  edited: boolean;
  stale: boolean;
  /** Shown on the subtitles lane and burned into the output. At most one layer is visible at a
   *  time, so exactly one track is burned; hiding keeps every cue. */
  visible: boolean;
}
...
export function parseTextLayers(value: unknown): TextLayers {
  if (!object(value) || !exact(value, [...textLayerNames])) {
  ...
  // Only one text layer is burned at a time, so two visible layers is not a document this build
  // can render; it fails where it is read rather than silently picking one.
  if (textLayerNames.filter((name) => (value[name] as { visible: boolean }).visible).length > 1) {
    throw new Error('INVALID_TEXT_LAYERS');
  }
...
/** The one layer shown on the subtitles lane and burned into the output; `null` when every layer
 *  is hidden. A snapshot without text layers keeps the original behaviour: the displayed track. */
export function visibleTextLayer(snapshot: TextSnapshot): TextLayerName | null {
  if (!snapshot.text_layers) return 'displayed';
  return textLayerNames.find((name) => snapshot.text_layers?.[name].visible) ?? null;
}
