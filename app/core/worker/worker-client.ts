...
export class WorkerClient extends EventEmitter {
  ...
  constructor(python: string, workerPath: string, workspace: string) {
    ...
    lines.on('line', (line) => {
      ...
      try {
        ...
        if (msg.event === 'error')
          pending.reject(new RemoteError(String(msg.data.code || 'WORKER_FAILURE')));
  ...
  request<T = Record<string, unknown>>(
    ...
    const result = new Promise<T>((resolve, reject) => {
      ...
      // The pending table is type-erased across requests of differing T; each resolver
      // is only ever invoked with this request's own worker response (see the message handler).
      this.pending.set(id, { resolve: resolve as (data: unknown) => void, reject, revision });
    ...
    return {
      id,
      result,
      cancel: () => this.request<{ requested: boolean }>('cancel', { request_id: id }).result,
    };
  }
