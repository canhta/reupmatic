...
export async function protectSources(
  destination: string,
  ...
  for (const source of sources) {
    ...
    if (
      resolved === original ||
      (target && info && target.ino !== 0 && target.ino === info.ino && target.dev === info.dev)
    )
      throw new Error('SOURCE_OVERWRITE');
  }
}
...
export async function saveChosenExport(
  ...
  try {
    ...
    checkActive();
    if ((await realpath(path.dirname(filename))) !== directory)
