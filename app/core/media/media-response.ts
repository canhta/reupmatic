...
export async function registeredMediaResponse(
  ...
  try {
    ...
    const types: Record<string, string> = {
      ...
      '.avi': 'video/x-msvideo',
      // Library covers: FFmpeg writes JPEG; the rest are accepted so the registry stays a
      // generic local-file responder. Without a real image type, X-Content-Type-Options:
      // nosniff makes Chromium refuse the image and the row shows a broken glyph.
      '.jpg': 'image/jpeg',
      '.jpeg': 'image/jpeg',
      '.png': 'image/png',
      '.webp': 'image/webp',
