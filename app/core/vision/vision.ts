...
export interface VisionParams extends Record<string, unknown> {
  asset_id: string; start_ms: number; end_ms: number;
  language?: ContentLanguage; sample_ms?: number; min_confidence?: number;
  target?: 'manual' | 'text'; padding_px?: number; region?: Region;
}
export interface VisionInput {
  request_id: string; revision: number; method: 'media.ocr' | 'media.ocr.extract' | 'media.inpaint'; params: VisionParams;
}
export interface Detection { text: string; confidence: number; box: [number, number, number, number] }
export interface Observation { start_ms: number; end_ms: number; detections: Detection[] }
export interface OcrResult extends Record<string, unknown> {
  kind: 'ocr'; asset_id: string; source_sha256: string; start_ms: number; end_ms: number;
  language: ContentLanguage; sample_ms: number; cues: Cue[]; observations: Observation[];
  analysis_id: string; width: number; height: number;
  scope?: 'full-source';
...
export interface InpaintResult extends Record<string, unknown> {
  kind: 'inpainting'; asset_id: string; artifact_id: string; duration_ms: number;
  path?: string; url?: string; width: number; height: number; fps: number;
}
export type VisionResult = OcrResult | InpaintResult;
export interface ModelState {
  available: boolean; code: string | null; languages: ContentLanguage[]; verified: boolean;
}
export interface ModelStatus { ocr: ModelState; inpainting: ModelState }
...
function validRegion(value: unknown): value is Region {
  ...
  return [x, y, width, height].every(v => typeof v === 'number' && Number.isFinite(v))
    ...
    && Number(x) + Number(width) <= 1 + 1e-9 && Number(y) + Number(height) <= 1 + 1e-9;
}
export function parseVisionInput(value: unknown): VisionInput {
  if (!object(value) || Object.keys(value).sort().join(',') !== 'method,params,request_id,revision'
    || typeof value.request_id !== 'string' || !/^[a-zA-Z0-9_-]{8,128}$/.test(value.request_id)
    || !integer(value.revision, 0, 2 ** 31 - 1)
    || !['media.ocr', 'media.ocr.extract', 'media.inpaint'].includes(String(value.method)) || !object(value.params)) {
    throw new RemoteError('INVALID_REQUEST');
  }
  const p = value.params, ocr = value.method !== 'media.inpaint';
  const required = ['asset_id', 'start_ms', 'end_ms', ...(ocr
    ? ['language', 'sample_ms', 'min_confidence'] : ['target', 'padding_px'])];
  const allowed = [...required, 'region', ...(!ocr ? ['language'] : [])];
  if (required.some(key => !(key in p)) || Object.keys(p).some(key => !allowed.includes(key))
    || !identifier(p.asset_id) || !integer(p.start_ms, 0, 86400000)
    || !integer(p.end_ms, 1, 86400000) || Number(p.end_ms) <= Number(p.start_ms)
    || ('region' in p && !validRegion(p.region))) throw new RemoteError('INVALID_REQUEST');
  if (value.method === 'media.ocr.extract' && p.start_ms !== 0) throw new RemoteError('INVALID_REQUEST');
  if (value.method !== 'media.ocr.extract' && Number(p.end_ms) - Number(p.start_ms) > (ocr ? 120000 : 10000)) throw new RemoteError('VISION_LIMIT');
  if (ocr) {
    if (!integer(p.sample_ms, 100, 2000) || typeof p.min_confidence !== 'number'
      || !Number.isFinite(p.min_confidence) || p.min_confidence < 0 || p.min_confidence > 1) {
      throw new RemoteError('INVALID_REQUEST');
    }
  } else if (!['manual', 'text'].includes(String(p.target)) || !integer(p.padding_px, 0, 32)
    || (p.target === 'manual' && (!('region' in p) || 'language' in p))
    || (p.target === 'text' && 'region' in p)) throw new RemoteError('INVALID_REQUEST');
...
export function validateVisionResult(value: unknown, input: VisionInput): VisionResult {
  if (!object(value) || value.asset_id !== input.params.asset_id
    || value.start_ms !== input.params.start_ms || value.end_ms !== input.params.end_ms
    || !integer(value.width, 2, 1920) || !integer(value.height, 2, 1920)
  ...
  if (input.method !== 'media.inpaint') {
    if (value.kind !== 'ocr' || value.language !== input.params.language
      || value.sample_ms !== input.params.sample_ms || !identifier(value.analysis_id)
      || !Array.isArray(value.observations) || value.observations.length > 1200) {
      throw new RemoteError('INVALID_WORKER_RESPONSE');
    }
    if (input.method === 'media.ocr.extract' && (value.scope !== 'full-source' || !object(value.evidence)
    ...
    try { assertCues(value.cues); } catch { throw new RemoteError('INVALID_WORKER_RESPONSE'); }
    ...
    for (const item of value.observations) {
      if (!object(item) || !integer(item.start_ms, previous, input.params.end_ms - 1)
        || !integer(item.end_ms, Number(item.start_ms) + 1, input.params.end_ms)
        || !Array.isArray(item.detections) || item.detections.length > 100) {
        throw new RemoteError('INVALID_WORKER_RESPONSE');
      }
      previous = Number(item.end_ms);
      for (const d of item.detections) {
        if (!object(d) || typeof d.text !== 'string' || d.text.length > 10000 || d.text.includes('\0')
          || typeof d.confidence !== 'number' || !Number.isFinite(d.confidence)
          || d.confidence < 0 || d.confidence > 1 || !Array.isArray(d.box) || d.box.length !== 4
          || !integer(d.box[0], 0, Number(value.width) - 1) || !integer(d.box[1], 0, Number(value.height) - 1)
  ...
  } else if (value.kind !== 'inpainting' || !identifier(value.artifact_id)
    || typeof value.path !== 'string' || !value.path || value.path.includes('\0')
    || !integer(value.duration_ms, 1, 10050) || value.fps !== 24) {
    throw new RemoteError('INVALID_WORKER_RESPONSE');
  }
  return value as unknown as VisionResult;
}

interface Operation { input: VisionInput; cancelled: boolean; ticket?: Ticket<Record<string, unknown>> }
type WorkerPort = Pick<WorkerClient, 'request' | 'on' | 'off'>;

/** Admission/lifecycle, not a scheduler: every operation uses the existing worker queue. */
