...
// Single source of truth for a post query's sort keys/directions: the derived types below feed
// PostQuery and the store's comparator map (post-store.ts), so the two stay in sync at compile
// time.
export const POST_SORT_KEYS = ['title', 'channel', 'state', 'planned'] as const;
export type PostSortKey = (typeof POST_SORT_KEYS)[number];
export const POST_QUERY_SORT_DIRECTIONS = ['ascending', 'descending'] as const;
export type PostQuerySortDirection = (typeof POST_QUERY_SORT_DIRECTIONS)[number];
...
export interface PostQuery {
  ...
  offset: number;
  limit: number;
  /** Omitted: 'upcoming' defaults to ascending planned time, every other view to creation order. */
  sort_by?: PostSortKey;
  sort_dir?: PostQuerySortDirection;
}
