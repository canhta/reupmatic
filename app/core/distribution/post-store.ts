...
export class PostStore {
  ...
  list(input: PostQuery): Page<Post> {
    ...
    // Sort applies to the whole filtered set before offset/limit slices a
    // page, so an explicit sort_by always wins; only its absence falls back
    // to the view's own default order (ascending planned time for
    // 'upcoming', otherwise storage/creation order).
