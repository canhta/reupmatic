...
/**
 * Filtering and sorting over the Search result set already retrieved — never a request for more.
 *
 * Ticket 08 has no cited desktop precedent for faceted filtering before acquisition, so the facet
 * set is deliberately minimal: the facts that help choose *what to download*. The rule it shares
 * with the Library is the important one — a facet a candidate cannot answer excludes nothing, so a
 * withheld counter never reads as zero and an item without a duration is never hidden by a
 * duration filter.
 */
...
export interface DouyinCandidateView {
  items: DouyinItem[];
  /** Candidates considered, i.e. what the scan retrieved before filtering. */
  total: number;
  matched: number;
  /** False when no candidate carries the facet, so its control is offered disabled. */
...
/**
 * The offered quality tier a candidate is described by. Douyin orders the ladder best-first, and
 * `video.bit_rate[]` carries `width`/`height`/`data_size` per gear, so size and resolution are
 * known before download. Ties keep the earlier (better-ranked) entry.
 */
...
/**
 * The index of `bestQualityTier` in `video.tiers`, so the download can name the rung it takes.
 * Defaults to `0` (Douyin's own best-first ordering) when the ladder is absent.
 */
...
export function candidateViews(item: DouyinItem): number | null {
  ...
  // Douyin reports `play_count: 0` on every public endpoint (measured); it is withheld,
  // not a real zero, so it must render unavailable rather than `0` (SL-R02).
  return views !== null && views > 0 ? views : null;
}

/** Likes (`digg_count`). A withheld counter is `null`, never zero (SL-R02). */
...
/** `null` (a withheld or absent value) always sorts after a known one, and ties keep input order. */
...
/** Applies the query to the retrieved set only; it never asks the source for more. */
export function viewDouyinCandidates(
  ...
  const matched = items.filter((item) => {
    ...
    if (durationRange) {
      ...
      // No duration is not "zero": the candidate stays and is shown as unmeasured.
      if (duration !== null) {
    ...
    if (minViews !== undefined) {
      ...
      // A withheld play count must not read as zero, so it cannot fail an "at least N" filter.
