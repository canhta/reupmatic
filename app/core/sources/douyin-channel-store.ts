...
/** One Douyin channel the user has searched, kept so it can be scanned again without a link. */
...
/**
 * The small durable list of Douyin channels behind the Downloads tab: whenever a search returns a
 * channel it is upserted here, and the tab lists them so a later scan needs no pasted link. Keyed
 * on the durable `sec_uid` — a nickname changes and collides, `sec_uid` does not.
 */
export class DouyinChannelStore {
  ...
  /** Most recently scanned first — the channel the user just searched is the one they see. */
  list(): DouyinChannel[] {
    return this.db
      .prepare(
