...
export class WorkspaceCatalog {
  ...
  snapshot(executionAvailable: boolean): CatalogSnapshot {
    return { revision: this.db.version, channels: this.distribution.channels(), links: this.distribution.links(),
      labels: this.taxonomy.list(), content_labels: this.taxonomy.content(), link_usage: this.posts.usage(),
      profiles: this.profiles.list(), workflows: this.workflows.list(), runs: this.workflows.runs(),
      execution_available: executionAvailable };
  }

  dependencies(itemId: string) {
    return { ...this.posts.contentDependencies(itemId),
      workflows: this.workflows.list().filter(workflow => workflow.item_ids.includes(itemId)).length };
  }
