...
export function installRecovery(host: Host) {
  ...
  try {
    store = new RecoveryStore(path.join(host.workspace, 'editor-recovery.sqlite'));
  } catch (error) {
  ...
  host.wire('recovery-open', async (input) => {
    ...
    const id = requestId(value.id),
      revision = requestRevision(value.expected_revision);
  ...
  host.wire('recovery-flush-result', (input) => {
    ...
    if (!pending || value.request_id !== pending.id || typeof value.saved !== 'boolean')
