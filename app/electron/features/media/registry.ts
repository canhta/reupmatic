...
/** An Electron file-dialog filter; `extensions` is a plain list, never a tuple of literals. */
export interface MediaFilter {
  name: string;
  extensions: string[];
}

// The extensions come from core's `LOCAL_MEDIA_EXTENSIONS`, the same list the copy-mode allowlist
// enforces, so a native picker can never offer a file the import path then refuses.
...
// Still images are a Project media kind the Editor's logo uses, not a Library
// import kind (`LOCAL_MEDIA_EXTENSIONS` stays video/audio/subtitle).
export const imageFilters: MediaFilter[] = [{ name: 'Images', extensions: ['png', 'jpg', 'jpeg'] }];
/** Every kind a local Library import accepts, in one picker entry. */
...
/** The Editor's one Add…/Import Media picker: every picker kind plus a logo image. */
export const editorImportFilters: MediaFilter[] = [...localImportFilters, ...imageFilters];

/** The media-registry view of a registered subtitle file: identity plus the id to load cues with. */
...
export class MediaRegistry {
  ...
  /**
   * A still image a project's logo draws (PNG/JPEG, PNG alpha kept). Registered like
   * the other media so the renderer can read it through `media://local/...` — the app
   * CSP blocks `file:` — and the render coordinator can hand the worker an asset id.
   */
  async registerImage(
    filename: string,
  ...
  /**
   * Publishes a Library item's local cover under the id `libraryCoverAssetId(id)` so the
   * renderer can read it through `media://local/...`; a `file://` URL is blocked by the app CSP.
   * Idempotent: the same cover is re-registered each time its page is listed.
   */
