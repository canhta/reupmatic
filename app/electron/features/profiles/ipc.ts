...
import { parseProfileDocument } from '../../../core/profiles/profile-store.js';
...
export function installProfiles(host: CatalogHost): void {
  host.wire('profile-save', input => {
    const result = host.catalog().profiles.save(input);
    host.changed();
    return result;
  });
  host.wire('profile-read', async () => {
    const chosen = await dialog.showOpenDialog(host.getWindow(), {
      properties: ['openFile'], filters: [{ name: 'Reupmatic processing profile', extensions: ['json'] }],
    });
    ...
    try {
      ...
      catch { throw new Error('PROFILE_INVALID'); }
      return parseProfileDocument(value);
    } finally { await file.close(); }
  });
  host.wire('profile-export', async input => {
    const id = requestId(requestRecord(input, ['id']).id);
    const document = host.catalog().profiles.document(id);
    const chosen = await dialog.showSaveDialog(host.getWindow(), {
      defaultPath: 'processing-profile.json', filters: [{ name: 'Reupmatic processing profile', extensions: ['json'] }],
    });
