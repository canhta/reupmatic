...
import type {
  ...
  BatchSnapshot,
  FileIdentity,
} from '../../../core/batch/batch-types.js';
import type { VideoSource } from '../../../core/media/media-types.js';
import { parseModelFingerprints, parseProcessingRecipe } from '../../../core/processing/recipe.js';
...
type Wire = (name: string, handler: (input: unknown) => Promise<unknown> | unknown) => void;
interface Host {
  wire: Wire;
...
export async function installBatch(host: Host): Promise<BatchQueue | undefined> {
  ...
  host.wire('batch-from-library', async (input) => {
    ...
    if (
      !Array.isArray(data.item_ids) ||
      !data.item_ids.length ||
      data.item_ids.length > 100 ||
      !host.resolveLibrary ||
      videos.size + data.item_ids.length > 2000
    ) {
      throw new RemoteError('INVALID_REQUEST');
    }
    const result: BatchSelection = { items: [], rejected: [] };
  ...
  host.wire('batch-enqueue', async (input) => {
    ...
    if (
      ...
      throw new RemoteError('INVALID_REQUEST');
    const processing =
      data.processing === undefined ? undefined : parseProcessingRecipe(data.processing);
    const requests: BatchJobInput[] = data.items.map((value) => {
      ...
      const subtitle =
        item.subtitle_id === undefined ? undefined : subtitles.get(identifier(item.subtitle_id));
      ...
      return {
        ...
        encoding: 'review',
        ...(libraryIds.has(String(item.asset_id))
          ? { library_id: libraryIds.get(String(item.asset_id)) }
          : {}),
      };
    });
    if (processing) {
  ...
  host.wire('batch-cancel', (input) =>
    required().cancel(identifier(record(input, ['job_id']).job_id)),
  );
  host.wire('batch-retry', (input) =>
    required().retry(identifier(record(input, ['job_id']).job_id)),
  );
