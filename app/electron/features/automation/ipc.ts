...
export function installWorkflows(
  ...
  host.wire('workflow-save', (input) => {
    ...
    const output =
      value.output_id === undefined
        ? catalog.workflows.get(id).output_dir
        : directories.get(requestId(value.output_id));
  ...
  host.wire('workflow-run', async (input) => {
    ...
    try {
      const result = await runner().run(
        requestId(value.workflow_id),
        requestRevision(value.expected_revision),
