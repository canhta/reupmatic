...
export async function installFolders(host: Host): Promise<FolderMonitor | undefined> {
  ...
  const required = () => {
    if (!monitor) throw new RemoteError(problem);
    return monitor;
  };
  const allowed = () => {
    required();
  ...
  host.wire('folder-create', async (input) => {
    ...
    const source = directories.get(id(data.source_id));
    const output = directories.get(id(data.output_id));
    if (
      !source ||
      !output ||
      typeof data.include_existing !== 'boolean' ||
  ...
  host.wire('folder-start', (input) => {
    allowed();
    return required().start(id(record(input, ['rule_id']).rule_id));
  });
