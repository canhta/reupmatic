...
import { parseSynthesisInput, parseSynthesisStatus } from '../../../../core/speech/synthesis/contracts.js';
...
interface Host {
  workspace: string; media: MediaRegistry; savePath: (name: string) => string;
  wire: IpcWire;
  worker: WorkerClient;
...
export function installSynthesis(host: Host) {
  ...
  host.wire('synthesis-status', async () => parseSynthesisStatus(await host.worker.request('synthesis.status', {}).result));
  ...
  host.wire('synthesis-configure', async () => {
    ...
    try {
      const selected = await dialog.showOpenDialog(window, {
        title: host.getLanguage() === 'vi' ? 'Chọn cấu hình mô hình giọng nói cục bộ' : 'Choose local voice model manifest',
    ...
    } finally { choosing = false; setup = undefined; send('synthesis-models-changed'); }
  });
  ...
  return {
    get activeCount() { return coordinator.activeCount + Number(choosing) + exports.activeCount; },
    async close() {
      closing = true;
      cancelled = true;
      exports.close();
