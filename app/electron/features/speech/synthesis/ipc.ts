...
export function installSynthesis(host: Host) {
  ...
  host.wire('synthesis-configure', async () => {
    ...
    try {
      ...
      // Bring-your-own-bundle goes through the same single mechanism a download does: the host
      // records the bundle's on-disk hashes, then the one configuration transaction verifies them.
  ...
  return {
    ...
    /** The one gate the render path uses: re-verify the saved voice artifact through the same
     *  inspection the save dialog uses, then hand back the WAV the worker may register. A
     *  caller-supplied filesystem path is never admitted. */
