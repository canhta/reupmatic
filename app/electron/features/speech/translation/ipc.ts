...
import { parseTranslationStatus } from '../../../../core/speech/translation/contracts.js';
...
export function installTranslation(host: Host) {
  ...
  host.wire('translation-status', async () =>
    parseTranslationStatus(await host.worker.request('translation.status', {}).result),
  );
  host.wire('translation-start', (input) => {
  ...
  host.wire('translation-cancel', (input) => coordinator.cancel(input.request_id));
  host.wire('translation-configure', async () => {
    ...
    try {
      const selected = await dialog.showOpenDialog(window, {
        title:
          host.getLanguage() === 'vi'
            ? 'Chọn cấu hình mô hình dịch cục bộ'
            : 'Choose local translation model manifest',
        properties: ['openFile'],
      ...
      choosing = false;
      setup = undefined;
      send('translation-models-changed');
    }
  });
