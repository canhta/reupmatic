...
export function installSpeech(host: Host) {
  ...
  host.wire('speech-status', async () => parseSpeechStatus(await host.worker.request('speech.status', {}).result));
  host.wire('speech-start', value => {
    if (closing) throw new Error('APP_CLOSING');
    if (choosing) throw new Error('EDITOR_BUSY');
    const input = parseSpeechInput(value), source = host.media.getVideo(input.params.asset_id);
  ...
  host.wire('speech-configure', async () => {
    ...
    try {
      const selected = await dialog.showOpenDialog(window, {
        title: host.getLanguage() === 'vi' ? 'Chọn cấu hình mô hình nhận dạng cục bộ' : 'Choose local speech model manifest',
    ...
    } finally { choosing = false; setup = undefined; send('speech-models-changed'); }
  });
