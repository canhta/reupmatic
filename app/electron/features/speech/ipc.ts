...
export function installSpeech(host: Host) {
  // Credentials go to the hosted child's environment only, never read for another purpose.
  ...
  const offered = installOfferedModels({
    ...
    // The entry's task decides which panel refreshes; one channel for all missed Editor panels.
    modelsChanged: (task) =>
      send(
        task === 'synthesis'
