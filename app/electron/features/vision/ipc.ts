...
import { VisionCoordinator, parseVisionInput } from '../../../core/vision/vision.js';
import type { WorkerClient } from '../../../core/worker/worker-client.js';

interface Host {
  wire: (name: string, handler: (input: any) => Promise<unknown> | unknown) => void;
  getWindow: () => BrowserWindow | undefined;
  worker: WorkerClient; workspace: string;
...
export function installVision(host: Host): VisionCoordinator {
  ...
  coordinator.on('job', message => {
    ...
    if (message.event === 'result' && message.data.kind === 'inpainting') {
      ...
      if (typeof filename !== 'string' || path.dirname(path.resolve(filename)) !== renderRoot
  ...
  host.wire('vision-cancel', value => {
    if (!value || Object.keys(value).length !== 1 || typeof value.request_id !== 'string'
      || !/^[a-zA-Z0-9_-]{8,128}$/.test(value.request_id)) throw new Error('INVALID_REQUEST');
