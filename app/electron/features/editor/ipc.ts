...
import type { RenderInput } from '../../../core/rendering/render-coordinator.js';
...
export function installEditor(host: Host): void {
  ...
  wire('ass', async input => {
    ...
    return { revision, ...await worker.request('subtitles.preview', { cues: value.cues,
      canvas: { width: source.width, height: source.height },
      ...(value.style === undefined ? {} : { style: parseSubtitleStyle(value.style) }) }, revision).result };
  });
  wire('peaks', input => {
  ...
  wire('render', input => {
    const value = requestRecord(input, ['request_id', 'asset_id', 'cues', 'revision', 'mode', 'encoding', 'start_ms', 'end_ms', 'processing', 'soundtrack', 'composition']);
  ...
  wire('cancel', input => host.renderer.cancel(requestId(requestRecord(input, ['request_id']).request_id)));
  wire('save-project', async input => {
    ...
    if (project.composition) linked = (await host.library.recordCompositionLinks(project.composition, 'project', chosen.filePath)) && linked;
    if (project.soundtrack) {
      const audioLinked = await host.library.recordLink(source, 'audio', project.soundtrack.source.path);
  ...
  wire('open-project', async () => {
    const chosen = await dialog.showOpenDialog(host.getWindow(), {
      properties: ['openFile'], filters: [{ name: 'Reupmatic project', extensions: ['json'] }],
    });
    if (chosen.canceled) return null;
    ...
    const snapshot = await restoreProjectSnapshot(media, host.getWindow(), project, source, host.getLanguage());
    return snapshot ? { media: media.publicVideo(source), snapshot } : null;
  });
  wire('save-subtitles', async input => {
    const value = requestRecord(input, ['cues', 'asset_id', 'timing', 'format', 'style', 'editing', 'composition']);
    if (!['source', 'output'].includes(String(value.timing)) || !['srt', 'ass'].includes(String(value.format))) {
    ...
    const composition = value.composition === undefined ? undefined : parseComposition(value.composition);
    ...
    const chosen = await dialog.showSaveDialog(host.getWindow(), {
      defaultPath: host.savePath(name), filters: [{ name: value.format === 'ass' ? 'ASS' : 'SubRip', extensions: [String(value.format)] }],
    });
    if (chosen.canceled || !chosen.filePath) return null;
    const method = value.format === 'ass' ? 'subtitles.prepare' : 'subtitles.save';
    const params = value.format === 'ass'
      ? { cues: value.cues, asset_id: value.asset_id,
        ...(composition ? { canvas: { width: composition.canvas.width, height: composition.canvas.height } } : {}), ...(style ? { style } : {}), ...(editing ? { editing } : {}) }
    ...
    return { saved: true, library_linked: await host.library.recordLink(source, 'subtitle', chosen.filePath) };
  });
  wire('save-video', async input => {
    ...
    if (!artifactPath || !artifactPath.startsWith(path.join(host.workspace, 'renders') + path.sep)) {
      throw new Error('UNKNOWN_ASSET');
    }
    const chosen = await dialog.showSaveDialog(host.getWindow(), {
      defaultPath: host.savePath('processed.mp4'), filters: [{ name: 'MP4', extensions: ['mp4'] }],
    });
    if (chosen.canceled || !chosen.filePath) return null;
    await saveChosenExport(artifactPath, chosen.filePath, media.originalPaths);
    let linked = await host.library.recordLink(media.artifactSource(artifactId), 'export', chosen.filePath);
    const composition = media.artifactComposition(artifactId);
    if (composition) linked = (await host.library.recordCompositionLinks(composition, 'export', chosen.filePath)) && linked;
