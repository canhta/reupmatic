...
import type {
  ...
  LibraryItem,
  LibraryLinkKind,
} from '../../../core/library/library-types.js';
...
export async function installLibrary(host: Host) {
  ...
  try {
    ...
    service = new Service(store, path.join(host.workspace, 'library-originals'), (filename) =>
      host.media.registerVideo(filename),
    );
  ...
  host.wire('library-open-project', async (input) => {
    ...
    const item = required().store.get(id);
    const linked = item.links.find(
      (link) => link.id === requestId(value.link_id) && link.kind === 'project',
    );
    ...
    const anchor =
      item.sha256 === project.source.sha256
        ? item
        : required().store.findContent(project.source.sha256);
  ...
  host.wire('library-dependencies', (input) => {
    ...
    return {
      item,
      pending_jobs: host.pendingJobs(item),
  ...
  host.wire('library-forget', (input) => {
    ...
    if (host.pendingJobs(item) || related?.pending_posts || related?.workflows)
  ...
  return {
    get active() {
      return Boolean(importing) || links.size > 0;
    },
    get service() {
      return service;
    },
    ...
    recordBatchOutput(itemId: string, sha256: string, filename: string, outputHash: string) {
      return trackLink(async () => {
        ...
        try {
          item = service.store.get(itemId);
        } catch (error) {
          if (errorCode(error) === 'LIBRARY_ITEM_MISSING') return;
          throw error;
        }
    ...
    recordCompositionLinks(
      ...
      return trackLink(async () => {
        ...
        for (const sha256 of new Set(composition.clips.map((clip) => clip.source.sha256))) {
          ...
          if (!item) continue;
          try {
            await new LibraryAssets(service.store).register(item.id, kind, filename);
          } catch {
            linked = false;
          }
        }
