...
interface Host {
  ...
  /** Attaches a connector item's hashtags as taxonomy labels; see `ContentLibraryOptions`. */
  assignTags?(contentId: string, tags: readonly DouyinTag[]): void;
}

/**
 * Probes a picked local file, dispatching on its extension to the media-registry kind that
 * extension names. The registry computes the hash and probes duration, so core receives probed
 * facts and never has to infer a kind from a filename itself.
 */
...
export async function installLibrary(host: Host) {
  ...
  try {
    ...
    contentLibrary = new Library(
      ...
      path.join(host.workspace, 'library-originals'),
      {
        ...
        // Covers come from FFmpeg, which Python owns, so core receives this as an injected step.
        // A local import has no cover of its own; without a poster frame half the Library would
        // render blank cells (UI-CM06). Only a video has a poster frame,
        // so an audio/subtitle row records the designed `unavailable` state instead of a failed
        // video probe.
        ...
        // Labels live in the catalog's taxonomy, so the Library resolves a label facet through
        // this injected read rather than owning a second tag table.
        contentLabels: () => host.contentLabels?.() ?? [],
        // A Douyin item's hashtags become `tag` labels in that same taxonomy, through the one
        // `assignDouyinTags` map (ticket 09). A local import never has intake hashtags. Omitted
        // entirely when the host has no taxonomy, so an intake with tags fails loudly instead of
        // registering an item with its hashtags silently dropped.
  ...
  host.wire('library-list', (input) => {
    ...
    // Covers are local files, and the renderer CSP forbids `file://`; publish each ready cover
    // through the media registry so the row can render `media://local/...`. This is a read path
    // only — it never mutates the record, and a missing path never fails the listing.
  ...
  async function resolve(id: string): Promise<RegisteredVideo> {
    try {
      ...
      // The Editor opens video sources only. An audio or subtitle record still resolves for
      // verification (availability is recorded above), but there is nothing here to open.
  ...
  // "Open in Editor" with a project already open adds the resolved original as
  // a Project media row instead of replacing the open project (D-63).
