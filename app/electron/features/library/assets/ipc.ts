...
export function installLibraryAssets(host: Host, getStore: () => LibraryStore, changed: () => void): void {
  ...
  host.wire('library-asset-attach', async input => {
    ...
    if (!['project', 'subtitle', 'export', 'audio'].includes(kind)) throw new Error('INVALID_REQUEST');
    getStore().get(itemId);
    const filters = kind === 'audio' ? audioFilters : kind === 'export' ? videoFilters
      : [{ name: kind === 'project' ? 'Reupmatic project' : 'Subtitles', extensions: kind === 'project' ? ['json'] : ['srt', 'ass'] }];
    const chosen = await dialog.showOpenDialog(host.getWindow(), { properties: ['openFile'], filters });
  ...
  host.wire('library-asset-check', async input => {
    ...
    const item = requestId(value.item_id), link = requestId(value.link_id);
    try {
  ...
  host.wire('library-asset-preview', async input => {
    ...
    const itemId = requestId(value.item_id), linkId = requestId(value.link_id);
    ...
    if (link.kind === 'export') {
      ...
      result = { kind: link.kind, name: link.name, url: host.media.publicVideo(video).url, duration_ms: video.duration_ms };
      ...
      result = { kind: link.kind, name: link.name, url: audio.url, duration_ms: audio.source.duration_ms };
    } else {
