...
export async function installCatalog(host: Host) {
  ...
  try {
    const { WorkspaceCatalog: Catalog } = await import('../../../core/catalog/workspace-catalog.js');
    catalog = new Catalog(path.join(host.workspace, 'catalog.sqlite'));
  } catch (error) { failure = errorCode(error); }
  const context: CatalogHost = {
    ...host,
    catalog() { if (!catalog) throw new Error(failure); return catalog; },
    library() { const library = host.library(); if (!library) throw new Error('LIBRARY_UNAVAILABLE'); return library; },
    changed() {
  ...
  return {
    get active() { return workflows.active; },
    dependencies(id: string) { return context.catalog().dependencies(id); },
    async close() { await workflows.close(); catalog?.close(); },
  };
}
