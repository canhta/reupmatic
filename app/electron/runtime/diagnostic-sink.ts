...
/**
 * The one host-owned Diagnostic log sink (D-61). Every producer — main, core, renderer and the
 * worker stderr reader — funnels through it, because redaction and the wire format happen inside
 * it and nowhere else.
 *
 * Writes are synchronous: an ordinary crash cannot orphan a buffered record, so "flush" here
 * means finalising the file rather than draining a buffer.
 */

export const DIAGNOSTIC_FILE = 'reupmatic-diagnostics.ndjson';
/** Rotate at 5 MB keeping the 5 most recent files per install (D-61). */
export const MAX_FILE_BYTES = 5 * 1024 * 1024;
export const RETAINED_FILES = 5;

/** A non-sensitive summary of the machine, the only thing a support bundle adds to the records. */
...
export interface DiagnosticSink extends DiagnosticRecorder {
  /** Where a developer or agent points: the directory holding the retained Diagnostic logs. */
  readonly directory: string;
  /** Writes the retained logs plus `summary` to `destination`; returns what it wrote. */
  exportBundle(destination: string, summary: SystemSummary): { path: string; records: number };
  /** Finalises the file. Records offered afterwards are dropped, not buffered. */
  close(): void;
}

export interface DiagnosticSinkConfig {
  directory: string;
  /** `info` in production; `debug` only through the environment, never a Settings control. */
...
/** `REUPMATIC_DIAGNOSTIC_LEVEL=debug` raises verbosity on a machine the developer controls. */
...
/** Newest first: the live file, then `.1`, `.2`, … as rotation pushed them back. */
...
export function createDiagnosticSink(config: DiagnosticSinkConfig): DiagnosticSink {
  ...
  /** Creates the directory and picks up an existing file's size, once per open sink. */
  function open(): void {
    if (ready) return;
    try {
  ...
  function rotate(): void {
    // The retained window is the live file plus `retain - 1` rotated ones. Drop the oldest, shift
    // each survivor back one rung, then move the live file into `.1`.
  ...
  return {
    ...
    exportBundle(destination, summary) {
      ...
      // Oldest first, so the bundle reads forwards in time like the log itself.
      const lines: string[] = [
        JSON.stringify(
          redact(
            validateDiagnosticRecord({
      ...
      for (const name of [...files].reverse()) {
        ...
        // Already redacted on the way in; re-read as records so a hand-edited file cannot
        // smuggle anything unvalidated into the bundle.
