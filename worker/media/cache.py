...
class RenderCache:
    ...
    def read(self, req):
        try:
            ...
            if (not isinstance(manifest, dict) or manifest.get("recipe") != self.recipe
                    or sha256(self.output, lambda: self.host.cancelled(req)) != manifest["sha256"]
                    or not isinstance(manifest.get("media"), dict)):
                return None
            media = manifest["media"]
            if (type(media.get("duration_ms")) is not int or media["duration_ms"] <= 0
                    or type(media.get("width")) is not int or type(media.get("height")) is not int):
                return None
    ...
    def publish(self, req, temporary: Path, media: dict, details=None):
        ...
        staging.write_text(json.dumps(manifest, ensure_ascii=False, allow_nan=False), encoding="utf-8")
        self.host.cancelled(req)
    ...
    def result(self, manifest, cache_hit):
        result = {**manifest["media"], "artifact_id": self.key, "path": str(self.output),
                  "sha256": manifest["sha256"], "cache_hit": cache_hit,
                  "start_ms": self.recipe["start_ms"], "end_ms": self.recipe["end_ms"]}
