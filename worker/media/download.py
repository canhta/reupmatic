"""Streams a remote media URL to a caller-chosen path, then proves the bytes are playable.

The FFmpeg probe is the completion fact: an HTTP 200 and a metadata row are claims, not a usable
file (SC-01 SL-AC03). Bytes land in `<dest>.part`, and only an atomic rename publishes them, so a
failed, cancelled or truncated transfer can never leave a half-file at the destination.

stdlib only: urllib for the transfer, no new dependency.
"""
...
_CHUNK = 256 * 1024
# One socket timeout governs both the initial connect and every subsequent read: Python's socket
# timeout applies per blocking call, so a server that keeps sending *some* bytes never trips it,
# only a read that makes no progress for this long does — which is exactly D-62's stall
# definition. Env-configurable so a test can shrink it instead of waiting out a real 60s stall.
...
def _refused(req: dict, code: str, phase: str, host: str, exc: BaseException | None, **detail):
    """Every refused transfer owes the status, phase and CDN host that explain it (D-61): the
    host-side error code alone cannot tell an expired signed URL (403) from a dead mirror."""
    diagnostics.emit(
                ...
                with part.open(mode) as handle:
                    while True:
                        ...
                        try:
                            ...
                            # A read that made no progress for `_TIMEOUT` seconds — the socket
                            # timeout applies per blocking call, so this specifically means the
                            # transfer stalled mid-flight, not that the connection never opened
                            # (that is the "connect" phase above, still DOWNLOAD_FAILED).
                            _refused(
                                req,
                                "DOWNLOAD_STALLED",
                                "read",
