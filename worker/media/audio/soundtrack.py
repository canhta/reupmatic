...
def probe_audio(host, req, path):
    ...
    audio = next(
        (stream for stream in data.get("streams", []) if stream.get("codec_type") == "audio"), None
    )
    if not audio:
...
def resolve_soundtrack(host, req, value):
    ...
    # A muted music lane is absent from the mix: its file is never opened and it never enters the
    # graph, so its `replace` mode cannot suppress the original audio either.
    if p["muted"]:
        return None
    source = host.assets.verify(p["asset_id"], "audio", lambda: host.cancelled(req))
    ...
    if p["mode"] not in ("replace", "mix"):
        raise WorkerError("INVALID_SOUNDTRACK")
    duck = exact(p["duck"], {"enabled", "amount_db", "release_ms"})
    if (
        type(duck["enabled"]) is not bool
        or type(duck["amount_db"]) not in (int, float)
        or not math.isfinite(duck["amount_db"])
        or not 1 <= duck["amount_db"] <= 24
        or type(duck["release_ms"]) is not int
        or not 10 <= duck["release_ms"] <= 5000
    ):
        raise WorkerError("INVALID_SOUNDTRACK")
...
def fingerprint(track):
    return (
        {key: value for key, value in track.items() if key not in ("path", "asset_id")}
        if track
        else None
    )
