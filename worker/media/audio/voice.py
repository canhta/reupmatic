...
# The rates the synthesis contract can report (mirrors speech.synthesis.contracts.SAMPLE_RATES).
# A voice recording is never assumed to be at the mix bus rate; its own rate converts the plan's
# frame spans into the seconds the audio filters place and trim by.
...
def resolve_voice(host, req, value):
    ...
    The asset is admitted through the same registration the soundtrack uses, so a caller-supplied
    filesystem path is never trusted. Each line carries its placement offset and rate from the
    voice timing plan, and the frame span of its own speech inside the recording — the producer's
    inter-line silence is never part of a line and is never placed.
    """
    if value is None:
        return None
    p = exact(
    ...
    if type(p["muted"]) is not bool:
        raise WorkerError("INVALID_VOICE")
    # A muted voice lane is absent from the mix: its recording is never opened and its `replace`
    # mode cannot suppress the original audio either.
    if p["muted"]:
        return None
    ...
    for line in lines:
        ...
        # The plan's spans come from one recording, in cue order and never overlapping; a param
        # that folds a later line back over an earlier one is not a plan this worker will place.
