...
def audio_arguments(
    source_index, track_index, track, has_source, edit, start, end, duration, source_offset_ms=0
):
    ...
    graph = [
        f"[{track_index}:a:0]"
        + ",".join(soundtrack_filters(track, output_start, duration))
        + "[music]"
    ]
    if original and track["mode"] == "mix":
        filters = audio_filters(
            start - source_offset_ms, end - source_offset_ms, speed, edit.get("audio", {}), duration
        )
        filters += ["aresample=48000", "aformat=sample_fmts=fltp:channel_layouts=stereo"]
        graph += [
            f"[{source_index}:a:0]" + ",".join(filters) + "[original]",
