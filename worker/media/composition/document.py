...
def parse_composition(value):
    try:
        p = exact(value, {"version", "canvas", "clips"})
        canvas = exact(p["canvas"], {"width", "height", "fps"})
        width = number(canvas["width"], 2, 4096)
        height = number(canvas["height"], 2, 4096)
        if type(p["version"]) is not int or p["version"] != 1 or type(canvas["fps"]) is not int or canvas["fps"] != 30:
            raise WorkerError("INVALID_COMPOSITION")
        if width % 2 or height % 2 or not isinstance(p["clips"], list) or not 1 <= len(p["clips"]) <= 64:
            raise WorkerError("INVALID_COMPOSITION")
        ids, spans, offset = set(), [], 0
        for item in p["clips"]:
            clip = exact(item, {"id", "source", "start_ms", "end_ms", "speed"})
            source = exact(clip["source"], {"asset_id", "sha256", "duration_ms"})
            if (not isinstance(clip["id"], str) or not re.fullmatch(r"[a-zA-Z0-9_-]{1,128}", clip["id"])
                    or clip["id"] in ids or not isinstance(source["asset_id"], str)
                    or not 1 <= len(source["asset_id"]) <= 128 or not isinstance(source["sha256"], str)
            ...
            speed = number(clip["speed"], .25, 4, False)
            length = math.floor((end - start) / speed + .5)
            if length < 100:
