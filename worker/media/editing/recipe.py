...
def resolve_fade_window(editing, duration, sample):
    """Resolve a render window and, when a head/tail fade overlaps a sample, widen it
    to cover the fade so the sample's frames match the full render.

    FFmpeg's `fade` cannot start before zero, so a sample that begins inside the
    fade-in (or ends inside the fade-out) is decoded/assembled from the fade's own
    start; the fade is then applied on the full output clock and the result trimmed
    back to the requested sample. Returns `(window, output_sample, full_output_ms)`;
    `output_sample` is `(start_ms, end_ms)` on the OUTPUT clock only when a fade
    touches the sample, else None.
    """
    edit = parse_editing(editing) if editing is not None else {}
    ...
    if not head and not tail:
        return window, None, full_ms
    # A sample already anchored at the output head/tail needs no extra decode.
