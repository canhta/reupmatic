...
def render(host: WorkerContext, req: dict) -> dict:
    p = exact(req["params"], {"asset_id", "mode", "encoding"}, {"subtitle_id", "start_ms", "end_ms", "soundtrack", "composition"})
    ...
    subtitle = host.assets.verify(p["subtitle_id"], "subtitle", check) if p.get("subtitle_id") else None
    soundtrack = resolve_soundtrack(host, req, p.get("soundtrack"))
    start, end = window["start_ms"], window["end_ms"]
    recipe = {"schema": 1, "renderer": "ffmpeg-media-0.12.0", "source": source["sha256"],
              "subtitle": subtitle["sha256"] if subtitle else None, "soundtrack": fingerprint(soundtrack), "start_ms": start,
              "end_ms": end, "source_offset_ms": source_offset, "encoding": p["encoding"], "runtime": host.runtime_identity}
    ...
    with tempfile.TemporaryDirectory(dir=host.workspace, prefix="render-") as tmp:
        ...
        encode_video(host, req, source["path"], partial, start_ms=start, end_ms=end,
                     encoding=p["encoding"], subtitle=subtitle["path"] if subtitle else None, soundtrack=soundtrack, source_offset_ms=source_offset)
