...
def make_recognizer(directory: str):
    ...
    return WhisperModel(directory, device='cpu', compute_type='int8', cpu_threads=2,
...
def run(job: dict, progress: Path) -> dict:
    ...
    segments, info = model.transcribe(
        job['audio'], language=job['language'], task='transcribe', beam_size=5,
        temperature=0, condition_on_previous_text=False, word_timestamps=False, vad_filter=False,
    )
    if info.language != job['language']:
        raise WorkerError('MODEL_LANGUAGE_UNAVAILABLE')
    duration = job['end_ms'] - job['start_ms']
    cues = timed_segments(segments, job['start_ms'], job['end_ms'],
                          lambda completed: atomic_json(progress, {'completed_ms': completed,
                                                                    'duration_ms': duration}))
    verify_bundle(job['model'])
    runtime = ';'.join(f'{name}@{importlib.metadata.version(name)}'
