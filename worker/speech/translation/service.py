...
ERRORS = {'MODEL_HASH_MISMATCH', 'MODEL_RUNTIME_MISSING', 'MODEL_INFERENCE_FAILED',
          'MODEL_NETWORK_DISABLED', 'MODEL_OUTPUT_INVALID', 'TRANSLATION_MANIFEST_INVALID',
          'MODEL_MISSING', 'TRANSLATION_TOKEN_LIMIT', 'TRANSLATION_TRUNCATED',
          'TRANSLATION_RESULT_TOO_LARGE', 'TRANSLATION_LIMIT'}
...
def translate(host, req: dict) -> dict:
    ...
    model = host.translation_models.require(p['source_language'], p['target_language'],
                                            expected_id=p['model_id'], check=check)
    if shutil.disk_usage(host.workspace).free < 16 * 1024**2:
        raise WorkerError('TRANSLATION_DISK_LOW')
    emit = lambda fraction: host.emit(req, 'progress', {'phase': 'translationRunning', 'fraction': fraction})
    with tempfile.TemporaryDirectory(dir=host.workspace, prefix='translation-') as directory:
        tmp = Path(directory)
        path = tmp / 'request.json'
        path.write_text(json.dumps({'params': p, 'model': model}, ensure_ascii=False), encoding='utf-8')
        emit(None)
        last = -1
        ...
        def progress():
            ...
            try:
                ...
                if (type(completed) is int and last < completed <= len(p['cues'])
        ...
        host.process.run(req, [sys.executable, '-u', '-m', 'speech.translation.runner', str(path)],
