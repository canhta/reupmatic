...
def parse_rules(value: object) -> list[dict]:
    ...
    for rule in value:
        ...
        if (
            any(
                not isinstance(rule[key], str) or "\x00" in rule[key] or units(rule[key]) > 256
                for key in ("find", "replace")
            )
            or not rule["find"].strip()
        ):
            raise WorkerError("INVALID_REQUEST")
...
def parse_options(value: object) -> dict:
    p = exact(value, KEYS)
    if (
        p["source_layer"] not in ("transcript", "displayed")
        or any(
            not isinstance(p[key], str) or p[key] not in LANGUAGES
...
def validate_output(value: object, params: dict) -> dict:
    if (
        ...
        or set(value) != {"cues", "runtime"}
        or not isinstance(value["runtime"], str)
        or not 0 < units(value["runtime"]) <= 128
