...
FIELDS = {"engine", "directory", "source_language", "target_language", "files"}


def runtime_available() -> bool:
    try:
        return all(
            importlib.util.find_spec(name) is not None for name in ("ctranslate2", "sentencepiece")
        )
...
class TranslationRegistry:
    ...
    def _read(self) -> dict:
        try:
            ...
            if (
                ...
                or any(
                    not isinstance(value[key], str) or value[key] not in LANGUAGES
            ...
            if (
                ...
                or files.keys() - ALLOWED
                or files.keys() & VOCABULARIES
                not in (
            ...
            identity = {
                key: bundle[key]
                for key in ("engine", "source_language", "target_language", "files")
            }
    ...
    def status(self) -> dict:
        ...
        try:
            bundle = self._read()
            value.update(
                {key: bundle[key] for key in ("model_id", "source_language", "target_language")}
            )
...
def unconfigure_translation(host, req: dict) -> dict:
    """Forget the configured direction when it is the bundle at `directory`, so deleting that
