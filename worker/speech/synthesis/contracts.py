...
def parse_options(value: object) -> dict:
    ...
    if (p['source_layer'] != 'spoken' or p['language'] not in ('en', 'vi')
    ...
    try:
        if (not 1 <= len(p['cues']) <= 100
                or any('style' in c or not c['text'].strip()
                       or len(c['text'].encode('utf-16-le')) // 2 > 300 for c in p['cues'])
...
def validate_audio(value: object, params: dict) -> dict:
    ...
    if (not isinstance(value, dict) or set(value) != keys or not clean(value['runtime'], 256)
            or type(value['frames']) is not int or not 1 <= value['frames'] <= MAX_FRAMES
            or not isinstance(value['segments'], list) or len(value['segments']) != len(params['cues'])):
        raise WorkerError('MODEL_OUTPUT_INVALID')
    start = 0
    for cue, segment in zip(params['cues'], value['segments']):
        if (not isinstance(segment, dict) or set(segment) != {'cue_id', 'start_frame', 'end_frame'}
                or segment['cue_id'] != cue['id'] or type(segment['start_frame']) is not int
                or segment['start_frame'] != start or type(segment['end_frame']) is not int
...
def make_result(params: dict, audio: dict, artifact_id: str, digest: str) -> dict:
    return {'kind': 'synthesis', **params, **audio, 'artifact_id': artifact_id, 'sha256': digest,
            'sample_rate': RATE, 'duration_ms': math.ceil(audio['frames'] * 1000 / RATE),
            'timing': 'natural-sequential', 'gap_ms': 250}
