...
from speech.synthesis.contracts import MAX_FRAMES, MAX_RESULT, RATE, make_result, parse_options, validate_audio
from speech.synthesis.models import verify_bundle

ERRORS = {'MODEL_HASH_MISMATCH', 'MODEL_RUNTIME_MISSING', 'MODEL_INFERENCE_FAILED', 'MODEL_NETWORK_DISABLED',
          'MODEL_OUTPUT_INVALID', 'SYNTHESIS_MANIFEST_INVALID', 'SYNTHESIS_VOICES_INVALID', 'MODEL_MISSING',
          'SYNTHESIS_RUNTIME_VERSION', 'SYNTHESIS_TOKEN_LIMIT', 'SYNTHESIS_LIMIT', 'SYNTHESIS_VOICE_UNAVAILABLE'}
...
def read_audio(directory: Path, params: dict) -> dict:
    try:
        ...
        if response.get('ok') is not True:
            ...
            raise WorkerError(code if isinstance(code, str) and code in ERRORS else 'MODEL_INFERENCE_FAILED')
        if set(response) != {'ok', 'data'}:
            raise ValueError
        data = validate_audio(response['data'], params)
        path = directory / 'speech.wav'
        if path.is_symlink() or not path.is_file() or path.stat().st_size != 44 + data['frames'] * 2:
            raise ValueError
        with wave.open(str(path), 'rb') as audio:
            if audio.getparams()[:4] != (1, 2, RATE, data['frames']) or audio.getcomptype() != 'NONE':
                raise ValueError
...
def synthesize(host, req: dict) -> dict:
    ...
    model = host.synthesis_models.require(params['language'], params['voice_id'], params['model_id'], check)
    ...
    emit = lambda fraction: host.emit(req, 'progress', {'phase': 'synthesisRunning', 'fraction': fraction})
    with tempfile.TemporaryDirectory(dir=root, prefix='.synthesis-') as directory:
        tmp = Path(directory)
        path = tmp / 'request.json'
        path.write_text(json.dumps({'params': params, 'model': model}, ensure_ascii=False), encoding='utf-8')
        emit(None)
        last = -1
        ...
        def progress():
            ...
            try:
                ...
                if (type(completed) is int and last < completed <= len(params['cues'])
                        and type(value.get('total')) is int and value['total'] == len(params['cues'])):
                    last = completed
                    emit(completed / len(params['cues']))
            except (OSError, ValueError, AttributeError):
                pass

        host.process.run(req, [sys.executable, '-u', '-m', 'speech.synthesis.runner', str(path)],
                         cwd=Path(__file__).resolve().parents[2], timeout=3600, on_poll=progress)
        progress()
        check()
