...
The child dispatches to whichever architecture the configured bundle names
(`speech.synthesis.models.ENGINES`); the adapter owns execution and writes the WAV,
this module only validates the request and the produced metadata. Both architectures
produce the same result shape at whatever rate the engine reports.
"""
