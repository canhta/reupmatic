...
def runtime_code() -> str | None:
    try:
        ...
        if not all(importlib.util.find_spec(name) for name in ('vieneu', 'numpy', 'onnxruntime', 'tokenizers')):
...
def read_voices(root: Path) -> list[dict]:
    try:
        ...
        if len(raw) > 2 * 1024**2:
            raise ValueError
        data = json.loads(raw)
        if not isinstance(data, dict) or set(data) != {'version', 'voices'} or type(data['version']) is not int or data['version'] != 1:
            raise ValueError
        voices = data['voices']
        ...
        seen = set()
        for v in voices:
            if (not isinstance(v, dict) or set(v) != {'id', 'label', 'speaker_emb', 'ref_codes'}
                    or not clean(v['id'], 128) or not clean(v['label'], 160) or v['id'] in seen):
                raise ValueError
            seen.add(v['id'])
            emb, codes = v['speaker_emb'], v['ref_codes']
            if (not isinstance(emb, list) or len(emb) != 192
                    or any(type(n) not in (float, int) or not math.isfinite(n) or abs(n) > 1000 for n in emb)
                    or not any(emb) or not isinstance(codes, list) or not 1 <= len(codes) <= 500):
                raise ValueError
            width = len(codes[0]) if isinstance(codes[0], list) else 0
            if (not 1 <= width <= 32 or any(not isinstance(row, list) or len(row) != width
                    or any(type(n) is not int or not 0 <= n < 65536 for n in row) for row in codes)):
                raise ValueError
        return voices
...
class SynthesisRegistry:
    ...
    def read(self) -> dict:
        try:
            ...
            if (not isinstance(value, dict) or set(value) != FIELDS or type(value['version']) is not int
                    or value['version'] != 1 or value['engine'] != ENGINE):
                raise ValueError
            langs, files, directory = value['languages'], value['files'], value['directory']
            if (not isinstance(langs, list) or not 1 <= len(langs) <= 2
                    or any(l not in ('en', 'vi') for l in langs) or len(set(langs)) != len(langs)
                    ...
                    or not isinstance(directory, str) or not directory or len(directory) > 4096
    ...
    def status(self) -> dict:
        value = {'available': False, 'code': None, 'model_id': None, 'languages': [], 'voices': [], 'verified': False}
        try:
            bundle = self.read()
            value.update(model_id=bundle['model_id'], languages=bundle['languages'],
                         voices=[{k: v[k] for k in ('id', 'label')} for v in read_voices(Path(bundle['directory']))])
            code = runtime_code()
            if code:
                raise WorkerError(code)
