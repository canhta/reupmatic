...
def process_video(host, req):
    ...
    recipe = {
        ...
        # The requested sample on the output clock (None for a full render): two
        # samples that widen to the same fade-corrected window still render and
        # cache differently.
...
def resolve_logo(host, req, value, editing):
    """The project media image a logo placement draws, when the recipe places one."""
    if value is None:
        return None
