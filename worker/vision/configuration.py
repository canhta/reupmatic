...
def _read_store(destination: Path) -> dict | None:
    """Read the raw store without resolving its files, so a store whose bundle is already gone can
    still be merged into or cleaned. `None` means there is no store."""
    try:
...
def merge_models(host, req: dict) -> dict:
    """Add one manifest's capabilities to the store instead of replacing it. The offered catalogue
    has independent vision entries — OCR packs and an inpainting model — that share the one manifest
    the adapter reads, so installing the second must not erase the first. `configure_models` stays
    the replace path for the hand-written picker, whose manifest is the complete configuration."""
    candidate = _candidate(req)
...
def unconfigure_models(host, req: dict) -> dict:
    """Forget only the sections whose artifacts live under `directory`, so deleting one offered
    bundle leaves the others configured and no store pointing at files that no longer exist."""
