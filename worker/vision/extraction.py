...
def extract_ocr(host, req):
    ...
    options = {key: value for key, value in params.items() if key not in ("asset_id", "start_ms", "end_ms")}
    processing = {"version": 1, "ocr": {key: options[key] for key in ("language", "sample_ms", "min_confidence")}}
    ...
    with tempfile.TemporaryDirectory(dir=host.workspace, prefix="ocr-extract-") as directory:
        ...
        staging.mkdir()
        result = scan_cues(host, req, options, 0, params["end_ms"], staging, fingerprints, keep_evidence=True)
        chunks = result.pop("chunks")
        result.update({"kind": "ocr", "asset_id": params["asset_id"], "source_sha256": source["sha256"],
                       "start_ms": 0, "end_ms": params["end_ms"], "language": params["language"],
                       "sample_ms": params["sample_ms"], "analysis_id": analysis_id,
                       "model_fingerprints": {"ocr": fingerprints["ocr_" + params["language"]]},
                       "algorithm_version": "ocr-extraction-0.10.0", "scope": "full-source",
                       "evidence": {"chunks": len(chunks), "preview_count": len(result["observations"]),
        ...
        (staging / "chunks.json").write_text(json.dumps(chunks, ensure_ascii=False), encoding="utf-8")
