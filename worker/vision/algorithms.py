...
def pixel_rect(value: dict, width: int, height: int) -> tuple[int, int, int, int]:
    value = region(value)
    return (max(0, math.floor(value["x"] * width)),
...
def timed_cues(observations: list[dict]) -> list[dict]:
    ...
    for item in observations:
        ...
        text = "\n".join(unicodedata.normalize("NFC", line["text"]).strip()
        ...
        if not text:
            adjacent = False
            continue
        if (adjacent and cues and cues[-1]["text"] == text
                and cues[-1]["end_ms"] == item["start_ms"]):
            cues[-1]["end_ms"] = item["end_ms"]
        else:
            cues.append({"id": f"ocr-{len(cues) + 1:06d}", "start_ms": item["start_ms"],
                         "end_ms": item["end_ms"], "text": text})
        adjacent = True
    return cues


def make_mask(width: int, height: int, rectangle: dict | None,
              detections: list[dict], padding: int):
    import cv2
    import numpy as np
...
class RapidAdapter:
    ...
    def detect(self, rgb, *, confidence: float = .5, rectangle: dict | None = None) -> list[dict]:
        import cv2
        import numpy as np

        height, width = rgb.shape[:2]
        x0, y0, x1, y1 = pixel_rect(rectangle, width, height) if rectangle else (0, 0, width, height)
        result = self.engine(cv2.cvtColor(rgb[y0:y1, x0:x1], cv2.COLOR_RGB2BGR),
                             use_cls=False, text_score=confidence)
        ...
        for polygon, text, score in zip(boxes, texts, scores):
            ...
            if (points.shape != (4, 2) or not np.isfinite(points).all()
                    or not isinstance(text, str) or len(text) > 10000 or "\x00" in text
            ...
            if right > left and bottom > top:
                detections.append({"text": unicodedata.normalize("NFC", text.strip()),
                                   "confidence": float(score), "box": [left, top, right, bottom]})
        return detections
...
class LamaAdapter:
    ...
    def erase(self, rgb, mask):
        ...
        if (rgb.ndim != 3 or rgb.shape[2] != 3 or rgb.dtype != np.uint8
                or mask.shape != rgb.shape[:2] or mask.dtype != np.uint8):
            raise WorkerError("MODEL_OUTPUT_INVALID")
        ys, xs = np.where(mask > 0)
        if not len(xs):
            return rgb.copy()
        context = max(32, int(max(xs.max() - xs.min(), ys.max() - ys.min()) * .2))
        ...
        image = cv2.resize(crop, (rw, rh), interpolation=cv2.INTER_AREA if scale < 1 else cv2.INTER_LINEAR)
        ...
        feed = {"image": np.ascontiguousarray(image.transpose(2, 0, 1)[None], dtype=np.float32) / 255,
        ...
        if (output.shape != (1, 3, 512, 512) or not np.isfinite(output).all()
                or float(output.min()) < -1 or float(output.max()) > 256):
            raise WorkerError("MODEL_OUTPUT_INVALID")
