...
def parse_options(method: str, params: object) -> dict:
    ...
    required = common | ({"language", "sample_ms", "min_confidence"} if ocr else {"target", "padding_px"})
    allowed = required | {"region"} | (set() if ocr else {"language"})
    if (not isinstance(params, dict) or not required <= params.keys()
            or params.keys() - allowed
            or not isinstance(params["asset_id"], str) or not 0 < len(params["asset_id"]) <= 128
    ...
    if ocr:
        ...
        if (type(sample) is not int or not 100 <= sample <= 2000
                or type(confidence) not in (int, float) or not math.isfinite(confidence)
                or not 0 <= confidence <= 1):
            raise WorkerError("INVALID_REQUEST")
    else:
        if (params["target"] not in ("manual", "text") or type(params["padding_px"]) is not int
...
class VisionService:
    ...
    def run(self, req: dict, *, staging: Path | None = None, emit_progress=None) -> dict:
        ...
        stream_info = json.loads(host.process.run(req, [host.ffprobe, "-v", "error", "-select_streams", "v:0",
            "-show_entries", "stream=width,height,sample_aspect_ratio:stream_side_data=rotation",
            "-of", "json", str(source["path"])], timeout=30))["streams"][0]
        if stream_info.get("sample_aspect_ratio", "1:1") not in ("1:1", "0:1", "N/A"):
            raise WorkerError("VISION_FORMAT_UNSUPPORTED")
        rotation = next((d["rotation"] for d in stream_info.get("side_data_list", []) if "rotation" in d), 0)
        if rotation % 90:
            raise WorkerError("VISION_FORMAT_UNSUPPORTED")
        ...
        with tempfile.TemporaryDirectory(dir=host.workspace, prefix="vision-") as directory:
            ...
            host.process.run(req, [host.ffmpeg, "-v", "error", "-nostdin", "-threads", "2", "-ss", str(start),
                "-i", str(source["path"]), "-t", str(duration), "-map", "0:v:0", "-an", "-sn",
                "-vf", f"fps={rate}:start_time=0,scale={width}:{height},setsar=1",
                "-frames:v", str(max_frames), "-pix_fmt", "rgb24", "-f", "rawvideo", "-threads", "1", "-n", str(frames)])
            ...
            job = {**params, "kind": kind, "models": models, "width": width, "height": height,
                   "frame_count": count, "input_frames": str(frames), "output_frames": str(processed)}
            ...
            def progress():
                ...
                try:
                    ...
                    pass
            host.process.run(req, [sys.executable, "-u", "-m", "vision.runner", str(job_path)],
                         cwd=Path(__file__).resolve().parents[1], timeout=3600, on_poll=progress)
            progress()
            ...
            if response.get("ok") is not True:
                known = {"MODEL_HASH_MISMATCH", "MODEL_RUNTIME_MISSING", "MODEL_INFERENCE_FAILED",
                         "MODEL_NETWORK_DISABLED", "MODEL_SHAPE_UNSUPPORTED", "MODEL_OUTPUT_INVALID",
                         "VISION_FRAME_INVALID", "VISION_RESULT_TOO_LARGE"}
                raise WorkerError(response.get("code") if response.get("code") in known else "MODEL_INFERENCE_FAILED")
            data = response["data"]
            ...
            metadata = {"kind": kind, "asset_id": params["asset_id"], "source_sha256": source["sha256"],
                        "start_ms": params["start_ms"], "end_ms": params["end_ms"],
                        "width": width, "height": height,
                        "model_fingerprints": {key: bundle["fingerprint"] for key, bundle in models.items()},
            ...
            if ocr:
                ...
                if not isinstance(data.get("observations"), list) or len(data["observations"]) > 1200:
                    raise WorkerError("MODEL_OUTPUT_INVALID")
                result = {**metadata, **data, "language": params["language"],
                          "sample_ms": params["sample_ms"], "analysis_id": str(uuid.uuid4())}
            ...
            args = [host.ffmpeg, "-v", "error", "-nostdin", "-f", "rawvideo", "-pix_fmt", "rgb24",
                    "-s", f"{width}x{height}", "-r", "24", "-i", str(processed)]
            if staging:
                args += ["-map", "0:v:0", "-an", "-c:v", "ffv1", "-level", "3"]
            else:
                args += ["-ss", str(start), "-i", str(source["path"]), "-t", str(duration),
            ...
            if staging is None and file_hash(source["path"], check) != source["sha256"]:
                raise WorkerError("SOURCE_CHANGED")
            aid = str(uuid.uuid4())
            result = {**metadata, **data, "artifact_id": aid, "path": str((staging or host.cache) / f"{aid}{output.suffix}"),
                      "duration_ms": generated["duration_ms"], "has_audio": generated["has_audio"],
                      "fps": 24, "cache_hit": False, "sha256": file_hash(output, check)}
            check()
