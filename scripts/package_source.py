...
def version_details(root: Path) -> tuple[str, tuple[int, int, int], str]:
    ...
    if lock.exists():
        ...
        if (
            locked.get("version") != version
            or locked.get("packages", {}).get("", {}).get("version") != version
        ):
    ...
    def visit(directory: Path):
        for item in sorted(directory.iterdir()):
            ...
            if not item.is_file():
                continue
            if (
                item.name.lower() in {"agent.md", "agents.md", "claude.md"}
                and relative.as_posix() != "AGENTS.md"
            ):
...
def validate_declared_paths(root: Path, files: list[Path], version: str):
    ...
    if {item["scope"] for item in trace["features"]} != {
        f"SC-{index:02d}" for index in range(1, 15)
    }:
...
def build_archive(root: Path, output: Path) -> Path:
    ...
    metadata = {
        "format": "reupmatic.source-release",
        "version": version,
        "archive": destination.name,
        "kind": "source-only",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "verification": f"research/integration-{label}/RESULTS.md",
        "files": {},
    }
