// Stage a relocatable Python with the worker's dependencies under `python/`, for the current
// OS/arch. electron-builder copies it into the packaged app's resources (electron-builder.yml) and
// app/electron/main.ts resolves it when packaged. Run on each target OS — never cross-build.
...
// Pinned so a release is reproducible; bump both together and re-verify SHA256SUMS.
...
// Each domain's own probe, so a staged runtime that cannot actually run an adapter fails here
// rather than in a user's first job.
const PROBE = `
