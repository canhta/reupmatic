"""Compose gitignored wire schemas from tracked `*.source.schema.json` bases and domain shapes."""
...
# Hand-authored: which worker methods the runtime answers inline (no queue, no progress
# events) versus dispatches through the job queue. Not derivable from the request schema,
# which only knows parameter shape, not dispatch behavior.
INSTANT_METHODS = frozenset(
    {"cancel", "hello", "models.status", "speech.status", "synthesis.status", "translation.status"}
)

# Hand-authored: the progress phase values the worker's own generic accept/dispatch loop emits
# for every queued operation, before any handler-specific phase (e.g. "rendering",
# "processingOcr") takes over. Those handler-specific names stay domain-owned; only these two are
# seam-generic and worth a canonical, generated home. Keyed by the generated constant name.
...
def ruff_format(text):
    """Canonicalize generated Python through the project's own ruff formatter."""
    result = subprocess.run(
...
def operations_module(worker_request_schema):
    """Render worker/runtime/operations.py from the composed worker-request branches."""
