...
import { projectRoot, readSystemMap, location, checkStructure } from './system-structure.mjs';
...
export function checkTraceability(root = projectRoot) {
  ...
  const scopes = new Set(Array.from({ length: 14 }, (_, index) => `SC-${String(index + 1).padStart(2, '0')}`));
  if (trace.format !== 'reupmatic.scope-traceability' || trace.version !== 1 || trace.authority !== 'BUSINESS_SCOPE.md'
    || trace.source_version !== map.source_version) throw new Error('Invalid traceability version/authority');
  ...
  const moduleIds = item => {
    if (!item.modules?.length || item.modules.some(id => !modules.has(id))) throw new Error(`Unknown module: ${item.id}`);
  };
  const identity = (item, pattern) => {
    if (!pattern.test(item.id) || ids.has(item.id)) throw new Error(`Invalid/duplicate identity: ${item.id}`);
    ids.add(item.id);
    if (!['partial', 'planned'].includes(item.status) || !item.title?.trim() || !item.remaining?.trim()) {
  ...
  for (const feature of trace.features) {
    ...
    if (!scopes.has(feature.scope) || !feature.id.startsWith(feature.scope + '-')) throw new Error(`Unknown scope: ${feature.id}`);
    if (!feature.specs?.length) throw new Error(`Missing specification: ${feature.id}`);
    for (const spec of feature.specs) requireFile(spec);
    if (feature.status === 'partial' && !feature.delivered?.trim()) throw new Error(`Missing implemented slice: ${feature.id}`);
  }
  for (const flow of trace.flows) {
    identity(flow, /^FLOW-\d{2}$/);
    if (!flow.scope?.length || flow.scope.some(scope => !scopes.has(scope))) throw new Error(`Unknown flow scope: ${flow.id}`);
    if (!Array.isArray(flow.steps) || flow.steps.length < 2 || flow.steps.some(step => !step.trim())) throw new Error(`Incomplete flow: ${flow.id}`);
  }
  for (const scope of scopes) {
    if (!source.includes(`### ${scope} `) || !trace.features.some(item => item.scope === scope)
      || !trace.flows.some(flow => flow.scope.includes(scope))) throw new Error(`Unmapped scope: ${scope}`);
  }
  const expected = ['sources', 'editor', 'channels', 'automation', 'settings'];
  if (trace.screens.length !== 5 || new Set(trace.screens.map(screen => screen.area)).size !== 5) throw new Error('Five task areas required');
  expected.forEach((area, index) => {
    const screen = trace.screens.find(item => item.id === `SCR-0${index + 1}`);
    if (screen?.area !== area || !modules.has(screen.module)) throw new Error(`Missing screen: ${area}`);
  });
  if (trace.modes.length !== 3 || !['direct-editing', 'user-started-batch', 'automation'].every(mode => trace.modes.includes(mode))) {
  ...
  return { scopes: scopes.size, screens: trace.screens.length, modules: modules.size,
    features: trace.features.length, flows: trace.flows.length };
}

export function coverageMarkdown(trace) {
  const lines = ['# Scope delivery and remaining work — ' + trace.source_version, '',
    ...
    '## Five task areas', '', '| ID | Area | Owner |', '|---|---|---|'];
  for (const screen of trace.screens) lines.push(`| ${screen.id} | ${screen.area} | ${screen.module} |`);
  for (let number = 1; number <= 14; number++) {
    const scope = `SC-${String(number).padStart(2, '0')}`;
    lines.push('', '## ' + scope, '');
    for (const feature of trace.features.filter(item => item.scope === scope)) {
      lines.push(`### ${feature.id} — ${feature.title}`, '', `**${feature.status}** · Owners: ${feature.modules.join(', ')}`, '',
        `Delivered slice: ${feature.delivered || 'None.'}`, '', `Remaining: ${feature.remaining}`, '',
        `Authority: ${feature.specs.map(spec => '[' + spec + '](../../' + spec + ')').join(', ')}`, '');
    }
  }
  lines.push('## Cross-module flows', '');
  for (const flow of trace.flows) lines.push(`### ${flow.id} — ${flow.title}`, '',
    `**${flow.status}** · ${flow.scope.join(', ')} · Owners: ${flow.modules.join(', ')}`, '',
    flow.steps.join(' → '), '', 'Remaining: ' + flow.remaining, '');
  lines.push('## Acceptance and policy', '',
    ...
    'Open decisions are not defaults: ' + trace.open_policy_sources.map(name => '`' + name + '`').join(', ') + '.', '');
  return lines.join('\n');
}

export function systemMarkdown(map) {
  const lines = ['# System map — source ' + map.source_version, '',
    ...
    '## Capability ownership', '', '| Capability | Scope | State | Remaining work |', '| --- | --- | --- | --- |'];
  for (const module of map.modules) lines.push(`| ${module.title} | ${module.scope.join(', ')} | ${module.status} | ${module.remaining} |`);
  lines.push('', '## Safe maintenance', '',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (args.some(arg => arg !== '--write')) throw new Error('Usage: scope-traceability.mjs [--write]');
    checkStructure();
    ...
    for (const [relative, expected] of [
      ...
      else if (readFileSync(name, 'utf8') !== expected) throw new Error('Regenerate scope maps: npm run scope:generate');
    }
    console.log(report);
