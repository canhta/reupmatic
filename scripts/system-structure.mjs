...
export function readSystemMap(root = projectRoot) {
  const map = JSON.parse(
    readFileSync(path.join(root, 'docs/architecture/system-map.json'), 'utf8'),
  );
  ...
  for (const module of map.modules) {
    if (!module.id || ids.has(module.id) || !['partial', 'planned'].includes(module.status))
  ...
  if (map.scope_authority !== 'BUSINESS_SCOPE.md' || map.modules.length === 0)
    throw new Error('Missing scope authority');
  for (const module of map.modules) {
    if (!Array.isArray(module.scope) || !module.scope.length || !module.remaining?.trim())
      throw new Error(`Missing scope: ${module.id}`);
    if (module.status === 'planned' && module.implemented.length)
      throw new Error(`Planned implementation: ${module.id}`);
    if (module.status === 'partial' && !module.implemented.length)
      throw new Error(`Empty implementation: ${module.id}`);
    for (const name of [...module.implemented, ...module.reserved]) {
      if (name.includes('/routing') && module.id !== 'automation')
        throw new Error('Automation owns routing');
    }
  }
  return map;
}
...
function containsSource(directory) {
  return readdirSync(directory, { withFileTypes: true }).some((entry) => {
    ...
    return entry.isDirectory()
      ? containsSource(path.join(directory, entry.name))
...
export function checkStructure(root = projectRoot) {
  ...
  for (let n = 1; n <= 14; n++)
    if (!coverage.has(`SC-${String(n).padStart(2, '0')}`)) failures.push(`Unmapped SC-${n}`);
  for (const module of map.modules) {
    for (const name of module.implemented) {
      ...
      if (
        ...
        !containsSource(directory)
      )
        failures.push(`Missing implementation: ${name}`);
    }
    for (const name of module.reserved) {
      const marker = location(root, `${name}/.gitkeep`);
      if (!existsSync(marker) || !lstatSync(marker).isFile())
        failures.push(`Missing reserved boundary: ${name}`);
    }
  }
  const retained = JSON.parse(
    readFileSync(path.join(root, 'docs/architecture/retained-paths.json'), 'utf8'),
  );
  for (const name of retained.paths) {
    const actual = retained.moves[name] ?? name;
    if (!lstatSync(location(root, actual), { throwIfNoEntry: false })?.isFile())
      failures.push(`Delivered source removed without a mapped move: ${name}`);
  }
  if (failures.length) throw new Error(failures.join('\n'));
  return {
    modules: map.modules.length,
    reserved: map.modules.flatMap((module) => module.reserved).length,
  };
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    if (process.argv.slice(2).some((arg) => arg !== '--create'))
      throw new Error('Usage: system-structure.mjs [--create]');
    if (process.argv.includes('--create'))
      console.log(`Created ${scaffold().length} markers; existing files untouched.`);
